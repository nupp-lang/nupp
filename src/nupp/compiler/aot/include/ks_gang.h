 ...
 * every combination the lane lowering can choose has its typedefs here whichever
 * one a file picked: a typedef costs nothing, and defining them all once is what
 * keeps two gangs that share a vector, as the eight-lane pair does, from each
 * defining it.
 *
 * The helpers are another matter. GCC on x86 computes the vector ABI of every
 * function it sees, called or not, so a static inline helper returning a vector
 * wider than the register class the file is compiled for is a -Wpsabi error even
 * when no body calls it. The helpers over 32-byte vectors are therefore defined
 * only where the compiler announces AVX, and those over 64-byte vectors only
 * under AVX-512, which is exactly when a gang needing them can have been chosen.
 * Other architectures pass a wide vector by reference and say nothing, so they
 * keep every helper. Within one width the helpers hold to one rule: none of them
 * may call another, because GCC weighs a call for inlining before it drops the
 * caller as unused, and weighing it computes the callee's vector ABI.
 *
...
#define KS_GANG_HAS_32 1
#define KS_GANG_HAS_64 1
#else
#if defined(__AVX__)
#define KS_GANG_HAS_32 1
#endif
...
/* One gang width: the vectors at `lanes` lanes, 32-bit elements in `bytes32`
 * bytes and 64-bit ones in twice that. Typedefs only; the helpers over each
 * vector width follow, under that width's guard. */
...
/* The helpers over the 32-bit and the 64-bit elements of one gang. */
#define KS_GANG_NARROW(lanes) \
    KS_GANG_VALUE(f32, float, 32, lanes) \
    KS_GANG_VALUE(i32, int32_t, 32, lanes) \
    KS_GANG_VALUE(u32, uint32_t, 32, lanes)
#define KS_GANG_WIDE(lanes, bytes64) \
...
/* 16-byte vectors: every target has these. */
KS_GANG_NARROW(2)
KS_GANG_WIDE(2, 16)
KS_GANG_NARROW(4)
...
#if defined(KS_GANG_HAS_32)
KS_GANG_WIDE(4, 32)
KS_GANG_NARROW(8)
KS_GANG_MASK(32, 32, 8)
#endif

#if defined(KS_GANG_HAS_64)
KS_GANG_WIDE(8, 64)
#endif
