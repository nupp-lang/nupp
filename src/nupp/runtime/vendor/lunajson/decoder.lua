...
-- Calculate the negative zero at runtime so positive JSON zero stays positive.
local negativeZero = -1 / inf
local mininteger = rawget(math, "mininteger")
...
if _VERSION == "Lua 5.1" then
    -- use the cluttered pattern because lua 5.1 does not handle \0 in a pattern
    -- correctly
...
local function newdecoder()
    ...
    local radixmark = assert(match(tostring(0.5), '[^0-9]'))
    ...
    local function f_str_subst(ch, ucode)
        if ch == 'u' then
            ...
            ucode = f_str_hextbl[
                c1 - 47
            ] * 0x1000 + f_str_hextbl[c2 - 47] * 0x100 + f_str_hextbl[c3 - 47] * 0x10 + f_str_hextbl[c4 - 47]
            if ucode ~= inf then
