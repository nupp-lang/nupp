...
/* SHA-256 lives in the native provider, which the host links whatever features
 * it carries: the trailer's digest is checked before a byte of the payload is
 * handed to Lua, so it cannot be optional. */
extern bool nuppSha256(const uint8_t *bytes, size_t length, char *output);

void nupp_host_digest_prefix(const uint8_t *bytes, size_t length, uint8_t out[8]) {
    char hex[65];
    size_t at;
    memset(out, 0, 8);
    if (!nuppSha256(bytes, length, hex)) {
        return;
    }
    for (at = 0; at < 8; at++) {
        unsigned value = 0;
        ...
        for (step = 0; step < 2; step++) {
            char letter = hex[at * 2 + step];
            unsigned digit = letter >= 'a' ? (unsigned)(letter - 'a' + 10)
                : (unsigned)(letter - '0');
