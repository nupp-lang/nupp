...
#if NUPP_FEATURE_JSON
extern int luaopen_jsonNative(lua_State *state);
#endif
...
static void set_flag(lua_State *state, const char *name) {
...
#endif
#if NUPP_FEATURE_LUA_UTF8
    set_flag(state, "lua-utf8");
#endif
#if NUPP_FEATURE_NATIVE_FILES
    set_flag(state, "native-files");
#endif
#if NUPP_FEATURE_NATIVE_PROCESS
    set_flag(state, "native-process");
#endif
#if NUPP_FEATURE_WORKERS
    set_flag(state, "workers");
#endif
...
#if NUPP_FEATURE_JSON
    preload_opener(state, "jsonNative", luaopen_jsonNative);
#endif
#if NUPP_FEATURE_LPEG
