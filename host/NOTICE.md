...
The stub links three MIT-licensed projects into itself, so a binary stamped
...
files as they arrive in the pinned sources, byte for byte.
...
file against the notice in the archive it just verified, and fails the build
when they differ — so bumping a pin without updating the notice beside it stops
the build rather than shipping the wrong text.

`cjson` and `luautf8` are behind Cargo features. A host built without one does
