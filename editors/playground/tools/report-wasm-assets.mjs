...
process.stdout.write(JSON.stringify({
  ...
  appRuntime: statSync(path.join(dist, manifest.appRuntime)).size,
  appHostJavaScript: statSync(path.join(dist, manifest.appHostModule)).size,
  appHostWasm: statSync(path.join(dist, manifest.appHostWasm)).size,
}, null, 2) + "\n");
