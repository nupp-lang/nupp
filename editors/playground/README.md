...
uses browser providers for timers, Web Crypto, IndexedDB, HTTP and WebGPU.
...
2 MiB effect-byte budget, a 4 MiB response budget and 512 KiB storage values.
...
must be standalone because the browser has no project filesystem. Inline C declarations and guest-native layouts are supported; local header
files and host preprocessing are not available. Examples should end with a
small edit that demonstrates a diagnostic.
