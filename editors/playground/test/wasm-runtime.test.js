...
test("runs generated Lua in the isolated application Wasm host", async () => {
  const createHost = (await import(
    pathToFileURL(path.join(dist, manifest.appHostModule)).href
  )).default;
  const compiler = await createCompilerHost({
    moduleUrl,
    wasmUrl,
    compilerUrl,
    expectedDigest: manifest.compilerSha256,
  ...
  const compiled = compiler.request({
    kind: "compile",
  ...
  const result = await runNuppWasmApp({
    createHost,
    locateFile: (name) => path.join(dist, name),
    app: new TextEncoder().encode(compiled.code),
    initialize: readFileSync(path.join(dist, manifest.appRuntime)),
    managed: true,
    sideModules: [],
    wasmBinary: readFileSync(path.join(dist, manifest.appHostWasm)),
  });
  assert.deepEqual(result, {stdout: "hello\t42"});
});
