...
static void digest_hex(const uint8_t *data, uint32_t length, char out[65]) {
    static const char digits[] = "0123456789abcdef";
    uint8_t digest[SIZE_OF_SHA_256_HASH];
    uint32_t index;

    calc_sha_256(digest, data, length);
    for (index = 0; index < SIZE_OF_SHA_256_HASH; ++index) {
...
int32_t nupp_boot(const uint8_t *bundle, uint32_t length) {
    ...
    digest_hex(bundle, length, actual);
    if (strcmp(actual, NUPP_BUNDLE_SHA256) != 0) {
        snprintf(last_error, sizeof(last_error),
            "compiler bundle SHA-256 mismatch: expected %s, got %s",
            NUPP_BUNDLE_SHA256, actual);
        return -3;
    }
