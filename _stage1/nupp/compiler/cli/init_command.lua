...
local function defaultDirectory ( source )
local fs = require ( "nupp.compiler.fs" )
if source . kind == "builtin" then
return source . name
end
if source . kind == "directory" then
return fs . basename ( source . path )
end
if source . subdir then
return fs . basename ( source . subdir )
end

return ( fs . basename ( source . url ) : gsub ( "%.git$" , "" ) )
end
...
local function confirm ( description , count , destination )
io . write (
...
local function reportSteps ( plan , destination , dryRun )
local template = require ( "nupp.compiler.template" )
local ran = { }
if dryRun then
return ran
...
local function scaffold ( source , destination , values , set )
local template = require ( "nupp.compiler.template" )
local fs = require ( "nupp.compiler.fs" )
...
local limit = values . from and 1 or 2
...
if not set then
return command : usageError ( setErr )
end
local template = require ( "nupp.compiler.template" )
...
do local __nuppT13=0; local  __nuppT19 ; local __nuppT20=false ; const __nuppT14,__nuppT15,__nuppT16=__nuppT6(function() do const __nuppT21=__nuppT1( nupp . io . files . createTemporaryDirectory (
...
nupp . io . files . remove ( owned : toString ( ) , true )
