...
and `nupp.data`, exposed through the `Buffer`, `ByteView`,
...
Linux and Windows. The native-process workflow exercises only the Rust process
provider there; the compiler and files path still have only macOS coverage.
