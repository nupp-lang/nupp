...
needed for line attribution: `nupp.gen` already emits code on the source line
...
environment and calling the public `nupp.coverage.flush()` before it exits.
...
Teach `nupp.gen` to construct a per-module manifest only in coverage mode. Each
...
Add `nupp.coverage` as a small runtime module. It receives the run directory,
...
Reuse `nupp.doc.highlight` rather than introducing a browser-only highlighter:
...
1. Add coverage to compile/build settings, cache identity, and a separate
   output namespace.
2. Refactor only enough of `nupp.gen` to make a normal emitter and optional
...
1. Implement `nupp.coverage` counter storage, manifest registration, flush,
...
1. Build a coverage page/layout module, sharing only the established
   highlighter and token styles from `nupp.doc`.
