...
| --- | --- | --- |
| open / close | `Owned<T, close>` result | the external producer is fresh and `close` is correct |
...
| clone / release | explicit `Owned<T, release>` clone producer | clone really creates an independent reference |
