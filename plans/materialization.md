...
annotation, and literal runtime actions derive `R` from the named callback return
types. Genuinely dynamic grammar strings retain `Peg<any>`.

The materializer lowering contract now has a provider-owned inferred-target slot, so
this is not PEG-only machinery. A later code generator whose blueprint fully
determines its public type can opt into unannotated boundaries; providers that need
application knowledge continue to return NUPP2414 until a type is declared.

The later anchored-recognition comparison now also clears every named workload.
On the recorded LuaJIT run, runtime-compiled PEG reached a geometric mean of
0.99x against the static matcher over short identifiers, long identifiers, fixed
dates and HTTP routes; compiling the three dynamic grammars cost about 1.1 ms.

The remaining work is deliberately outside the closed materialization core:
add fine-grained cross-module invalidation to the now structural,
annotation-aware C2a reflection graph, replace the synchronous LSP wait with
...
returns a precise `Peg<R>` when it is literal and `Peg<any>` otherwise.
Match-time captures, locale-dependent classes and
