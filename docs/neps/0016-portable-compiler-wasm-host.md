...
implementation and repaired host differences with browser shims. That mixed
three questions: whether the compiler source was valid Lua 5.1, which compiler
subsystems the playground needed, and how the browser hosted Lua.

Official Lua 5.1.5 rejects that artifact at its first `goto`. Fixing the host
before the artifact would preserve an unmeasured compiler closure and make
...
LuaJIT remains the fast compiler host. Selecting a portable compiler must not
...
Each session creates one checker environment per output dialect and retains
the last successful tree for hover. A JSON `request` method adapts this
...
first arena is constructed. Both providers are checked source and pass the
same compiler-owned suite.

This contract does not enter `nupp.runtime.seam.registry`. That registry says
which representations and host facilities generated programs may require;
compiler storage selection is an implementation detail.

The portable entry makes the same one-time choice for scalar bit operations
and the pinned lunajson adapter. It does not install fake `ffi`, `jit`, or LPeg
modules.

### Target source selection

A module target's `sources` selects its initial checked and packaged source
set. Static dependencies extend that set, while entries remain unconditional.
A computed `require` succeeds only if another selection included its eventual
module.
...
matches that host. A Lua 5.1 compiler producing LuaJIT source cannot use its
own parser to reject LuaJIT `const` declarations or cdata literals. The
...
or a virtual filesystem. A content-hashed compiler filename lets browsers
retain the proven source when only the host changes and retain the host when
only the compiler changes.
...
- Lua 5.1 parsing a multi-megabyte source bundle may dominate startup. The
  measured browser gate requires Wasm boot plus the first check to stay within
  twenty percent of the existing playground.
- Static closure discovery cannot see computed module names. The exercised
  browser paths therefore carry a runtime require inventory as evidence.
- Emscripten's ES-module Worker output and buffer ABI may change. The host pins
  one proven Emscripten version rather than accepting ambient tool behavior.
- A digest proves byte identity, not publisher identity. The Pages artifact
...
Those providers have a smaller owner and a different reason to change.

**Maintain Lua fallbacks for every runtime facility.** Hashing, UUIDs, UTF-8,
PEGs, and similar facilities already have third-party Lua implementations. A
checked adapter and the seam's behavioral suite establish the required
contract without making the compiler maintain another implementation.
