...
consumer of the verified scalar IR and `nupp.gpu` a runtime service with a device
...
- Carry both device implementations on one contract, so generated bindings name neither.
...
### Device implementations share a binding contract

Native and browser devices need one typed provider interface. The uniform block
must have a shared byte layout, so both implementations consume the same words
and conformance vectors. Generated kernel bindings can then retain the chosen
context and invoke its methods directly.
...
[NEP 23](0023-structured-workgroup-phases.md) where it cooperates. Nothing in
...
scratch is `var<workgroup>`, which is the same structural mapping NEP 23 gives
the other backends.
...
The generated binding writes the block through the context interface. At most 128 bytes of `u32` and `f32` makes the writer a
fixed sequence of 32-bit stores into a provider-owned byte region, emitted from the
...
a numeric heap address, which is the same rule the Wasm storage provider already
holds.
...
- **Uniform writes must match the shader layout.** Conformance vectors compare
  the written bytes against the layout declared by each shader.
...
likely to break silently, which is the argument for one contract.
