...
The same scope owns [worker tasks](0016-structured-worker-tasks.md). Cancelling
...
- Defining the clock. [NEP 19](0019-portable-time.md) owns monotonic time, the
...
[`time.now()`](0019-portable-time.md). If the parent has no deadline, the
...
[NEP 19](0019-portable-time.md) applies to `sleep`.
