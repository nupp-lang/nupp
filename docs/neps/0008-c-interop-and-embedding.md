...
visible symbol binds directly through the FFI, and a header-only callable
...
Much of a modern C API is `static inline` and function macros, which have no
...
error boundary, so an engine could reuse the ideas and could not link one
supported SDK.
...
A declaration with an externally visible symbol binds directly, with no
wrapper and no copy:
...
physical calling convention may come from C, while what a call borrows, takes,
retains, or releases, along with cleanup identity, counted relationships, and
effects, stay explicit Nupp contracts. This is the most important one: a header
does not contain those facts, so deriving them is invention, and an invented
ownership contract at an FFI boundary is worse than none.
...
crossing the boundary, and a generated macro wrapper receives those values as C
parameters, so a macro may mention a parameter repeatedly and cannot re-evaluate
...
close the routes around a guarantee before adding the guarantee.
