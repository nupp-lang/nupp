...
struct values, and a runtime-bound seam lazily requires a named third-party
module for JSON, SHA-256, UTF-8, UUID, bitsets or PEG and checks its runtime
shape. Both the adapter and the suite are ordinary checked source files tested
in isolation, and generated output contains only the selected backend
installation rather than fallback source stored in a compiler string. Nupp
specifies the contracts and may ship thin adapters for common modules, but it
...
are most of the Lua that exists. An author who wants both audiences today
writes the library twice.
...
The missing piece is not a lowering, since most of the typed layer erases
already. It is anything that decides which Lua a build is for, checks the
...
existing machinery, lacking only a runtime seam, a contract its adapter has to
satisfy, and an artifact requirement naming the runtime module a consumer must
install. The stage-zero JSON copy is not the model: it remains a bootstrap
implementation rather than the first of many algorithms Nupp maintains twice.
...
build output named in `nupp.lua`, and a `platform` is a binary platform selected
by `--platform`. This is a third axis and takes a third word, matching the sense
the README already uses when it calls Nupp a superset of LuaJIT's Lua dialect.
...
lowering, and for a runtime-bound seam it installs a lazy boundary that requires
the selected third-party module and validates its values. `test` invokes the
compiler-owned behavioral suite against the same implementation, and is not a
...
and owns the lazy validator and adapter for that module's values; and both
attach the seam name, contract version, installer and compiler-owned conformance
suite. `Backend.new` rejects an empty collection and duplicate seam names, which
lets one backend bundle several facilities without weakening any contract.

A backend descriptor must also be readable without running it, so the exported
value is restricted to `Backend.new` with a constant name and a literal list of
compiler-owned seam constructor calls, each runtime-bound call containing its
exact module string. Compile-bound implementation code remains in ordinary
checked modules; only the descriptor is declarative. Imports naming `Backend`
and the seam factories are immutable `const` bindings to exact `require`
strings, so reassignment cannot make the runtime value disagree with the static
descriptor, and the checker writes the backend and seam identities into the
module interface so `check`, `build` and task inspection resolve a backend
without executing its top level or a third-party dependency.
...
metadata owns the runtime dependency; it may wrap BitOp or another library
rather than reimplementing the operations. The checked call to `Bitops.seam`
records the exact runtime module, and the seam lazily checks its shape and
attaches the bitops suite. Under `luajit` the same seam resolves to native
operators and this module is never loaded, bound, or named in the output.
...
where it was an offset. An absent capability costs a diagnostic, and one
satisfied by a backend costs whatever that backend costs.

### SIMD has a scalar meaning

Automatic lane lowering of an ordinary scalar `@aot` loop is an optimization,
not a capability: with no vector backend the original scalar loop is already its
complete lowering, and the program does not fail merely because a target cannot
vectorize it.
...
This revisits one decision in [NEP 11](0011-simd.md), which correctly rejected
public boxed vectors and a second source spelling for map loops. Both rejections
stand — emulated values remain private, non-escaping backend values, and scalar
map source remains scalar map source — and only the conclusion that an
explicit-SIMD function cannot execute when native AOT is disabled is removed.
Under `luajit` with native AOT selected, the existing intrinsic lowering remains
direct and no emulation module is loaded.
...
default with the same seam name, and two project backends supplying the same
seam are an error. That keeps resolution explicit while letting one package
bundle bitops, wide integers and several library adapters in one `Backend`
value, and the checked module interface records the backend name, seam names and
...
today, and portability never inserts a common wrapper in front of it.
...
buffers and PEG are library facilities, and a portable artifact may obtain one
from a selected runtime module rather than an implementation maintained in
Nupp.
...
requires the one recorded module when the reached member is initialized, checks
the runtime values that the contract can check, and raises an error naming both
the standard facility and the missing or incompatible module. A build does not
execute the backend, so an absent runtime rock is a runtime dependency error
rather than a compiler execution side effect.
...
dialect and writes the compiler-owned runtime support it reaches as ordinary Lua
modules beside the artifact, depending on nothing in the compiler checkout's
runtime path. The backend records the adapter module name rather than running
LuaRocks or probing installed alternatives during checking.
...
allowed to be unavailable on a host, and not allowed to replace secure UUID
entropy with `math.random` or otherwise weaken the documented contract.
...
or the build fails. Nothing is discovered by scanning a directory, nothing is
ambient, and no resolution order decides between two candidates, because there
...
backend entry carries the module name and digest of the checked `Backend` value,
and each reached seam entry carries its name, contract version, whether it binds
at compile time or runtime, and, where a selected target dependency installs the
exact runtime module, that dependency's package and pinned version. Changing a
checked backend cannot leave an artifact claiming it resolved the old one. Code
behind an exact third-party runtime module name is deliberately not hashed: it
may be supplied by a rock or a host after the artifact was built, while the
package lock accounts for the selected artifact.
...
`NUPP3012` is a build-time selection error saying which seam contract to supply.
Where a runtime seam named a third-party module that is absent or incompatible
on the machine running the artifact, the lazy adapter raises a runtime
dependency error naming that module. The two are not collapsed, because the
compiler can prove the first and cannot inspect the second without executing the
target's dependency environment.
...
stored and what it costs to touch, and does not change what a program observes.

Some things cannot be preserved without a representation, such as a `uint64`
field in a runtime whose numbers are doubles where no `int64` backend was named.
The build reports `NUPP3007` there rather than lowering a slow answer or a wrong
one, and naming a backend permits the slow answer only because that backend
supplies a representation and operations for the complete `int64` contract.
...
call has the checked type. It does not claim that an arbitrary implementation of
`band` computes the right bits, because a function body can lie behind any
...
isolated worker, or one seam selected by name. `--runtime lua5.1` writes the
checked closure as real Lua files and launches that interpreter, so a portable
suite is not accidentally tested inside the LuaJIT compiler process. The command
reports the seam name and contract version, backend source digest, and runtime
that ran it, and exits unsuccessfully on the first mismatch. Native seams run
these same suites, making them the reference implementation without a private
test path. It is an explicit command for backend authors, package CI, and
consumers auditing a dependency: `check` and `build` never run it implicitly,
never execute a module only because `nupp.lua` named it, and never turn a cached
result into a proof, and the artifact records identities and digests rather than
...
That is one line and depends on nothing, and it is not a claim that every global
in Lua 5.1 exists in later versions. The dialect holds resolved prelude
identities to a compiler-owned intersection table: core operations with the same
contract remain available, a name used only by generated code may use the
prologue, and a source use of `setfenv`, `getfenv`, `package.loaders`, a LuaJIT
module, or any other runtime-specific identity reports `NUPP3010` at that use. A
local or module field with the same spelling is not a prelude identity and is
left alone.
...
lowering, since availability on only one runtime does not widen the
intersection. That is what makes `check --dialect lua51` a portability check
rather than only a parser check, and what lets the same published artifact reach
a 5.4 project or a browser interpreter once the artifact's recorded runtime
...
- **Nothing dogfoods `lua51`.** The compiler needs `cinterop` and will never
  build under it, so the CI runtime matrix specified above is the only thing
  keeping the common surface true, and without it the surface rots quietly.
- **The standard library is FFI-shaped.** Seventeen modules under `src/nupp/`
  reach the FFI, including all of `io` and `mem`, and much of `data`. The seam
  accounting above prevents those modules from becoming an unnamed portability
  hole, but it moves availability into package management, so a portable
  artifact can build successfully and then fail at first use when its declared
  runtime module was not installed on the target host.
- **Third-party APIs do not share one shape.** A checked runtime seam or thin
  adapter has to normalize each chosen module to Nupp's contract, so Nupp avoids
  maintaining SHA-256, UTF-8, JSON and PEG algorithms while somebody still owns
  each adapter and its compatibility range.
- **Runtime validation is necessarily shallow.** An adapter can check that a
  module loads and exports values of the promised shapes, not that a SHA
  implementation hashes correctly or a UTF-8 implementation agrees on every
  invalid sequence; the explicit conformance suite remains the evidence for
  those behaviors.
...
- **Portable SIMD can allocate.** A table-backed vector preserves the confined
  vocabulary but may allocate per operation, a cost belonging only to an
  artifact that selected the emulation backend; native AOT retains direct vector
  lowering.
...
complete to run against. A `Backend` may contain a subset of the known seams so
...
exactly that, one layer further away from the reader.
...
The portable contract needs a selected runtime seam and a useful
missing-dependency error, not every algorithm living in this repository.
...
its output is what it is now, so a project that never passes `--dialect` never
loads a portable backend or standard-library adapter. A byte-for-byte generated
output corpus enforces that claim rather than leaving it a stated intention.
...
contract while the output language stays Lua, where AOT retargeting keeps only
an admitted kernel after an IR and verifier have separated it from the rest of
the program. [NEP 14](0014-lua-in-wasm-aot.md) combines the two, with capability
backends supplying the portable Lua host and a cross-compiled AOT artifact
supplying selected numeric kernels.
