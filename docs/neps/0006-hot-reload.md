...
another. Once that has happened, the reported behavior of the program is not
the behavior of any version of its source, and debugging it is worse than
restarting.
...
only the program knows which it is in. A reload system that picks the moment
itself is guessing.
...
reference, captured value, and instance metatable becomes stale. Identity
preservation is the requirement that rules this out.
