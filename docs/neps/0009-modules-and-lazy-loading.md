...
to an untyped result. That is a correctness hole that grows with the size of the
standard library, and it cannot be closed one module at a time.
...
This is the constraint that eliminated most of the design space: any option
...
hidden direct import per containing module. There is no runtime namespace tree
to walk and nothing to allocate.
...
over a foundation that was still moving would have been designed against the
wrong thing.
...
- **Selection-time laziness is coarse.** A module reached by any live checked
  code initializes with its container, even on a path that never runs. That is
