...
ambiguous, or unsupported type loses only the fact it could not establish, falls
back to `any`, and reports a warning rather than making valid Lua fail to build.
...
and generic information in comments, and Nupp already parses the Lua source,
infers what it can, and publishes the returned value as the type of `require`.
The holes left at that boundary are often exactly the facts sitting immediately
above the declaration.

Ignoring them turns a large typed ecosystem back into `any` at the moment it
crosses into Nupp, and requiring a hand-written `.d.nupp` first preserves types
only after somebody duplicates the upstream API and keeps the copy current.

### Opt-in defeats an ecosystem boundary

A manifest switch makes the consumer discover, choose, and maintain a setting
for information already present in the source, and a pragma asks upstream
authors to change a file before Nupp can use the annotations they already wrote.
Both split projects into ones whose comments mean something and ones whose
identical comments do not.

The `.lua` extension already says that the runtime language is Lua and the
checking floor is gradual. Reading a comment as a foreign type contract adds no
Nupp syntax and makes nothing strict; it improves the facts available to the
same gradual checker, so it is always on.
...
Most useful forms are either shared or distinguishable from their syntax, so the
reader recognizes one compatibility grammar and uses dialect evidence only where
a spelling has more than one meaning. An explicit dialect remains a migration
aid for the genuinely ambiguous residue, not a condition of normal checking.
...
Recovery is therefore local: the failed type position becomes `any`, the rest of
the declaration and module continue to check, and a warning points at the
...
the incremental checker, documentation, and editor overlays. No configuration
key or source directive disables or enables this pass.
...
unknown documentary tags remain comments, and a known type-bearing tag is parsed
...
The declared parameter and result types seed the existing function checker, and
...
Equivalent spellings normalize into one contract representation: `T[]` and the
corresponding generic array form become a Nupp array, `fun` types become Nupp
function types, a nullable union becomes an optional where possible, and an
annotation enum becomes a literal union while its Lua table remains untouched.

Tags whose foreign meaning has no Nupp contract do not invent one. Foreign
diagnostic suppressions do not suppress Nupp diagnostics, and `async`,
`nodiscard`, version, source, and navigation metadata do not imply Nupp effects
or ownership; they remain available to documentation and editor presentation.
...
shared forms need no label, and this permits old EmmyLua definitions, current
...
An annotation block may provide stronger evidence for the blocks associated with
the same declaration, but it does not silently set a file-wide mode, which keeps
one stray legacy block from changing the meaning of everything below it.

### Recovery and warnings

Annotation diagnostics are non-fatal warnings, reported at the smallest comment
range that failed and replayed from the incremental cache like other
diagnostics. Strict checking does not promote them to errors.
...
gradual, and which supported spelling would establish it, which makes fallback
...
export of that module, reached through the required module rather than through a
new ambient namespace:
...
foreign annotations and does not leak names into Nupp source, and conflicts in
it recover under the warning rules above.
...
entry to accept a call; a foreign class does not create nominal runtime
identity; and a comment cannot establish an ownership transfer or complete
effect summary unless a future shared spelling states those exact semantics.
...
representation, or other facts in the explicit declaration, and a disagreement
between an inline public fact and the declaration is a warning attached to the
inline annotation.

This is not an opt-out: the Lua annotations were consumed and compared, and the
...
migrator and becomes the same module at `.g.nupp`, and future source-family
...
only; it does not enable annotation ingestion, create a project setting, or
affect later `.lua` checks. `--check` prints diagnostics and the planned rename
without changing files, and `--json` returns the same plan, warnings, and edits
in structured form.

Without `--check`, migration writes the complete destination, parses and checks
it as `.g.nupp`, and only then removes the source, refusing to replace an
...
`.lua` document, sending the unsaved text and selected dialect, when one is
chosen, to the language server's migration planner and applying the returned
...
highlighting, while Nupp owns only this action and the warnings produced when
the Lua module participates in a Nupp project. Command-line and editor migration
cannot drift because neither contains a second translator.

### Lowering and caching

Annotation ingestion emits no Lua. A `.lua` file remains the runtime source and
its comments remain comments; only the compile-time module surface changes.

The normalized annotation contract contributes to the module interface
fingerprint, so changing prose alone may recheck the file but does not
invalidate consumers, while changing a parameter, result, field, alias, or other
public fact does. Cross-file annotation declarations are keyed dependencies, so
changing an ambient foreign type invalidates only the modules whose annotations
named it.
...
- **Comments become API.** Changing what looked like documentation can now
  invalidate consumers or reveal an implementation mismatch, which is the point
...
- **The compatibility index broadens project discovery.** Large dependency
  trees may contain many annotated definitions, so header-only scanning,
...
- **Dialect evolution is external.** New upstream syntax must recover with a
  warning until supported, and must never turn into a fatal parser error.
- **Always-on behavior removes an escape hatch.** An annotation a project does
  ...
  configuration, so that identical source has identical meaning in every Nupp
  project.
...
metatable semantics that a comment did not establish, where an erased foreign
