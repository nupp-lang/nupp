...
wants the canonical layout, while a large simulation array wants its columns
contiguous, and both are correct uses of one declaration.

Putting the choice on the declaration forces one answer on both uses and makes
the declaration's nominal identity hide two incompatible physical meanings, so a
value's memory layout would depend, invisibly, on where it came from.

### Syntax has to stay ordinary

Column storage is worth having only if the code that uses it reads like the code
that does not, since a separate access syntax would make switching layouts a
rewrite and defeat the purpose of being able to choose.
...
even though the value type is unchanged, so hot reload must restart or invoke an
application-owned migration rather than reinterpret live bytes. Saying so is
what keeps the change from being silently unsafe, since the value type is
identical and nothing else in the system would notice. See
...
That is not a crash but a read of reused memory returning a plausible value, and
chunking fixes reallocation while doing nothing about it.
...
because boxing an eight-byte value as cdata costs about 200, so leaving tokens
as tables saves nothing. The third way out is a lifetime system rather than a
stage, which would have to prove that every structure storing an element also
retains its pool, through locals, returns, closures, nested nodes, calls,
unions, and containers mixing pools.
...
physical layouts, so a value's memory layout would depend on its declaration
