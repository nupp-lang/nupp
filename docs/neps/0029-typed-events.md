...
- Cross-worker or cross-runtime transport. A bus is local to one Lua state;
  [NEP 16](0016-structured-worker-tasks.md) owns what crosses one.
...
parameters erase ([NEP 22](0022-const-monomorphization.md)), so every
...
the channels are members of a transport provider interface. What the task and
...
migration. The runtime is pull-based by [NEP 30](0030-filesystem-watching.md)'s
...
change to NEP 30's contract and is left to a proposal of its own, with this
...
([NEP 28](0028-fetched-stage-zero.md)) reaches every module under `src`,
