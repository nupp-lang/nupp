...
named, not by a project setting or a pragma, and because the extension is not
part of the module's name, tightening a file is a rename that changes nothing
requiring it.
...
invisible in a directory listing, in a review diff that does not reach the top
of the file, and in a code search, and it is copied by accident when a file is
...
Annotations, generics, interfaces, and affine policies erase, so the generated
Lua for the strict file above is the generated Lua for the gradual one:
...
**A pragma inside the file**, as Luau's `--!strict` does. It gets the
granularity right and was the closest competitor, but the marker is invisible
where files are listed, reviewed, and searched, and can be silently lost or
copied.
