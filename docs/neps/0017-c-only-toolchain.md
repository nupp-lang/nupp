...
and C++. Rust appears in exactly two places, both existing to be called through
a C ABI from LuaJIT's FFI, so the implementation language is an implementation
detail — one that currently costs every contributor a second toolchain, every CI
job a rustup step and a Cargo cache, and every embedder a Rust static library in
their link line.
...
Doing this as a rewrite would mean one flag day where nothing is testable, where
doing it behind the existing ABI makes the two implementations interchangeable
at every point, so each feature ports, runs its existing suite, and lands.
...
and `NUPP_CXX` respectively, so existing scripts and CI jobs keep building. They
are documented as compatibility aliases from this proposal onward and removed no
earlier than the release after the port lands.
...
native builds, so the second run is a cache hit and a worktree shares the first
run's work. The offline and archive overrides that exist for dependency fetching
apply here too: a machine with the archives already present builds without a
network.

The driver is the only way Nupp builds its own native prerequisites, and
`bin/nupp` calls it instead of reaching for `cargo` or requiring `luajit` on
`PATH`.

This is not new capability, since CI already builds pinned LuaJIT and LPeg. What
the driver adds is that the build is atomic, cached, offline-capable, and the
same on a contributor's machine as in CI.
...
`platform_windows.c` with a shared header, rather than expressing processes,
path semantics or readiness polling in portable C with `#ifdef` blocks threaded
through one file: the two implementations are two files and the shared code is
what genuinely does not vary.
...
the process, because a global singleton would make two embedded runtimes in one
process share cancellation and limits.
...
enqueuing into bounded buffers and handing back readiness tokens that the Lua
side turns into values. DNS is configured asynchronously, because a synchronous
resolver stalls a scheduler frame for however long the network takes to answer.

TLS is pinned libcurl over pinned mbedTLS, built by the bootstrap driver, which
keeps the one-compiler promise honest at the cost of owning a security-update
cadence and a CA-root policy, both written down as part of the pin. The
...
- **C is a worse language for this code than Rust.** The provider parses
  ...
  sanitiser builds in CI cover what review does not; if it is wrong it shows up
  as memory-safety bugs in exactly the features listed above.
- **Pinned libcurl and mbedTLS mean owning security updates.** A CVE in either
  is Nupp's to ship, mitigated by the pin being a single file and the update a
  digest change, not by the risk being small.
...
**Rewrite in one change.** Smaller total diff and one review, but nothing is
testable until all of it is, and the failure mode of a flag day is discovering
the process-polling semantics were subtly wrong after the Rust implementation is
already deleted.

**Remove C++ as well, by replacing simdjson.** It would make the promise "one C
compiler" rather than "one compiler pair", but simdjson is the reason JSON is
fast, and a hand-written replacement trades a real property for a cosmetic one.
...
**Vendor Rust output as prebuilt objects.** It would remove the Rust toolchain
requirement without a port, by checking in compiled artifacts, at the cost of
making the tree unbuildable from source on any platform nobody prebuilt for and
moving the toolchain requirement onto whoever cuts a release.
...
The cost is the C++ standard: Ada 4 needs C++20 for `std::endian` where simdjson
needed C++17, so the floor stated above moves with it. GCC 10 and Clang 10 are
the first releases that carry it, both from 2020, and the supported compiler
pairs are otherwise unchanged.
...
directory traversal — so the split made the risk visible without reducing it.

[libuv](https://libuv.org/) already owns that portability boundary for Node.js,
supplying the process and pipe lifecycle, filesystem calls, a bounded event
...
`Landing order` was a five-phase sequence and all five phases landed, ending
with the Cargo removal the summary describes. `Exit criteria` went the same way
as the gates in [NEP 16](0016-portable-compiler-wasm-host.md): the commands and
the coverage behind them are what continuous integration runs, not a list of
things still to do. Neither section is a pending plan, and reading them as one
inverts what this proposal records.

One thing worth separating, because the words collide: a Cargo dependency kind
remains available to a project that wants one, and it is unrelated to the two
Nupp crates this removed. The build guide owns that distinction.
