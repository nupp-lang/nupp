...
traverses a complete value inside one codec entry, rather than generating a
serializer for every type, format and protocol or crossing the Lua/native
boundary once per token.

`@derive(nupp.derive.Serde)` creates one schema and binding for either a record
or a struct, replacing format-owning derives as the architectural boundary while
format-specific convenience APIs may remain as compatibility wrappers. A schema
can also be constructed and frozen at run time, and its dynamic binding uses the
same codecs, which is the path for schema-driven clients such as a Smithy client
loaded from a model without generated types.
...
A JSON derive knows both what a type contains and how JSON represents it, and an
XML or CBOR derive would learn the first fact again. A protocol with a different
timestamp, union or member-name rule either needs another derive or adds
protocol conditionals to generated JSON code, so code size grows with the
product of types, formats and protocol variants even though the type topology
was constant.

Moving only the generated methods behind a common interface changes the spelling
of the call while each type still owns a format traversal.
...
announces a structure and calls a serializer for each member, and on a JVM the
JIT may inline and devirtualize those calls. In Nupp, a native writer call for
every key, scalar and container crosses the Lua/C boundary and may drain a
...
mechanism result rather than proof of a complete implementation, but they
establish which boundary one has to preserve: the whole traversal, not each
serde event, is the native operation.
...
consuming unknown input without constructing a value. A generic document remains
the answer when the caller asked for a document, and is not the intermediate
representation of typed decoding.

### Dynamic clients require a run-time schema

A generated client can compile field access into source, where a dynamic client
...
materializes it from reflection and a model adapter builds and freezes the same
graph directly, so format extensions and dynamic slot bindings apply to both
...
with the same three logical members, reached by Lua keys, byte offsets and
integer slots respectively, so putting any one access strategy in the schema
either excludes the other two or duplicates the schema.

Separating `Schema` from `Binding<T>` lets format specialization depend on the
logical graph and access specialization depend on the representation, which is
...
An annotation is input declared on a schema node, where an extension is derived
data computed from a schema or binding and cached for later use. They are not
the same facility: annotations survive model conversion, and extensions may
contain native pointers, interned strings and other process-local acceleration
data.
...
typed annotations, where the dense index is an acceleration value rather than a
wire identifier or persistent model identity.

Representation-independent constraints such as length, numeric range and
enumeration membership may be precomputed on the schema. A codec validates wire
syntax and scalar conversion; the schema-guided builder validates model
...
explicitly admits a struct, and does not let an arbitrary record-only provider
...
declaration, normally held in a module-level `const`, an operation descriptor or
a prepared codec, so a hot member traversal does not look the binding up
repeatedly.
...
Both spellings produce the same schema and binding blueprint, so the derive is
convenience and discoverability rather than a second implementation.
...
A dynamic structure contains its schema identity, a presence bit set and a dense
value array. Member names are resolved while binding input or when an
application explicitly asks for a member by name; codecs operate on indices.

The logical schema contains none of these facts, so two physical types may bind
the same schema and one dynamic binding may represent a schema that has no Nupp
declaration.

### Struct rules

Serde reads declared struct fields and never copies or hashes the raw memory
image: a physical load is converted to the schema's semantic scalar and handed
to the codec, and decoding performs the inverse checked conversion before
storing at the derived offset.
...
A programmer may omit such a field or provide a typed adapter: a pointer and
count can be exposed as bytes only by an adapter that names the count,
establishes the root and returns a checked span. The derive never guesses that
relationship and never serializes an address.
...
storage, so reusing that value repeats neither name lookup nor structural
...
in dense key slots and compute an absent value once, so the provider runs only
on the cold preparation path.
...
An extension provider prepares a recursive schema as a graph, recording an empty
layout node in its own identity map before visiting that node's children and
fixing the child references after their layouts are available. It never
recursively asks the same host for the same extension, and accidental re-entry
keeps the reflection extension rule and reports recursive initialization.

Schemas and bindings are immutable after preparation begins, so no cache
invalidation exists. A dynamic model owns its schemas, schemas retain their
extension values, prepared layouts retain what they address, and releasing the
model releases the complete graph. Separate worker states build their own native
extensions rather than sharing process-local pointers.
...
Those byte fragments are trusted values constructed from frozen schema input, so
the encoder appends them without repeating UTF-8 validation or escaping. They
are schema constants rather than instance caches, and a caller's reusable
pre-encoded instance value remains a separate trusted-value API. The portable
writer path uses the existing opaque `json.EncodedString` and `json.EncodedValue`
forms; the fused codec may retain equivalent native byte spans inside its layout.

The decoder receives a key as bytes from the parser, compares the next member
expected from schema order, then uses the cached packed or hashed lookup,
returning a member index rather than a Lua key string. Escaped keys are
unescaped into parser scratch before the same lookup, and an unknown value is
consumed and validated without materialization when policy permits it.
...
Two profiles may derive different JSON layouts from the same schema, so profile
identity participates in the layout cache. The profile object, not the schema,
knows whether to prefer a format-specific annotation, an external model trait or
the logical member name.
...
The prepared plan is immutable and shareable, while the mutable encoder or
decoder session owns its output buffer, parser scratch, path stack,
required-member bits and other per-operation state. A completed session can be
reset and reused with the same prepared plan, so a client may pool sessions
without pooling schemas or bindings, and a session is never cached as an
extension or used concurrently. Its `close()` and `flush()` behavior, where
applicable, follows the separate I/O protocols rather than becoming a codec or
schema rule.
...
select the prepared traversal when the binding exposes one, so the callback path
is a semantic fallback rather than the implementation they must use.
...
the retained benchmark, each case measuring record, struct and dynamic bindings
...
validation or diagnostics, and unless the prepared path retains a material
advantage over per-event callbacks that justifies its native binding complexity.
Struct and dynamic results establish their own baselines rather than borrowing
the record result, and cold preparation is reported rather than hidden by
...
The protocol selected for the service supplies the JSON, XML or CBOR profile and
prepares each operation's payload schemas, where a generated client instead
places derived record bindings in the operation descriptor. Generated and
dynamic clients then share the protocol implementation and codecs, differing
only in their bindings.

Smithy traits remain typed Smithy metadata that a Smithy profile interprets when
producing a format extension. Neither `serde.Schema` nor a JSON codec acquires
service, operation or AWS-specific fields.
...
values and carries the matching layout fingerprint, without pasting source,
exposing an unchecked offset constructor or making target-specific data part of
the logical schema.

No `writeJSON` or `fromJSON` traversal is generated for each type, and temporary
...
- **The flat-structure speedup may shrink with complete semantics.** Recursive
  ...
  work, and the design depends on retaining a material advantage after those
  cases join the benchmark.
- **A generic native traversal is more code to trust.** It handles several
  ...
  could corrupt a struct, so binding construction must remain sealed and
  validate layout fingerprints.
- **Profiles can multiply cached layouts.** Interning makes equality cheap but
  does not bound a program deliberately constructing many distinct policies, and
  layouts follow schema lifetime, so dynamic clients must release models they no
  longer use.
- **First use performs real work.** Hash tables, encoded tags and recursive
  layouts are not free, so `prepare` gives clients an eager warm-up point and
...
- **Custom callback bindings may be much slower.** They preserve expressivity,
  not the performance promise of a prepared built-in binding, so documentation
  and profiling must make the selected path visible.
- **Struct pointers cannot be inferred safely.** Rejecting them makes some C
  layouts require adapters, and guessing their extent or ownership would be a
...
- **Portable dialects may lack a native codec.** The schema and callback
  ...
  runtime provider, which fits capability-backed standard-library selection
  without making all providers equally fast.
- **Extension providers must be deterministic.** Two computations for one
  immutable host and profile must produce equivalent layouts, so a provider may
  retain process-local acceleration data but may not observe mutable instance
  state.

## Alternatives considered

**Keep one derive per format.** Easy to explain and free to specialize, but it
repeats type topology, multiplies code by protocol variants, provides no natural
dynamic-client path and measured poorly when each generated operation crossed
the native writer boundary.

**Generate one format-neutral callback implementation per type.** This is the
portable fallback the proposal retains; making it the only path leaves the
dominant per-event crossings intact and exploits neither raw-key parsing nor
...
for declarations known to the compiler, where dynamic clients have no reflected
...
**Decode through a generic document.** Maximally flexible, and retained as the
explicit document kind, but using it for typed decoding allocates keys and
containers the final representation does not need, then repeats lookup and
validation.
...
where the measured data-driven traversal is already fast enough to require proof
before adding those costs.

**Put field access into the schema.** It makes the first record implementation
smaller, prevents a record, struct and dynamic value from sharing a schema, and
makes format caches accidentally depend on physical representation.

**Serialize struct bytes directly.** Fast only by changing the answer, since
...
ownership for native data, where extensions attached to immutable schema
identity have a natural lifetime and need no global coordination. Cross-schema
deduplication may be added inside an extension provider when measurement earns
it.

**Store format caches as string-keyed schema fields.** Open to naming
collisions, exposing mutable implementation detail and paying a general table
lookup, where typed extension keys keep ownership and result types explicit and
can use dense internal slots.
...
standard-library value produced from checked reflection or built explicitly, and
the compiler participates only where it already has exclusive knowledge — derive
...
and instances remain ordinary records, structs or dynamic values. Dynamic values
carry schema identity because their slots otherwise have no interpretation.

### Are schema extensions generated code?

No. They are immutable data — encoded byte fragments, lookup tables, member
indices and physical access descriptors — that a generic codec consumes.
