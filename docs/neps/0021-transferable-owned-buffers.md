...
arbitrary shapes, a [NEP 20](0020-shared-immutable-byte-regions.md) region
...
- Moving Lua-heap values. [NEP 16](0016-structured-worker-tasks.md)'s
...
checks consumption where it is written. What NEP 16 lacked was a value that
...
**Writable regions.** Extending NEP 20's region with a write surface would
...
strictly more dangerous, and rejected on the same grounds as in NEP 20:
data races become the programmer's problem. Exclusive ownership keeps the
race impossible rather than managed.

## FAQ

**Why is this not an extension of NEP 20?** One proposal records one
decision, and NEP 20's decision is accepted: immutable storage, shared by
