...
and which gives two languages to learn, two diagnostic surfaces, and a boundary
at which ordinary-looking code changes meaning.
...
which holds only if it is also the fast route; otherwise the hot loop is written
...
path existed to remove, while letting the body call the VM directly reintroduces
...
and the feature gets used on whatever someone thought of.
...
and no characterization path committed as a fallback, an alternate API, or an
...
benchmark spike and then reimplemented in the compiler. That keeps a staged
feature from accumulating two of everything, where the temporary half outlives
the reason for it.
...
stable storage, so ordinary metamethods keep ordinary dispatch.
...
function gets follows from what it consumes and produces.
...
- **The compatibility promise is hard to keep.** "May widen, does not narrow"
  ...
  for correctness.
- **No fallback means a backend defect breaks builds**, which is deliberate: a
