...
Lowering uses lexical selector/result locals and an `if`/`elseif` chain. It does
not wrap the switch in an immediately invoked function. Consequently, writing a
switch in a hot loop does not add function-construction bytecode that would
abort and blacklist LuaJIT traces. Type-case bindings also reuse the one
selector local, so a computed selector is never repeated for `is` or field
loads.
...
exhaustive (with or without `else`). String
cases, type cases, block arms, and early arm returns remain explicit AOT subset
boundaries. Regular Lua lowering supports the complete switch described above.
