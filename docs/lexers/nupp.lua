...
-- `comptime` opens an evaluation block, so it gets a preprocessor-style colour
-- instead of blending into ordinary control keywords. `@nosuspend` is covered by
-- the annotation rule above.
local compileTimeKeyword = lex:tag(lexer.PREPROCESSOR, lexer.word_match({"comptime"}))
...
local associatedType = lex:tag(lexer.KEYWORD, P("associated") * S(" \t") ^ 1 * P("type") * #(-(lexer.alnum + P("_"))))
...
local decimal = (
    decDigits * (P(".") * -P(".") * (lexer.digit + underscore) ^ 0) ^ -1 + P(".") * decDigits
) * exponent ^ -1
local hexadecimal = P(
    "0"
) * underscore ^ 0 * S(
    "xX"
) * underscore ^ 0 * hexDigits * (P(".") * -P(".") * (lexer.xdigit + underscore) ^ 0) ^ -1 * hexExponent ^ -1
