...
JSON path that reached it. Decoding uses Nupp's strict simdjson-backed codec and
...
text)` when a type-witness API fits better than generated members; the witness
and allocation model are documented in
