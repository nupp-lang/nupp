...
local type NonNil<T> = match each T when nil then never else T
end

local type Parameter<Path> = match Path when `${infer _}:${infer Name}` then Name else never
end
```
...
A tuple pattern can destructure both a fixed prefix and everything after it:

```nupp
local type Tail<Tuple> = match Tuple when {infer Head, unpackof infer Rest} then Rest else never
end
```

`Head` is the first slot and `Rest` is the remaining tuple. When there are no
remaining slots, `Rest` is `{never}`, Nupp's canonical empty sequence. The
`unpackof` tail must be last. It may contain another pattern, and every `infer`
inside it is scoped to that arm. This makes structurally decreasing tuple
reducers possible without converting a heterogeneous sequence to a homogeneous
array.
...
local type Segment<
    Part
    > = match Part when `:${infer Name}` then {readonly [K in Name]: string} else {readonly [K in never]: string}
end

local type RouteParameters<
    Path
    > = match Path when `${infer Head}/${infer Tail}` then Segment<Head> & RouteParameters<Tail> else Segment<Path>
end
