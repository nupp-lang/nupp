...
An imported header is rechecked when its containing Nupp module is invalidated,
but the standalone watcher does not yet observe a header-only edit. Until
external header paths join its watch set, touch the importing `.nupp` file or
restart the command after editing the header.
...
The standalone case edits a required module inside one `nupp run --watch`
process, commits the compatible body, rejects a type error, requires restart
for a structural edit, and proves that the last good implementation remains
callable throughout.
