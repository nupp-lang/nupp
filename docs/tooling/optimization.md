...
`nupp bc --check FILE` checks the exact generated bytecode without executing it.
Its control-flow graph distinguishes a blocker every back edge must reach from
one only some paths reach; JSON includes the bytecode fingerprint, trace profile,
