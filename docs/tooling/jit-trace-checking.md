...
| risk | Runtime types, call targets, ABI details, shapes, or recorder limits decide | warning/advice; selected conservative boundaries are errors under `@jit` |
...
| `@jit` | automatically on every check/build | the annotated function and statically resolved checked callees | none |
| `nupp bc --check FILE` | when requested | exact generated bytecode and loop control flow | none; it does not execute the chunk |
| VS Code **Nupp: Check Function for JIT Trace Blockers** | when requested | the method/function under the cursor and resolved callees, using unsaved text | none |
...
`FNEW` constructs a Lua function. The selected LuaJIT recorder refuses it. This
capturing closure therefore breaks an `@jit` contract:
...
A non-capturing function is more directly repairable, so the existing
`loop-invariant-closure` warning (`NUPP2505`) remains its automatic diagnostic:
...
resolved checked identity when continuous transport is required.

## Exact generated bytecode: `FNEW` and `UCLO`
...
| `jit/runtime-too-many-snapshots` | risk | `too many snapshots` | Guard/snapshot capacity was exhausted |
...
## What to do in a large application
