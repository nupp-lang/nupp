...
[comptime.md](../comptime.md) for the value-level half of the same
...
`elements`, `sameNominal`, `fields`, `readKeys`, `writeKeys`, `readAt`, and
`writeAt` expose checked semantic information without exposing mutable checker
state. Builders
such as `literal`, `optional`, `array`, `tuple`, `map`, `shape`, `union`,
`intersection`, `pointer`, `carray`, `constof`, `function_`, and `pack` produce
validated handles.

`fields` accepts structural shapes and nominal records, interfaces, and structs.
For a nominal it preserves the declaration as the handle's identity while returning
...
local comptime function ReadView(T: type): type
    ...
    for _, field in ipairs(nupp.types.fields(T)) do
        if field.read then
            selected[#selected + 1] = {name = field.name, read = field.read}
        end
    end
    return nupp.types.shape(selected)
end
```
...
local same: SameFamily(Cell<string>, Cell<number>) = true
local different: SameFamily(Cell<string>, Other<string>) = false
```
...
See
[strictness.md](../gradual-typing.md#constructs-that-arent-erased) for the
...
[affine-types.md](../../runtime/ownership/affine-types.md#cleanup-identity-is-part-of-the-type)
...
::: seealso
- [comptime.md](../comptime.md) for `comptime do` and the value-level
