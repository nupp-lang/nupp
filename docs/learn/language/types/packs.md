...
A generic iterator's computed result tail is expanded before its loop bindings
are checked. A library may therefore derive any finite number of yielded values
from a pack argument without publishing fixed-width overloads:
...
local comptime function Values(Cells: typepack): typepack
    ...
    for index = 1, #cells do
        values[index] = nupp.types.arguments(cells[index])[1]
    end
    return nupp.types.pack(values)
end

local record Iterator<Cells...>
    next: function(): (integer, unpackof Values(Cells...))
end

local iterator: Iterator<(Cell<string>, Cell<number>)> = nil as any
for index, text, number in iterator.next do
    print(index, text, number)
end
```

A computed result mode of `borrowed` makes its binding a shared borrow of the
iterator triple for one execution of the body. `exclusive` makes it an
exclusive borrow. Neither may escape the body; `continue`, `break`, `return`,
and errors all end the current iteration's loans. Both modes and the pack
expansion erase, so the generated Lua is the generic loop written in source.
...
[suspension.md](../../runtime/concurrency/suspension.md) for the effect side of the same
boundary.
...
success arm. See [ownership.md](../../runtime/ownership/borrowing.md) for the model the modes come
from.
...
- [ownership.md](../../runtime/ownership/borrowing.md) for the modes a slot carries
- [narrowing.md](narrowing.md#narrowing-tests) for the tests that pick a
  correlated arm
- [type-level-computation.md](comptime-types.md) for `typepack`
