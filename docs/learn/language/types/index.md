...
A string literal's type is the bytes it denotes rather than the source that
spells them, so two spellings of one string are one type:

```nupp
const escaped: "\65" = "A" -- one byte, written two ways
```
...
- [Records and structs](records-and-structs.md): nominal tables and fixed
  reifiable layouts.
- [Affine types](../../runtime/ownership/affine-types.md): compile-time type construction, exact
  cleanup identities, transfer-only values, and capability-preserving generics.
- [Ownership](../../runtime/ownership/borrowing.md): moves, borrows, aggregates, pinning, and lexical
  destruction.
...
- [Comptime types](comptime-types.md): member transforms, const
...
See [strictness.md](../gradual-typing.md) for the two exceptions.

::: seealso
- [strictness.md](../gradual-typing.md) for the gradual typing model and
  what adopting the strict floor costs
- [modules.md](../modules.md) for where a declaration lives and how
