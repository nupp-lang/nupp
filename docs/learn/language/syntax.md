...
use it. See [File extensions](gradual-typing.md#file-extensions) for what each
extension turns on.
...
A `struct` is the same declaration with a fixed reifiable layout. Native
LuaJIT targets represent it as FFI cdata.
...
See [records.md](types/records-and-structs.md) for the difference between the
first two, and [interfaces.md](types/interfaces.md) for what
...
still produce code, and [constructs that aren't erased](gradual-typing.md#constructs-that-arent-erased)
