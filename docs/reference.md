...
A direct `function M.name(...)` declaration on the table the file returns
publishes its annotated signature before the other function bodies in that block
are checked, so module functions may call one another in either source order. A
field assignment and a `local function` remain flow-sensitive. Runtime writes
still happen in source order.
