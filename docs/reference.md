...
The selected constructor also supplies its result policy, so one overload may
return `T` while another returns `affine(T, T.destroy)`.
...
A constructor with no result annotation returns the ordinary record type. An
annotation may state the policy introduced by successful construction:
`constructor(self, ...): affine(T, T.destroy)`. It must be exactly one value and
must erase to `T`, because the generated body returns the instance it allocated;
it cannot claim to have built another representation, a borrow, or a pin. Inline
method signatures are hoisted, so the terminal may be declared later in the same
record. Invalid constructor declarations are **NUPP2208**.
...
record m.File
    ...
    constructor(self, path: string): affine(m.File, m.File.destroy)
        self.path = path
    end
...
do
    local file = new m.File("notes.txt")
    print(file:read())
end -- Calls m.File.destroy once.
...
A single-acquisition extent with ordinary fallthrough lowers to a plain block
and direct terminal calls when the body and terminals are proven non-raising.
The compiler retains protected lowering whenever those proofs or the simple
control-flow shape are absent.
...
target's `aot` policy says what a build does with it: `off`, the default, does
nothing; `emit-c` writes the generated C beside the build; `require` compiles
that C into the project's own shared library and fails the build when it cannot.
Under `require` the function is replaced where it was written by a checked
wrapper that calls the compiled symbol, so every call to it reaches native code
and the struct layouts are compared against the object's own before the module
finishes loading. A closure, table, interpolated string, vararg, `goto`, dynamic
...
Reports: `NUPP2801`, `NUPP2802`, `NUPP2803`, `NUPP2806`, `NUPP2807`, `NUPP2808`,
`NUPP2809`, `NUPP2810`, `NUPP2811`, `NUPP2812`, `NUPP2813`. `nupp explain
<code>` says more.
