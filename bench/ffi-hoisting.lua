...
-- `ffi.new`. Measured against the gate in docs/guides/performance.md -- a gain with
...
-- (src/nupp/compiler/lua/gen.nupp), so nupp never writes the slow form in the first place.
--
...
-- `expect` is what docs/guides/performance.md records about each, and what this run
...
if #stale > 0 then
   ...
   io.write(" docs/guides/performance.md wants rereading before anything is built\n\n")
   os.exit(1)
end
