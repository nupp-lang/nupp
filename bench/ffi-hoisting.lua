...
-- `ffi.new`. Measured against the gate in plans/optimizations.md -- a gain with
...
-- (src/nupp/gen.nupp), so nupp never writes the slow form in the first place.
--
...
-- `expect` is what plans/optimizations.md records about each, and what this run
...
if #stale > 0 then
   ...
   io.write(" plans/optimizations.md wants rereading before anything is built\n\n")
   os.exit(1)
end
