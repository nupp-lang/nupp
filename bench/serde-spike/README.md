...
## Authored writer result

`results/arm64-macos-authored-writer.json` is the current 15-sample run after
prepared JSON encoding moved from the former native encoder to the authored
recursive writer. The historical pooled-writer comparison above describes the
removed implementation and is retained as provenance, not a current cost.

| Scenario | Returned string | Caller writer | Caller/direct |
| --- | ---: | ---: | ---: |
| Recursive records, lists, and map | 12,140 ns | 12,033 ns | 1.01x |
| 64 KiB string | 97,684 ns | 91,596 ns | 1.07x |

The recursive result rules out a distinct public-writer penalty: both routes
use the same traversal, and their medians differ by less than one percent. The
caller writer wins on the large scalar because it publishes staged parts
directly into the destination. Returning a string has to concatenate those
parts and the buffer overload then copies that complete string into its caller.

## Constant fusion ceiling

`results/arm64-macos-constant-fusion-ceiling.json` is the 15-sample closing
experiment for constant-byte write fusion. It compares the ordinary
twelve-field record with an otherwise identical record whose fields are all
literals or defaults.

| Scenario | Median | Relative throughput |
| --- | ---: | ---: |
| Ordinary record | 6,291 ns | 1.00x |
| Constant record | 6,364 ns | 0.995x (0.989-1.013x) |

The gate required the constant record to take at most half the ordinary
record's time. It showed no measurable advantage, so another encoding route
for constant spans is not justified.
