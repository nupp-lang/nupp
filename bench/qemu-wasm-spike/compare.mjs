...
const output = path.resolve(process.argv[3] || 'build/qemu-wasm-spike/comparison');
await mkdir(output, {recursive: true});
const samples = [];
for (let pair = 0; pair < 3; pair++) {
  for (const backend of pair % 2 ? ['portable', 'qemu'] : ['qemu', 'portable']) {
    ...
    if (pair === 0 && backend === 'qemu') env.SPIKE_SCREENSHOT = path.join(output, 'game.png');
    try {
...
const summary = {};
for (const backend of ['qemu', 'portable']) {
