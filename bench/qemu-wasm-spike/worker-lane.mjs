// One `nupp.workers` lane: a module Web Worker holding its own Linux/LuaJIT VM.
//
...
async function boot(message) {
  await startGuest({appUrl: message.manifestUrl, config: {worker: true, setup: message.setup || ''},
    deadlineMs: 180000, effectHandlers: {lane: performLaneEffect},
    resetLimits: beginsTask,
    onProgress: progress => self.postMessage({type: 'progress', progress})}).result;
}
