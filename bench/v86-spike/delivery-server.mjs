...
for (const name of files) {
  ...
  for (const variant of ['raw', 'compressed', 'slim']) {
    ...
    const br = brotliCompressSync(original, {params: {[constants.BROTLI_PARAM_QUALITY]: 9}});
...
http.createServer((req, res) => {
  ...
  if (url.pathname === '/_stats') {
    ...
    res.end(JSON.stringify({rateBytesPerSecond: rate, firstByteDelayMs: delay, requests,
  ...
  res.on('finish', () => requests.push({path: url.pathname, bytes: body.length, encoding: compressed ? 'br' : 'identity'}));
  setTimeout(() => {
