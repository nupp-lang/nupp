...
local function markedEmpty(value, marker)
    return type(value) == "table" and next(value) == nil and getmetatable(value) == marker
end

local function same(left, right, simdjsonNumbers)
    if markedEmpty(left, simdjsonBench.EMPTY_ARRAY) or markedEmpty(left, simdjsonBench.EMPTY_OBJECT) then
...
for _, source in ipairs(corpus) do
    ...
    check(
        indexStatus == scalarStatus and indexPosition == scalarPosition,
    ...
    if #expectedTape > 0 then
        ...
        check(
            capacityStatus == indexer.TAPE_CAPACITY and capacityPosition > 0,
...
do
    ...
    check(valueBuilder.scratchWord(scratch, 0) == 0xDEADBEEF, "ordinary word scratch lost a value")
    check(not pcall(valueBuilder.scratchWord, scratch, 2), "ordinary word scratch accepted an out-of-bounds read")
    ...
    check(valueBuilder.scratchByte(byteScratch, 1) == 66, "ordinary byte scratch lost a value")
    local byteStream = valueBuilder.new(json.null)
    valueBuilder.openArray(byteStream, 1)
    valueBuilder.stringScratch(byteStream, byteScratch, 0, 2)
    ...
    check(valueBuilder.scratchByte(byteScratch, 0) == 67, "ordinary byte scratch did not reset")
end
...
do
    ...
    check(
        built.alpha[1] == 1 and built.alpha[2] == json.null and built["escaped\n"] == true,
...
do
    ...
    check(pulled.id == 7 and pulled.profile.name == "Nupp", "pull did not materialize selected fields")
    check(pulled.profile.ignored == nil and pulled.ignored == nil, "pull materialized an unselected field")
    check(
        #pulled.items == 2 and pulled.items[1] == 1 and pulled.items[2] == 2,
        "pull did not drop and compact null array values"
    )
    check(pulled.nullable == nil, "pull did not drop null by default")
    check(markedEmpty(pulled.emptyArray, simdjsonBench.EMPTY_ARRAY), "pull lost the empty-array sentinel")
    check(markedEmpty(pulled.emptyObject, simdjsonBench.EMPTY_OBJECT), "pull lost the empty-object sentinel")

    local preserved = simdjsonBench.pull(source, shape, nullValue)
    check(
        preserved.nullable == nullValue and preserved.items[2] == nullValue,
    ...
    check(
        eager.nullable == nil and eager.items[1] == 1 and eager.items[2] == 2,
        "eager decode did not drop null by default"
    )
    local eagerPreserved = simdjsonBench.decode(source, nullValue)
    check(
        eagerPreserved.nullable == nullValue and eagerPreserved.items[2] == nullValue,
        "eager decode did not preserve the caller's null sentinel"
    )
    check(
        markedEmpty(simdjsonBench.decode("[null]"), simdjsonBench.EMPTY_ARRAY),
        "an array emptied by null dropping lost its sentinel"
    )
    check(
        markedEmpty(simdjsonBench.decode([[{"only":null}]]), simdjsonBench.EMPTY_OBJECT),
        "an object emptied by null dropping lost its sentinel"
    )
    check(
        not pcall(simdjsonBench.pull, [[{"ignored":1e309,"id":1}]], {
            id = true
        }),
        "pull did not validate an unselected overflowing number"
    )
    check(
        markedEmpty(simdjsonBench.pull("[1,2,3]", simdjsonBench.arrayOf(false)), simdjsonBench.EMPTY_ARRAY),
        "false array shape did not drop every member"
    )
    check(not pcall(simdjsonBench.pull, "1", "all"), "pull accepted a truthy non-shape value")
end

do
    local nullValue = {}
    check(simdjsonBench.encode(simdjsonBench.EMPTY_ARRAY) == "[]", "empty-array sentinel serialized incorrectly")
    check(simdjsonBench.serialize(simdjsonBench.EMPTY_OBJECT) == "{}", "empty-object sentinel serialized incorrectly")
    check(simdjsonBench.encode({}) == "{}", "empty Lua table did not serialize as an object")
    check(
        simdjsonBench.encode(simdjsonBench.asArray({})) == "[]",
    ...
    check(not pcall(simdjsonBench.encode, {[1] = true, key = false}), "mixed array and object serialized as JSON")
    ...
    check(decoded.array[2] == nullValue and decoded.array[3] == 2, "serialized null sentinel did not round trip")
    check(
        markedEmpty(
            decoded.emptyArray,
            simdjsonBench.EMPTY_ARRAY
    ...
    writer:null():write(simdjsonBench.EMPTY_OBJECT):endArray():key("empty"):write(simdjsonBench.EMPTY_ARRAY):endObject()
    writer:close()
    check(out:tostring() == [[{"items":[1,null,{}],"empty":[]}]], "streaming writer emitted the wrong chunks")
    ...
    trustedWriter:startObject():key(key):write(simdjsonBench.verified("1")):endObject()
    trustedWriter:close()
    check(out:tostring() == [[{"quoted\"key":1}]], "streaming writer emitted trusted bytes incorrectly")
    check(
        not pcall(function()
            writer:write(true)
        end),
    ...
    check(
        ...
        "streaming writer accepted an object value without a key"
    )
end
...
do
    ...
    for case = 1, 200 do
        ...
        check(same(arena.decode(source, json.null), expected), "generated arena differential mismatch at case " .. case)
        check(
            same(arena.decodeFused(source, json.null), expected),
            "generated fused differential mismatch at case " .. case
        )
        check(
            same(simdjsonBench.decode(source, json.null), expected, true),
            "generated simdjson Lua DOM mismatch at case " .. case
        )
        check(
            same(simdjsonBench.pull(source, true, json.null), expected, true),
            "generated simdjson pull mismatch at case " .. case
        )
        local truncated = source:sub(1, -2)
        check(not pcall(arena.decode, truncated, json.null), "truncated generated document accepted at case " .. case)
        check(
            not pcall(arena.decodeFused, truncated, json.null),
            "truncated generated document accepted by fused parser at case " .. case
        )
        check(
            not pcall(simdjsonBench.decode, truncated, json.null),
...
for _, source in ipairs(invalidUtf8) do
    ...
    check(status == scalarStatus and position == scalarPosition, "packed UTF-8 failure differs from scalar oracle")
end
...
for prefix = 0, 40 do
    ...
    check(
        fusedValue == padding .. string.char(0xf0, 0x9f, 0x99, 0x82) .. '"tail',
...
do
    check(simdjsonBench.version():match("^%d+%.%d+%.%d+$") ~= nil, "simdjson version is unavailable")
