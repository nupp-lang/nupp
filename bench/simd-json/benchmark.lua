...
local nuppJson = require("nupp.data.json")
...
local payloads = {
   {name = "records", source = records(), short = false,
      pullShape = simdjsonBench.arrayOf({id = true}),
      nuppPullShape = nuppJson.arrayOf({id = true})},
   {name = "ascii", source = strings("ordinary-ascii-value-", 5000), short = false,
      pullShape = simdjsonBench.arrayOf(false), nuppPullShape = nuppJson.arrayOf(false)},
   ...
   {name = "numbers", source = numbers(), short = false,
      pullShape = simdjsonBench.arrayOf(false), nuppPullShape = nuppJson.arrayOf(false)},
   {name = "unique-keys", source = uniqueKeys(), short = false,
      pullShape = {wanted = true}, nuppPullShape = {wanted = true}},
   {name = "short", source = [[{"ok":true,"n":42,"s":"hello"}]], short = true,
      pullShape = {ok = true, n = true}, nuppPullShape = {ok = true, n = true}},
}
...
for payloadIndex, payload in ipairs(payloads) do
   ...
   if not indexOnly then
      ...
      summary.nuppPullToSimdjsonPullThroughput =
         ratioSummary(raw.simdjsonPullSelected, raw.nuppPull)
...
if not indexOnly then
   ...
   for sample = 1, samples do
      ...
      for _, payload in ipairs(report.payloads) do
         if not payload.short then
            logSum = logSum + math.log(
               payload.seconds.simdjsonLua[sample] / payload.seconds.nupp[sample])
            count = count + 1
         end
      end
      pairedGeomeans[sample] = math.exp(logSum / count)
   ...
   for sample = 1, samples do
      ...
      for _, payload in ipairs(report.payloads) do
         if not payload.short then
            logSum = logSum + math.log(
               payload.seconds.nupp[sample] / payload.seconds.nuppPull[sample])
            count = count + 1
         end
      end
      pairedGeomeans[sample] = math.exp(logSum / count)
   ...
   if hasCjson then
      ...
      for sample = 1, samples do
         ...
         for _, payload in ipairs(report.payloads) do
            if not payload.short then
               logSum = logSum + math.log(
                  payload.seconds.cjson[sample] / payload.seconds.nupp[sample])
               count = count + 1
            end
         end
         pairedGeomeans[sample] = math.exp(logSum / count)
...
if not indexOnly then
   ...
   print(("large-payload Nupp pull/simdjson pull geometric mean %.3fx [%.3f, %.3f]"):format(
   ...
   print(("large-payload Nupp pull/decode geometric mean %.3fx [%.3f, %.3f]"):format(
   ...
   if report.largeNuppToCjsonThroughput then
      print(("large-payload Nupp/cjson geometric mean %.3fx [%.3f, %.3f]"):format(
...
for _, payload in ipairs(report.payloads) do
   ...
   if not indexOnly then
      ...
      print(("  nupp/simdjson Lua %.3fx [%.3f, %.3f]"):format(
      ...
      print(("  nupp pull/simdjson pull %.3fx [%.3f, %.3f]"):format(
      ...
      print(("  nupp pull/decode %.3fx [%.3f, %.3f]"):format(
