...
local nuppJson = require("nupp.codec.json")
...
local function records()
    ...
    for index = 1, 2000 do
        rows[
            ...
            '{"id":%d,"active":%s,"name":"record-%d","values":[%.1f,%d,%d]}'
        ):format(index, index % 3 ~= 0 and "true" or "false", index, index * 0.5, index * 2, index % 17)
    end
...
local function uniqueKeys()
    local fields = {}
    for index = 1, 4000 do
        fields[index] = ('"ignored-%d":"value-%d"'):format(index, index)
    end
    fields[#fields + 1] = '"wanted":42'
...
local payloads = {
    {
        ...
        source = records(),
        short = false,
        pullShape = simdjsonBench.arrayOf({
            id = true
        }),
        nuppPullShape = nuppJson.arrayOf({
            id = true
        })
    },
    {
        name = "ascii",
        source = strings("ordinary-ascii-value-", 5000),
        short = false,
        pullShape = simdjsonBench.arrayOf(false),
        nuppPullShape = nuppJson.arrayOf(false)
    },
    {
        name = "unicode",
        source = strings("κόσμος-日本語-🙂-", 3500),
        short = false,
        pullShape = simdjsonBench.arrayOf(false),
        nuppPullShape = nuppJson.arrayOf(false)
    },
    {
        name = "escaped",
        source = strings("quote-\"-slash-\\-line-\n-", 4000),
        short = false,
        pullShape = simdjsonBench.arrayOf(false),
        nuppPullShape = nuppJson.arrayOf(false)
    },
    ...
    },
    {
        ...
        source = uniqueKeys(),
        short = false,
        pullShape = {wanted = true},
        nuppPullShape = {wanted = true}
    },
    {
        name = "short",
        source = [[{"ok":true,"n":42,"s":"hello"}]],
        short = true,
        pullShape = {ok = true, n = true},
        nuppPullShape = {ok = true, n = true}
    },
}
...
if hasCjson and not indexOnly then
    names[#names + 1] = "cjson"
    report.environment.cjson = cjson._VERSION or "available"
end
...
local function bootstrapSummary(ratios)
    ...
    return {median = median(ratios), low95 = percentile(bootstrap, 0.025), high95 = percentile(bootstrap, 0.975),}
end
...
for payloadIndex, payload in ipairs(payloads) do
    ...
    local runners = {
        ...
        nuppPull = function()
            local started = os.clock()
            for _ = 1, iterations do
                nuppJson.pull(source, payload.nuppPullShape, json.null)
            end
    ...
    for sample = 1, samples do
        for offset = 0, #names - 1 do
            ...
            -- Each implementation allocates a complete result. Starting the next
            -- one with its predecessor's allocation debt made runner rotation
            -- rotate which implementation paid for collection as well as order.
            collectgarbage()
    ...
    if not indexOnly then
        ...
        summary.simdjsonPullSelectedMBps = #source * iterations / median(raw.simdjsonPullSelected) / 1000000
        summary.nuppPullMBps = #source * iterations / median(raw.nuppPull) / 1000000
        ...
        summary.nuppPullToSimdjsonPullThroughput = ratioSummary(raw.simdjsonPullSelected, raw.nuppPull)
        summary.nuppPullToDecodeThroughput = ratioSummary(raw.nupp, raw.nuppPull)
        if hasCjson then
...
if not indexOnly then
    ...
    for sample = 1, samples do
        ...
        for _, payload in ipairs(report.payloads) do
            if not payload.short then
                logSum = logSum + math.log(payload.seconds.legacy[sample] / payload.seconds.nupp[sample])
                count = count + 1
            end
        end
        pairedGeomeans[sample] = math.exp(logSum / count)
    ...
    for sample = 1, samples do
        ...
        for _, payload in ipairs(report.payloads) do
            if not payload.short then
                logSum = logSum + math.log(payload.seconds.arena[sample] / payload.seconds.nupp[sample])
                count = count + 1
            end
        end
        pairedGeomeans[sample] = math.exp(logSum / count)
    ...
    for sample = 1, samples do
        ...
        for _, payload in ipairs(report.payloads) do
            if not payload.short then
                logSum = logSum + math.log(payload.seconds.simdjsonLua[sample] / payload.seconds.nupp[sample])
                count = count + 1
            end
        end
        pairedGeomeans[sample] = math.exp(logSum / count)
    end
    report.largeNuppToSimdjsonLuaThroughput = bootstrapSummary(pairedGeomeans)
        ...
        for _, payload in ipairs(report.payloads) do
            if not payload.short then
                logSum = logSum + math.log(
                    payload.seconds.simdjsonPullSelected[sample] / payload.seconds.nuppPull[sample]
                )
                count = count + 1
            end
        end
    ...
    report.largeNuppPullToSimdjsonPullThroughput = bootstrapSummary(pairedGeomeans)
    ...
    for sample = 1, samples do
        ...
        for _, payload in ipairs(report.payloads) do
            if not payload.short then
                logSum = logSum + math.log(payload.seconds.nupp[sample] / payload.seconds.nuppPull[sample])
                count = count + 1
            end
        end
        pairedGeomeans[sample] = math.exp(logSum / count)
    end
    report.largeNuppPullToDecodeThroughput = bootstrapSummary(pairedGeomeans)
    ...
    if hasCjson then
        ...
        for sample = 1, samples do
            ...
            for _, payload in ipairs(report.payloads) do
                if not payload.short then
                    logSum = logSum + math.log(payload.seconds.cjson[sample] / payload.seconds.nupp[sample])
                    count = count + 1
                end
            end
            pairedGeomeans[sample] = math.exp(logSum / count)
        end
        report.largeNuppToCjsonThroughput = bootstrapSummary(pairedGeomeans)
    end
end
...
print(
    (
        "large-payload index/classify geometric mean %.3fx [%.3f, %.3f]"
    ):format(
...
if not indexOnly then
    print(
        (
            "large-payload fused/legacy geometric mean %.3fx [%.3f, %.3f]"
        ):format(
    ...
    print(
        (
            "large-payload fused/arena geometric mean %.3fx [%.3f, %.3f]"
        ):format(
    ...
    print(
        (
            "large-payload Nupp/simdjson Lua geometric mean %.3fx [%.3f, %.3f]"
        ):format(
            report.largeNuppToSimdjsonLuaThroughput.median,
            report.largeNuppToSimdjsonLuaThroughput.low95,
            report.largeNuppToSimdjsonLuaThroughput.high95
        )
    )
    print(
        (
            "large-payload Nupp pull/simdjson pull geometric mean %.3fx [%.3f, %.3f]"
        ):format(
            report.largeNuppPullToSimdjsonPullThroughput.median,
            report.largeNuppPullToSimdjsonPullThroughput.low95,
            report.largeNuppPullToSimdjsonPullThroughput.high95
        )
    )
    print(
        (
            "large-payload Nupp pull/decode geometric mean %.3fx [%.3f, %.3f]"
        ):format(
            report.largeNuppPullToDecodeThroughput.median,
            report.largeNuppPullToDecodeThroughput.low95,
            report.largeNuppPullToDecodeThroughput.high95
        )
    )
    if report.largeNuppToCjsonThroughput then
        print(
            (
                "large-payload Nupp/cjson geometric mean %.3fx [%.3f, %.3f]"
            ):format(
                report.largeNuppToCjsonThroughput.median,
                report.largeNuppToCjsonThroughput.low95,
                report.largeNuppToCjsonThroughput.high95
            )
        )
    end
end
...
print(
    (
        "platform: %s/%s, %s, samples: %d, warmups: %d"
    ):format(report.environment.os, report.environment.arch, report.environment.targetTier, samples, warmups)
)
if not indexOnly then
    print(
        (
            "simdjson: %s, implementation: %s"
        ):format(report.environment.simdjson, report.environment.simdjsonImplementation)
    )
end
for _, payload in ipairs(report.payloads) do
    print(
        (
            "payload: %-8s %7.3f MB  hash32:%s  iterations:%d"
        ):format(payload.name, payload.bytes / 1000000, payload.hash, payload.iterations)
    )
    for _, name in ipairs(names) do
        local elapsed = median(payload.seconds[name]) / payload.iterations
        print(("  %-15s %8.3f ms  %8.1f MB/s"):format(name, elapsed * 1000, payload.bytes / elapsed / 1000000))
    end
    if not indexOnly then
        print(
            (
                "  classifier share %.1f%% [%.1f, %.1f]"
            ):format(
        ...
        print(
            (
                "  nupp/simdjson Lua %.3fx [%.3f, %.3f]"
            ):format(
                payload.summary.nuppToSimdjsonLuaThroughput.median,
                payload.summary.nuppToSimdjsonLuaThroughput.low95,
                payload.summary.nuppToSimdjsonLuaThroughput.high95
            )
        )
        print(
            (
                "  nupp pull/simdjson pull %.3fx [%.3f, %.3f]"
            ):format(
                payload.summary.nuppPullToSimdjsonPullThroughput.median,
                payload.summary.nuppPullToSimdjsonPullThroughput.low95,
                payload.summary.nuppPullToSimdjsonPullThroughput.high95
            )
        )
        print(
            (
                "  nupp pull/decode %.3fx [%.3f, %.3f]"
            ):format(
                payload.summary.nuppPullToDecodeThroughput.median,
                payload.summary.nuppPullToDecodeThroughput.low95,
                payload.summary.nuppPullToDecodeThroughput.high95
            )
        )
        print(
            (
                "  fused/legacy throughput %.3fx [%.3f, %.3f]"
            ):format(
        ...
        print(
            (
                "  fused/arena throughput %.3fx [%.3f, %.3f]"
            ):format(
    ...
    print(
        (
            "  index/classify throughput %.3fx [%.3f, %.3f]"
        ):format(
