...
local cases = {
    {name = "nupp-untyped", run = production.untypedDecode},
...
for _, case in ipairs(cases) do
    local count = 128
    while elapsed(case, count) < target do
        count = count * 2
    end
    case.count = count
    case.values = {}
...
for _, case in ipairs(cases) do
    ...
    if case.name == "simdjson-binding" then
...
for _, case in ipairs(cases) do
    print(
        (
            "%-18s %8.1f ns/op %8.1f MB/s %6.3fx simdjson"
        ):format(case.name, case.seconds * 1e9, #source / case.seconds / 1e6, controlSeconds / case.seconds)
    )
end
