local _tl_compat; if (tonumber((_VERSION or ''):match('[%d.]*$')) or 0) < 5.3 then local p, m = pcall(require, 'compat53.module'); if p then _tl_compat = m end end; local assert = _tl_compat and _tl_compat.assert or assert; local load = _tl_compat and _tl_compat.load or load; local math = _tl_compat and _tl_compat.math or math; local pairs = _tl_compat and _tl_compat.pairs or pairs; local string = _tl_compat and _tl_compat.string or string; local table = _tl_compat and _tl_compat.table or table; local type = type; local table_clear = require("table.clear")
...
function events.newFFIEvent(event, fields, structName)
   if event.eventId then
      error("Event has already been configured: " .. tostring(event) .. " : " .. event.eventId)
   end
   ...
   if not init then
      ...
      parts[#parts + 1] = ")\n"
      for i = 1, n do
         parts[#parts + 1] = string.format("    if a%d ~= nil then instance.%s = a%d end\n", i, fieldNames[i], i)
      end
      parts[#parts + 1] = "end"
...
local MESSAGE_BUS_MT = { __index = MessageBus }
...
function MessageBus:observeOnce(address, eventType, observer)
   ...
   wrappedFunction = function(e) self:stopObserving(address, eventType, wrappedFunction); observer(e) end
