...
--
-- One payload hashed over and over is not what a caller does, and it is not what LuaJIT
-- measures either: everything an implementation derives from the argument is
-- loop-invariant then, and the recorder hoists it out of the loop. That hid the whole
-- of `nupp.digest.internal.sha256`'s caller-side padding -- about sixty nanoseconds of
-- a three-hundred-nanosecond call -- and reported the 32-byte row some twenty-five
-- percent faster than the same code hashing different bytes each time.
--
