...
return {
    ...
    dependencies = {
        -- The implementation being replaced, kept as the control it now is. Nothing
        -- else in the tree builds this: it left the native provider when
        -- `nupp.digest.internal.sha256` took over, and it lives here beside
        -- `legacy.lua` for the same reason, which is that a port is worth what it can
        -- be measured against.
        sha256_control = {
            kind = "c",
