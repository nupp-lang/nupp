-- The implementation `nupp.compiler.build.hash` carried before
-- `nupp.digest.internal.sha256` replaced it: SHA-256 over LuaJIT's `bit` library, with
-- the message padded by concatenation and the schedule in an ordinary Lua table.
--
-- Frozen here as a benchmark baseline and a differential oracle. It is not on
-- any path any more; do not fix it, and do not make it faster. Its whole value
-- is being exactly what was measured before.
