...
local dt = tonumber(os.getenv("COLUMNS_DT") or 0.5)
...
for sample = 1, 9 do
    ...
    if sample % 2 == 1 then
        laneTime, laneChecksum = measure.run(count, steps, false, dt)
        scalarTime, scalarChecksum = measure.run(count, steps, true, dt)
    else
        scalarTime, scalarChecksum = measure.run(count, steps, true, dt)
        laneTime, laneChecksum = measure.run(count, steps, false, dt)
    end
...
io.write(("explicit SIMD C %10.0f ns\n"):format(lanes * 1e9))
io.write(("scalar C        %10.0f ns\n"):format(scalar * 1e9))
io.write(("SIMD/scalar     %10.3fx\n"):format(scalar / lanes))
