...
NUPP_EXPORT NuppBytes *nuppFilesReadLink(const uint8_t *data, size_t length) {
    ...
    if (settled(&request, path.value)) {
        size_t at;
        size_t target = strlen(request.ptr);
        char *stored = (char *)request.ptr;
        /* Windows keeps a reparse point's target in its own spelling, so a link
         * made from `a/b` reads back as `a\b`. Everything here takes the one
         * separator and this is the only call that would answer the other, so a
         * caller comparing what it stored with what it reads got two spellings
         * of one path. */
        for (at = 0; at < target; at++) {
            if (stored[at] == '\\') {
                stored[at] = '/';
            }
        }
        answer = nupp_bytes_copy((const uint8_t *)stored, target);
    }
    uv_fs_req_cleanup(&request);
    nupp_text_free(&path);
    return answer;
...
/* Where a path's root ends, so the walk never tries to create it.
 *
 * A drive letter is not a directory. `mkdir C:` is refused outright rather than
 * reporting that it is already there, so every absolute Windows path failed on
 * its own first component before a real directory was ever reached. Leading
 * separators are the root on either platform and are nobody's to make. */
...
static bool make_directories(char *path) {
    ...
    size_t at;
    if (start >= length) {
        /* A path that is only a root: already there by definition. */
        return true;
    }
    /* A relative path has no root to skip, and starting at zero would hand
     * `mkdir` an empty name. */
    if (start == 0) {
        start = 1;
    }
...
NUPP_EXPORT NuppBytes *nuppFilesCanonicalize(const uint8_t *data, size_t length) {
    NuppText path;
    NuppBytes *answer;
    uv_fs_t request;
    if (!nupp_text(&path, data, length, "path")) {
    ...
    if (request.result < 0) {
        nupp_fail_format("%s: %s", path.value, uv_strerror((int)request.result));
        uv_fs_req_cleanup(&request);
        nupp_text_free(&path);
        return NULL;
    }
