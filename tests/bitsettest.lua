-- Behavioural tests for nupp.bitset.Bitset.
--
...
local bitset = require("nupp.bitset")
...
function Oracle:set(index)
    self.bits[index] = true
end

function Oracle:clear(index)
    self.bits[index] = nil
end

function Oracle:get(index)
    ...
    for word = 0, set:wordCount() - 1 do
        ...
        for offset = 0, bitset.WORD_BITS - 1 do
            ...
            if bit.band(bits, mask) ~= 0 then
                ...
                check.assert(
                    want:get(word * bitset.WORD_BITS + offset),
...
function M.randomOperationSequence()
    ...
    for step = 1, 4000 do
        ...
        elseif choice < 90 then
            ...
            local position = next(300)
            check.equal(set:get(position), want:get(position), "get at step " .. step .. " position " .. position)
        end
...
function M.walkIsStatelessAndNestable()
    local set = bitset.Bitset.__nuppCtor1(64)
    for _, position in ipairs({3, 40, 41, 300}) do
        set:set(position)
    end
...
function M.wordCountBoundsEverySetBit()
    ...
    check.assert(set:wordCount() * bitset.WORD_BITS > highest, "every set bit is below the bound")
end
...
function M.positionsIntoMatchesTheWalk()
    ...
    for _, bits in ipairs({1, 32, 33, 200, 4096}) do
        ...
        for at = 1, #expected do
            check.equal(target[at - 1], expected[at], ("position %d at width %d"):format(at, bits))
        end
    end
end
...
function M.positionsIntoResumesWhenTargetFills()
    ...
    while from >= 0 do
        ...
        for at = 1, written do
            collected[#collected + 1] = target[at - 1]
        end
        check.assert(resume == -1 or written == 7, "a partial fill means the set was exhausted")
        from = resume
    end
