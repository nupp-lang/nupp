...
function M.filesystemQuestionsUseTheGenericProvider()
    ...
    test.equal(
        assert(module.newPath("alpha//./beta/.."):absolute()):toString(),
...
function M.pathStagesOnlyTheGenericFilesystemSlice()
    ...
    source:close()
    assert(not text:find("nupp.runtime.native", 1, true), "the shared path implementation has no provider dependency")
...
function M.windowsTextRulesArePlatformIndependent()
    ...
    test.equal(pathtext.normalize("C:\\alpha\\.\\beta\\..\\file.txt", true), "C:/alpha/file.txt")
    test.equal(pathtext.normalize("C:alpha\\..\\beta", true), "C:beta")
    test.equal(pathtext.part("C:\\alpha\\.\\beta\\..\\file.tar.gz", 0, true), "C:/alpha/./beta/..")
    ...
    test.equal(pathtext.clean("C:\\alpha\\.\\\\beta\\..", true), "C:/alpha/beta/..")
    test.equal(assert(pathtext.relative("C:\\alpha\\beta", "C:\\alpha\\other", true)), "../beta")
    test.equal(pathtext.with("C:\\alpha\\file.txt", "bak", true, true), "C:/alpha/file.bak")
end
...
function M.relatingWindowsPathsComparesAnchorsByKind()
    ...
    assert(type(reason) == "string" and #reason > 0, "two drives answer why there is no relative form")
...
function M.verbatimWindowsPathsAreCarriedUntouched()
    test.equal(pathtext.normalize("\\\\?\\C:\\a\\..\\b", true), "\\\\?\\C:\\a\\..\\b")
    test.equal(pathtext.normalize("\\\\?\\UNC\\server\\share\\a\\..\\b", true), "\\\\?\\UNC\\server\\share\\a\\..\\b")
    test.equal(pathtext.clean("\\\\?\\C:\\a\\.\\\\b", true), "\\\\?\\C:\\a\\.\\\\b")
    ...
    test.equal(answer, nil)
    assert(type(reason) == "string" and #reason > 0, "a verbatim path answers why there is no relative form")
end
