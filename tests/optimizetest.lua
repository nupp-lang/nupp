...
function M.doesNotPropagateAConstPathAfterItsRootIsReplaced()
    local src = table.concat(
        {"local M = {}", "const M.bar = {const BAZ = 123}", "M = {bar = {BAZ = 456}}", "return M.bar.BAZ",},
        "\n"
    )
    local code = compile(src)
    assertTrue(
...
function M.foldsEqualityAndInequalityWithoutChangingFalseResults()
    local values = {"nil", "false", "true", "0", "1", "2", "'a'", "'b'"}
...
function M.foldsShortCircuitsWithOnlyTheLeftOperandKnown()
    local source = [[
local function choose(value: any): any
   return false and value, true or value, true and value,
      false or value, nil ?? value, false ?? value, 0 or value, 0 and value
end
return choose
]]
...
function M.shortCircuitFoldingPreservesCallsErrorsAndResultAdjustment()
    local source = [[
local calls = 0
local function values() calls += 1 return 7, 8 end
local function selected() return true and values() end
local function varargs(...) return nil ?? ... end
local discarded = false and values()
local skipped = true or error('unreachable')
local count = select('#', selected())
local first, second = selected()
return calls, count, first, second, select('#', varargs(3, 4)), varargs(3, 4)
]]
...
function M.keepsUnknownLeftShortCircuitEffectsAndTableConstruction()
    local source = [[
local calls = 0
local function value() calls += 1 return true end
local a = value() and false
local b = value() or true
local c = {const known = 1, effect = value()} and false
return calls, a, b, c
]]
...
function M.removesConstantArmsAmongDynamicConditions()
    local source = [[
local function choose(first: boolean, second: boolean): number
   if false then
      return 901
   elseif first then
      return 1
...
function M.partialBranchFoldingPreservesConditionOrderAndScope()
    local source = [[
local calls = ''
local answer = 9
local function visit(name, result) calls = calls .. name return result end
if false then
   error('unreachable')
elseif visit('a', false) then
   answer = 1
elseif false then
   answer = 2
elseif visit('b', true) then
   local answer = 3
elseif true then
   answer = 4
elseif visit('c', true) then
   answer = 5
end
return calls, answer
]]
...
function M.constantBindingsDoNotReplaceNamesBoundByAnIfClause()
    local source = [[
local value = 17
local function choose(input: boolean?): boolean | number
   if value = input then return value end
   return value
end
return choose
]]
...
local function parameter(value: number): number return value end
local function capture(): number return value * 7 end
...
function M.doesNotPropagateLocalsWrittenDirectlyOrThroughClosures()
    local cases = {
        "local value = 6\nvalue = 7\nreturn value * 7",
        "local value = 6\nvalue += 1\nreturn value * 7",
        "local value = 6\nlocal function write() value = 7 end\nwrite()\nreturn value * 7",
        "local value = 6\nlocal function write() value += 1 end\nwrite()\nreturn value * 7",
        "local value = 6\nlocal function read() return value * 7 end\nvalue = 7\nreturn read()",
        "local value = 6\nfor i = 1, 3 do value += i end\nreturn value * 7",
        "local value = 6\ndo value = 7 end\nreturn value * 7",
        "local value = 6\nlocal previous = value\nvalue, previous = 7, value\nreturn value * 7",
    }
...
function M.unchangedLocalsPreserveTableAndWideIntegerIdentity()
    local source = [[
local wide: int64 = 7
local object = {value = 6}
local alias = object
object.value = 7
return type(wide), tostring(wide), object == alias, object.value
]]
    local want = {assert(loadstring((compile(source, 0))))()}
    local got = {assert(loadstring((compile(source, 1))))()}
    for index = 1, 4 do
...
function M.refoldsAfterInliningAndPropagatesTheResult()
    local source = [[
...
function M.refoldingAnInlinedHelperCanSelectItsCallersBranch()
    local source = [[
...
local function add(a: number, b: number): number return a + b end
local value = add(2, 3)
local function choose(flag: boolean): number
    if false then return 901 elseif flag then return value * 4 else return 0 end
end
...
function M.constantControlFlowPreservesBlockExpressionEvaluation()
    local source = [[
local calls = 0
local first = false and do calls += 100 yield true end
local second = true and do calls += 1 yield false end
if false then
   calls += 1000
elseif do calls += 2 yield second end then
   calls += 10000
elseif true then
   calls += 4
elseif do calls += 100000 yield true end then
   calls += 1000000
end
...
function M.foldedControlFlowKeepsAllocationAndFeatureAccountsAccurate()
    local source = [[
local function choose(flag: boolean)
   if false then
      return {}, nupp.system.availableParallelism()
   elseif flag then
      return true and {value = 1}
   elseif true then
      return false and {}, nupp.util.uuid4()
   else
      return {}, nupp.system.availableParallelism()
   end
end
return choose
    ...
    runOptimizer(result, {level = 1})
    local tables = 0
    for _, site in ipairs(optimize.allocationSites(result)) do
...
function M.elidesConstantConditionalArms()
    local src = table.concat(
        {"if false then", "   error('unreachable')", "elseif true then", "   return 9", "else", "   return 10", "end",},
        "\n"
    )
    local code = compile(src)
...
function M.keepsIpairsAfterTheArrayBindingIsReassigned()
    local code = compile(
        table.concat(
            {"local xs: {integer} = {1, 2, 3}", "xs = {4, 5, 6, 7}", "for _, value in ipairs(xs) do print(value) end",},
            "\n"
        )
    )
    assertTrue(
