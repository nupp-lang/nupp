...
function M.buildEmitsTheDependencyClosure()
   ...
   local ran = capture(("cd '%s' && LUA_PATH='./?.lua;;' luajit main.lua")
      :format(dir))
...
function M.explicitBuildCreatesItsOutputDirectory()
   ...
   local out = capture(("cd '%s' && '%s' build -o nested/out main.nupp")
      :format(dir, NUPP))
...
function M.buildReportsErrorsInDependencies()
   local dir = tempProject({
      ["nupp.lua"] = 'return {include = {"."}}\n',
      ["lib.nupp"] = "local function double(n: number): string\n"
         .. "    return n * 2\n"
         .. "end\n"
         .. "return {double = double}\n",
      ["main.nupp"] = APP,
   ...
   assert(out:find("NUPP2002", 1, true),
      "a type error inside a dependency is reported: " .. out)
   os.execute("rm -rf '" .. dir .. "'")
end
...
function M.declarationFilesEmitNoArtifact()
   ...
   assert(exists(dir .. "/main.lua"), "entry compiled")
   assert(not exists(dir .. "/shape.lua"),
      "a declaration file describes an interface and emits nothing")
   os.execute("rm -rf '" .. dir .. "'")
end
...
function M.importCWritesAModule()
   ...
   local out = capture(("cd '%s' && '%s' import-c '%s/fixtures/mini.h' --lib mini")
      :format(dir, NUPP, HERE))
...
function M.cliProvidesCommandHelpAndValidatesOptions()
   ...
   assert(missing:find("option --target requires a value", 1, true),
      "missing option values are rejected: " .. missing)
   assert(missing:find("nupp help build", 1, true),
      "validation errors point to command help: " .. missing)

   local unknown = capture(("'%s' check --wat"):format(NUPP))
   assert(unknown:find("unknown option --wat", 1, true),
...
function M.buildAndCheckResolveTheSameDialectOption()
   ...
   local rejected = capture(("cd '%s' && '%s' check --dialect lua54 main.nupp")
      :format(dir, NUPP))
   assert(rejected:find("option --dialect does not take lua54; expected luajit, luajit-compat, lua51", 1, true),
...
function M.cliProvidesHelpForMainAndEverySubcommand()
   ...
   assert(main:find("Usage:\n  nupp <command>", 1, true),
      "--help prints main help: " .. main)
   assert(main:find("Commands:", 1, true),
      "main help lists commands: " .. main)
   ...
   for _, command in ipairs(commands) do
      ...
      assert(out:find("Usage:", 1, true),
         command .. " --help did not print command help: " .. out)
      assert(not out:find("No such file", 1, true),
         command .. " --help was interpreted as an input: " .. out)
   end
end
...
function M.astCommandDumpsTextAndJsonSyntaxTrees()
   ...
   assert(text:find("chunk\n  block\n    localStmt\n", 1, true),
      "text dump indents nested nodes: " .. text)
   assert(text:find('      local "local"\n', 1, true),
      "text dump identifies and quotes tokens: " .. text)
   assert(text:find("      binop\n", 1, true),
      "text dump includes indented expressions: " .. text)
   ...
   local function containsKind(value, kind)
      if value.kind == kind then return true end
      for _, child in ipairs(value.children or {}) do
         if containsKind(child, kind) then return true end
      end
      return false
   end
   assert(containsKind(decoded.root, "binop"),
      "JSON includes structural expression children")
   os.execute("rm -rf '" .. dir .. "'")
end
...
return {
   ...
   local dry = capture(("cd '%s' && '%s' clean --target docs --dry-run")
      :format(dir, NUPP))
...
return {build = {entries = {"main"}, outDir = "../outside"}}
   ...
   assert(out:find("refusing to clean unsafe build output path", 1, true),
      "clean rejects parent traversal: " .. out)
   os.execute("rm -rf '" .. dir .. "'")
end
...
return {
   ...
   assert(listed:find("app (default) - Build the application", 1, true),
   ...
   assert(listed:find("test - Build and run", 1, true),
      "text listing includes the configured test task: " .. listed)
   assert(listed:find("fixpoint - Verify", 1, true),
      "text listing includes the configured self-host task: " .. listed)
   assert(listed:find("app", 1, true) < listed:find("tools", 1, true),
      "text listing is sorted by task name: " .. listed)

   local detail = capture(("cd '%s' && '%s' tasks app --text"):format(dir, NUPP))
   assert(detail:find("Output directory: out", 1, true),
      "text detail includes inherited configuration: " .. detail)
   assert(detail:find("  - app.main", 1, true),
   ...
   local fixpoint = capture(("cd '%s' && '%s' tasks fixpoint --text")
      :format(dir, NUPP))
   assert(fixpoint:find("Bootstrap: bootstrap/project.lua", 1, true),
...
return {
   ...
   assert(out:find("build.default references unknown target missing", 1, true),
...
function M.compilesModulesNothingRequires()
   ...
   assert(exists(dir .. "/build/app/orphan.lua"),
      "a module nothing requires is still compiled: " .. out)
   os.execute("rm -rf '" .. dir .. "'")
end
