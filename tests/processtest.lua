...
-- the draining and the deadlines -- everything a POSIX and a Win32 backend would
-- otherwise each get wrong in their own way -- so the test supplies the platform and
-- the module supplies the behaviour.
...
function M.sharedViewsCompleteAndCloseTheBorrowedStreams()
   ...
   assertTrue(writer:close(), "closing the view succeeds")
   assertTrue(backend.state.closed["in"], "and delivers EOF to the child")
   assertTrue(reader:close(), "the reader view closes")
