...
local function fakeBackend(script)
    ...
    function self:poll(_handle)
        ...
        -- A fake platform cannot hang, so a state machine that never converges spins
        -- here forever and the suite stops reporting instead of failing. This is the
        -- backstop: no honest test in this file comes near a thousand polls, so passing
        -- one means a loop is not making progress, and a named failure beats a wedged
        -- runner.
        if state.polls > 1000 then
            error("the state machine polled without converging; something is looping", 0)
        end
        if state.exited ~= nil then
            return state.exited
        end
    ...
    function self:kill(_handle, _force)
        ...
        -- Signalling a child that is already dying is not harmless here: `killPolls`
        -- starts again below, so a caller that re-signals every pump holds the child
        -- permanently one poll away from death. Real platforms absorb that quietly;
        -- this one does not, because a deadline enforced repeatedly is the bug this
    ...
    function self:read(handle, limit)
        ...
        local list, at
        if handle == "out" then
            list, at = state.out, "outAt"
        else
            list, at = state.err, "errAt"
        end
        if state.closed[handle] then
            return nil
        end
        ...
        if state[at] >= #list then
            ...
            if script.eofWhenExited and state.exited == nil then
                return ""
            end
            return nil
        end
        -- Answers at most what was asked for, and keeps the rest for the next read --
        -- which is what a pipe does. A fake that returned everything regardless would
        -- let an adapter quietly buffer the surplus and still look correct.
    ...
    function self:write(_handle, data)
        ...
        -- blocked writing output nobody drains has stopped reading. `pendingOut` is
        -- that condition: while output is waiting to be collected, writes take nothing.
        --
        -- This is what makes the deadlock real. A sequential implementation writes all
        -- the input first, gets zero, and never progresses; only one that also drains
        -- the outputs frees the child to accept more.
        if script.blockWritesWhileUnread and state.pendingOut > 0 then
            return 0, false
        end
    ...
    function self:reap(_handle)
        ...
        state.reaped = true
        -- Released the child and complained anyway. Not `waitpid` itself -- that
        -- answers the pid on success and reports an error separately -- but a
        -- seam-level case a native handle layer really produces, when it gives
        -- ownership up and has something to say about the cleanup around it.
    ...
    function self:waitReady(interest, timeoutMs)
        -- Counted and recorded, because the two things worth knowing about this call
        -- are that it happens when there is no handler and that it never happens when
        -- there is. A fake that only returned 0 could not tell either.
        state.waits = state.waits + 1
        -- The same backstop `poll` has. A blocking wait that never converges would sit
        -- here advancing a fake clock forever, and the suite would stop reporting
        -- rather than fail; no honest test needs anything like this many sleeps.
...
function M.drainsBothPipesAndWaits()
    local backend = fakeBackend({out = {"hello ", "world"}, err = {"warning"}, exitAfter = 1, code = 0,})
...
function M.aDeadlineThatHasNotPassedDoesNotFire()
    ...
    local child = spawnOn(backend, {args = {"quick"}, timeoutMs = 1000})
    backend:advance(10)
    local result = assert(child:communicate())
    child:close()
...
function M.closingIsIdempotent()
    ...
    local child = spawnOn(backend, {args = {"true"}})
    child:wait()
    child:close()
    child:close()
    ...
    local handler = {
        park = function(_self, waiting)
            ...
            while not waiting:ready() do
                ...
                suspension.poll()
                if spins > 100 then
                    error("the drain never resolved", 0)
                end
            end
        end,
    }
...
function M.communicateClosesStdinWhenTheChildEndsEarly()
    -- A child that has already exited will never read again, so the rest of the input
    -- is undeliverable. Returning with its stdin still open would leave the pipe held
    -- by a caller who was told the conversation was over.
    local backend = fakeBackend({
        inputChunk = 4,
        stopsReadingAfter = 2,
        out = {"bye"},
    ...
    local child = spawnOn(backend, {args = {"quitter"}})
...
function M.aReapThatReleasedAndComplainedIsStillAReap()
    ...
    -- `reaped`, exactly as it decides a stream's `closed`, and the complaint is
    -- carried.
    local backend = fakeBackend({out = {}, err = {}, exitAfter = 1, code = 0})
...
function M.aRefusalWithNoReasonIsStillARefusal()
    ...
    local second = spawnOn(other, {args = {"quiet"}})
...
function M.closeCanBeRetriedAfterItFails()
    ...
    -- release state, so asserting a retry is safe after one would be asserting
...
function M.aRaiseWithNothingToSayIsStillAFailure()
    ...
    local running = spawnOn(third, {args = {"stubborn"}})
...
function M.aRefusedStreamCloseCanBeRetried()
    -- `closed` is what the platform has done, not what was asked of it. Set on a
    -- refusal it would strand the descriptor, since every later attempt returns at
    -- once.
    --
...
function M.closeReenteredFromItsOwnTeardownJustReturns()
    -- The teardown drives the pump, and something the pump drives can call back in.
    -- That is the same frame arriving twice, not two callers: it cannot wait for
    -- itself, and one teardown is enough, so the only right answer is to return.
    local backend = fakeBackend({out = {}, err = {}, exitAfter = nil, code = 0})
...
function M.aSecondCloserUnderASchedulerWaitsForTheTeardown()
    ...
    -- the scope ending runs structural drop -- leaving the next toucher on a handle
    ...
    while coroutine.status(first) ~= "dead" or coroutine.status(second) ~= "dead" do
        ...
        if coroutine.status(first) == "suspended" then
            assert(coroutine.resume(first))
        end
        if coroutine.status(second) == "suspended" then
            assert(coroutine.resume(second))
        end
        suspension.poll()
    end
...
function M.waitingWithoutAHandlerAsksThePlatformToSleep()
    -- The point of `waitReady`. With nobody to yield to, a wait has to become a sleep
    -- in the kernel; a state machine that instead polled a non-blocking backend would
    -- get the same answer as fast as the CPU could ask for it.
...
function M.waitingUnderAHandlerNeverBlocksThePlatform()
    ...
    -- running, so the state machine parks and the handler decides when to come back.
    -- The fake raises if this is got wrong, so reaching the end is the assertion.
    local backend = fakeBackend({out = {"done"}, err = {}, exitAfter = 3, code = 0})
    local child = spawnOn(backend, {args = {"quiet"}})
    local handler = {
        park = function(_self, waiting)
            ...
            while not waiting:ready() do
                ...
                suspension.poll()
                if spins > 100 then
                    error("the wait never resolved", 0)
                end
            end
        end,
    }
...
function M.aQuietChildStillHitsItsDeadline()
    ...
    local backend = fakeBackend({out = {}, err = {}, exitAfter = nil})
    local child = spawnOn(backend, {args = {"sleep"}, timeoutMs = 50})
    ...
    local handler = {
        park = function(_self, waiting)
            ...
            while not waiting:ready() do
                ...
                suspension.poll()
                if spins > 50 then
                    error("the deadline never fired", 0)
                end
            end
        end,
    }
...
function M.aDeadlineSignalsOnlyOnceWhileTerminationTakesTime()
    -- Terminating takes several polls here. A deadline re-enforced every pump would
    -- keep signalling a process already dying, and a backend that treats each signal as
    -- a fresh request would never finish.
    local backend = fakeBackend({out = {}, err = {}, exitAfter = nil})
    backend.state.killLag = 4
    backend.state.kills = 0
    local child = spawnOn(backend, {args = {"stubborn"}, timeoutMs = 10})
    local handler = {
        park = function(_self, waiting)
            local spins = 0
            while not waiting:ready() do
                ...
                suspension.poll()
                if spins > 50 then
                    error("the deadline never resolved", 0)
                end
            end
        end,
    }
