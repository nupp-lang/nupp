...
local function run(dir, argv)
    ...
    local status = os.execute(("cd '%s' && NUPP_CACHE_DIR= '%s' %s > '%s' 2>&1"):format(dir, NUPP, argv, outfile))
...
function M.reportsTheProjectFilesThatAreNotFormatted()
    ...
    assert(lines(out) == "src/messy.nupp", "only the file that is not formatted is named: " .. lines(out))
    assert(readFile(dir .. "/src/messy.nupp") == UNFORMATTED, "and nothing is rewritten without --write")
...
function M.writesTheProjectAndSaysWhatItChanged()
    ...
    assert(lines(out) == "src/messy.nupp", "the file it rewrote is named: " .. lines(out))
    assert(readFile(dir .. "/src/messy.nupp") == FORMATTED, "and rewritten")
    assert(
        readFile(dir .. "/src/tidy.nupp") == FORMATTED,
        "a file already as the formatter would have it is left alone"
    )

    local again, againOk = run(dir, "fmt")
    assert(againOk and again == "", "and a formatted project reports nothing: " .. again)
    os.execute("rm -rf '" .. dir .. "'")
end
...
function M.aFailedAtomicRewriteLeavesTheSourceWhole()
    ...
    local dir = tempProject({["locked/messy.nupp"] = UNFORMATTED,})
    ...
    assert(
        readFile(dir .. "/locked/messy.nupp") == UNFORMATTED,
...
function M.formatsDeclarationFilesToo()
    ...
    assert(lines(out) == "src/decls.d.nupp", "and is named as one of the project's files: " .. lines(out))
    assert(
        readFile(dir .. "/src/decls.d.nupp") == "global record Thing\n    id: integer\nend\n",
...
function M.aProjectWithNoIncludeRootsIsItsOwnRoot()
    ...
    assert(lines(out) == "main.nupp", "only the hand-written file is formatted: " .. lines(out))
    assert(
        readFile(dir .. "/build/generated.nupp") == UNFORMATTED,
        "generated output is not rewritten to tidy what nobody wrote"
    )
    assert(readFile(dir .. "/.worktrees/other/copy.nupp") == UNFORMATTED, "and neither is somebody else's bookkeeping")
    os.execute("rm -rf '" .. dir .. "'")
end
...
function M.leavesFilesOutsideTheIncludeRootsAlone()
    ...
    local out, ok = run(dir, "fmt -w")
    assert(ok, "formatting succeeds: " .. out)
    assert(lines(out) == "src/messy.nupp", "only the include roots are formatted: " .. lines(out))
    assert(readFile(dir .. "/examples/demo.nupp") == UNFORMATTED, "a file outside them is left as it is")
    ...
    assert(readFile(dir .. "/examples/demo.nupp") == FORMATTED, "and rewrites it")
    os.execute("rm -rf '" .. dir .. "'")
end
...
function M.namedFilesStillWriteToStdout()
    ...
    assert(formattedOut == FORMATTED, "and still comes out: " .. ("%q"):format(formattedOut))
    ...
    assert(readFile(dir .. "/src/messy.nupp") == UNFORMATTED, "with the file itself untouched")
    os.execute("rm -rf '" .. dir .. "'")
end
...
function M.checkReportsNamedFilesWithoutWritingThem()
    ...
    assert(lines(out) == "src/messy.nupp", "and only it is named: " .. lines(out))
    assert(readFile(dir .. "/src/messy.nupp") == UNFORMATTED, "a check writes nothing")

    local clean, cleanOk = run(dir, "fmt --check src/tidy.nupp")
    assert(cleanOk and clean == "", "a formatted file passes quietly: " .. clean)

    -- Reaches outside the include roots, because it was named.
    local outside, outsideOk = run(dir, "fmt --check examples/demo.nupp")
    assert(
        not outsideOk and lines(outside) == "examples/demo.nupp",
...
function M.checkWithNoFilesIsTheProject()
    ...
    assert(not ok and lines(out) == "src/messy.nupp", "the project is checked: " .. lines(out))
...
function M.checkAndWriteTogetherAreRefused()
    ...
    assert(readFile(dir .. "/src/messy.nupp") == UNFORMATTED, "with nothing written on the way out")
    os.execute("rm -rf '" .. dir .. "'")
end
...
function M.addsMethodCallParensByDefault()
    ...
    assert(againOk and again == "", "and formatting it again changes nothing: " .. again)
    os.execute("rm -rf '" .. dir .. "'")
end
...
function M.noMethodParensLeavesTheSugarAlone()
    ...
    assert(ok and out == "", "already-formatted sugar passes the check: " .. out)
    os.execute("rm -rf '" .. dir .. "'")
end
...
function M.manifestCanTurnMethodParensOffProjectWide()
    local dir = tempProject({
        ["nupp.lua"] = 'return { include = { "src" }, ' .. 'fmt = { methodParens = false } }\n',
        ["src/sugar.nupp"] = "obj:m{a = 1}\n",
    })
    local out, ok = run(dir, "fmt --check")
    assert(ok and out == "", "the manifest default leaves the sugar formatted: " .. out)
    os.execute("rm -rf '" .. dir .. "'")
end
...
function M.widthBreaksALineOverTheGivenWidth()
    ...
    assert(defaultOk and atDefault == "", "the signature already fits 120 columns: " .. atDefault)

    local narrow, narrowOk = run(dir, "fmt --width 60 --check")
    assert(not narrowOk and narrow:find("wide.nupp", 1, true), "the same signature does not fit 60: " .. narrow)

    local out, ok = run(dir, "fmt --width 60 -w")
    assert(ok, "formatting at width 60 succeeds: " .. out)
    assert(readFile(dir .. "/src/wide.nupp") ~= WIDE_SIGNATURE, "the parameter list broke onto its own lines")

    local stillNarrow, stillNarrowOk = run(dir, "fmt --width 60 --check")
    assert(stillNarrowOk and stillNarrow == "", "and formatting it again at width 60 changes nothing: " .. stillNarrow)

    local nowDefault, nowDefaultOk = run(dir, "fmt --check")
    assert(
        not nowDefaultOk and nowDefault:find("wide.nupp", 1, true),
...
function M.refusesToFormatWhatItCannotParse()
    ...
    assert(out:find("broken.nupp", 1, true), "and says which file: " .. out)
    assert(readFile(dir .. "/src/broken.nupp") == broken, "the broken file is left exactly as it is")
    assert(readFile(dir .. "/src/messy.nupp") == FORMATTED, "and the rest of the project is still formatted")
    os.execute("rm -rf '" .. dir .. "'")
end
...
function M.theProjectAnswerIsTheSameWarmAsCold()
    ...
    assert(
        lines(cold) == lines(warm),
        ("warm answer differs from cold:\n  cold: %s\n  warm: %s"):format(lines(cold), lines(warm))
    )
    ...
    assert(
        not corruptOk and lines(corrupt) == "src/tidy.nupp",
...
function M.writeStillRewritesAFileAlreadyKnownToBeUnformatted()
    ...
    assert(
        readFile(dir .. "/src/messy.nupp") == FORMATTED,
        "and the file is actually rewritten, not merely known to be wrong"
    )
    os.execute("rm -rf '" .. dir .. "'")
end

function M.bindsProjectModuleAndTypeReferencesByDefault()
    local dir = tempProject({
        ["nupp.lua"] = 'return { include = {"src"} }\n',
        [
            "src/lib/tools.nupp"
        ] = 'module lib.tools\nexport type Count = integer\nexport function count(): integer\nreturn 1\nend\n',
        ["src/app.nupp"] = 'module app\nexport function count(): lib.tools.Count\nreturn lib.tools.count()\nend\n',
    })
    local out, ok = run(dir, "fmt --write src/app.nupp")
    assert(ok, out)
    local result = readFile(dir .. "/src/app.nupp")
    assert(result:find('const {type Count} = require("lib.tools")', 1, true), result)
    assert(result:find('const tools = require("lib.tools")', 1, true), result)
    assert(result:find('function count(): Count', 1, true), result)
    assert(result:find('return tools.count()', 1, true), result)
    local checked, valid = run(dir, "check src/app.nupp")
    assert(valid, checked)
    local again, settled = run(dir, "fmt --check src/app.nupp")
    assert(settled and again == "", again)
    os.execute("rm -rf '" .. dir .. "'")
end

function M.importVerdictsExpireWhenAProjectAddsAModule()
    local dir = tempProject({
        ["nupp.lua"] = 'return { include = {"src"} }\n',
        ["src/app.g.nupp"] = 'return library.tools.count()\n',
    })
    local first, initiallyFormatted = run(dir, "fmt --check src/app.g.nupp")
    assert(initiallyFormatted, first)
    assert(os.execute("mkdir -p '" .. dir .. "/src/library'") == 0)
    local module = assert(io.open(dir .. "/src/library/tools.nupp", "wb"))
    module:write('module library.tools\nexport function count(): integer\nreturn 1\nend\n')
    module:close()
    local after, stillFormatted = run(dir, "fmt --check src/app.g.nupp")
    assert(not stillFormatted and after:find("src/app.g.nupp", 1, true), after)
    os.execute("rm -rf '" .. dir .. "'")
end

function M.preservesAnExplicitTypeModuleBindingWithItsCanonicalName()
    local dir = tempProject({
        ["nupp.lua"] = 'return { include = {"src"} }\n',
        ["src/shapes.nupp"] = 'module shapes\nexport record Point\nx: number\nend\n',
        ["src/app.g.nupp"] = 'local shapes = require("shapes")\nlocal value: shapes.Point = nil as any\nreturn value\n',
    })
    local out, ok = run(dir, "fmt --write src/app.g.nupp")
    assert(ok, out)
    local result = readFile(dir .. "/src/app.g.nupp")
    assert(not result:find('const {type Point}', 1, true), result)
    assert(result:find('value: shapes.Point', 1, true), result)
    os.execute("rm -rf '" .. dir .. "'")
end
