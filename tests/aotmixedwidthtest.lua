...
return {map = map, fixed = fixed}
    ...
    write(dir .. "/nupp.lua", [[return {include={"src"}, build={targets={native={kind="modules",entries={"mixed"},outDir="build/native",aot="require"}}}}]])
    local status = os.execute(("cd %q && %q build --target native > %q 2>&1"):format(dir, HERE .. "/../bin/nupp", dir .. "/build.log"))
