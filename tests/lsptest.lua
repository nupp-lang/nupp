...
local OWNER_PRELUDE = table.concat({
...
-- An owner acquired and later disposed is the shape `with` exists for. The
-- rewrite keeps the acquisition exactly as written and ends the scope where the
-- owner's life already ended, at the `dispose`.
function M.codeActionWrapsAnOwnerInAWithScope()
   local projectDir = tempProject()
   local source = OWNER_PRELUDE .. table.concat({
      "local function work()",
      "    local handle = open_resource(\"a\")",
      "    use(handle)",
      "    dispose(handle)",
      "end",
      "",
   ...
   assert(action.kind == "refactor.rewrite",
      "a file that checks has no diagnostic to hang a quickfix from")
   assertContains(rewritten,
      "    with handle = open_resource(\"a\") do\n        use(handle)\n    end\n",
      "the scope opens at the acquisition and closes at the dispose")
   assert(diagnostics and #diagnostics == 0, "the rewritten file checks clean")
end
...
-- An owner acquired and never touched again gives an empty scope. Opening and
-- closing it land on the same offset, so they have to be one edit: two
-- zero-width inserts would leave the client deciding which came first.
function M.codeActionWrapsAnOwnerNothingElseUses()
   local projectDir = tempProject()
   local source = OWNER_PRELUDE .. table.concat({
      "local function work()",
      "    local handle = open_resource(\"a\")",
   ...
   local edits = actionNamed(actions, "Wrap in a 'with' scope").edit.changes[uri]
   for _, edit in ipairs(edits) do
      for _, other in ipairs(edits) do
         assert(edit == other or edit.range.start.line ~= other.range.start.line
            or edit.range.start.character ~= other.range.start.character,
            "no two edits start at the same place")
      end
   end
   assertContains(rewritten,
      "    with handle = open_resource(\"a\") do\n    end\n",
      "the scope opens and closes around nothing")
   assert(diagnostics and #diagnostics == 0, "the rewritten file checks clean")
end

-- A comment between the last use and the `dispose` is about the code above it,
-- so it moves into the body with that code rather than being left behind at the
-- outer indent.
function M.codeActionCarriesTrailingCommentsIntoTheScope()
   local projectDir = tempProject()
   local source = OWNER_PRELUDE .. table.concat({
      "local function work()",
      "    local handle = open_resource(\"a\")",
   ...
   local _, _, rewritten, diagnostics = codeActionSession(projectDir,
      "owner.nupp", source, positionOf(source, "local handle"),
      "Wrap in a 'with' scope")
   os.execute("rm -rf '" .. projectDir .. "'")

   assertContains(rewritten, table.concat({
      "    with handle = open_resource(\"a\") do",
      "        use(handle)",
      "        -- done with it",
      "    end",
   }, "\n"), "the comment moves in with the code it is about")
   assert(diagnostics and #diagnostics == 0, "the rewritten file checks clean")
end

-- Unwrapping restores the manual controls `with` replaced: acquisitions in the
-- order they were written, disposals in the reverse of it. The body comes out a
-- level, down to the `end` line, so a trailing comment leaves with its code.
function M.codeActionUnwrapsAWithScopeInReverseOrder()
   local projectDir = tempProject()
   local source = OWNER_PRELUDE .. table.concat({
      "local function work()",
   ...
   local actions, _, rewritten, diagnostics = codeActionSession(projectDir,
      "owner.nupp", source, positionOf(source, "with first"),
      "Unwrap 'with' into explicit disposals")
   os.execute("rm -rf '" .. projectDir .. "'")

   assert(actionNamed(actions, "Unwrap 'with' into explicit disposals").kind
      == "refactor.rewrite", "unwrapping is a refactoring, not a fix")
   assertContains(rewritten, table.concat({
      ...
      "    dispose(second)",
      "    dispose(first)",
   }, "\n"), "cleanup keeps the order `with` gave it")
   assert(diagnostics and #diagnostics == 0, "the rewritten file checks clean")
end

-- Neither rewrite is offered where it would change what the program does:
-- `with` cannot hand back an owner it closes on the way out, and a trailing
-- `dispose` does not run on the paths a `with` body leaves early by.
function M.codeActionRefusesWithRewritesThatWouldChangeMeaning()
   local projectDir = tempProject()
   local source = OWNER_PRELUDE .. table.concat({
      "@owned(close_resource)",
      "local function handed_back(): Resource",
      "    local handle = open_resource(\"a\")",
      "    use(handle)",
      "    return handle",
      "end",
      "",
      "local function early(flag: boolean)",
      "    with handle = open_resource(\"b\") do",
   ...
   local returned = codeActionSession(projectDir, "owner.nupp", source,
      positionOf(source, "local handle"))
   local escaping = codeActionSession(projectDir, "owner.nupp", source,
      positionOf(source, "with handle"))
   os.execute("rm -rf '" .. projectDir .. "'")

   assert(#returned == 0,
      "an owner that is returned cannot be wrapped in a scope that closes it")
   assert(#escaping == 0,
      "a body that returns out of the scope cannot be unwrapped")
end

-- A client asking for one kind of action gets only that kind.
function M.codeActionHonorsTheRequestedKind()
   local projectDir = tempProject()
   local source = OWNER_PRELUDE .. table.concat({
      "local function work()",
      "    local handle = open_resource(\"a\")",
   ...
   local uri = "file://" .. path
   local position = positionOf(source, "local handle")
   local out = runSession({
      { jsonrpc = "2.0", id = 1, method = "initialize", params = {} },
      { jsonrpc = "2.0", method = "textDocument/didOpen", params = {
         textDocument = { uri = uri, languageId = "nupp", version = 1,
   ...
   local fixes = responseWithId(out, 10).result
   local refactors = responseWithId(out, 11).result
   assert(type(fixes) ~= "table" or #fixes == 0,
      "the file checks, so it has no fixes to offer")
   assert(#refactors == 1 and refactors[1].kind == "refactor.rewrite",
      "the refactoring is still there when it is what was asked for")
end
...
function M.codeActionCapabilityIsAdvertised()
   ...
   assert(kinds.quickfix and kinds["refactor.rewrite"],
...
local function playRecording(recording, mode)
   ...
   for _, ask in ipairs(asked) do
      ...
      local written = canonical(response.result):gsub(projectDir, "<project>")
