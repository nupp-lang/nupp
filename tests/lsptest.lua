...
local function runLiveSession(steps, rootDir)
    ...
    local function waitFor(needle, label)
        ...
        error(
            ...
            ):format(label or needle, complaint, output():sub(-400)),
            2
        )
    end
...
local function decodeMessages(output)
    ...
    local pos = 1
    while pos <= #output do
        local headerStart, headerEnd, len, bodyStart = output:find("Content%-Length:%s*(%d+)\r\n\r\n()", pos)
...
local function stubHost()
    ...
    host.cancel = function(id)
        cancelled[id] = true
    end
    host.isCancelled = function(id)
        return cancelled[id] == true
    end
    host.clearCancelled = function(id)
        cancelled[id] = nil
    end
    host.documentVersion = function(uri)
        return versions[uri]
    end
    host.sawVersion = function(uri, version)
...
local function diagnosticsFor(output, uri)
    ...
    for _, message in ipairs(decodeMessages(output)) do
        if message.method == "textDocument/publishDiagnostics" and message.params and message.params.uri == uri then
...
function M.positionsAgreeWithAScanFromTheStart()
    ...
    for offset = 1, #source + 2 do
        ...
        assert(
            got.line == line and got.character == character,
            ("offset %d: want %d:%d, got %d:%d"):format(offset, line, character, got.line, got.character)
        )
...
function M.anAbsolutePathUnderARelativeRootReusesItsGraph()
    ...
    client.dispatch({
        ...
        params = {
            textDocument = {uri = uri, languageId = "nupp", version = 1, text = "local value = 1\nreturn value\n"}
        }
    })
    client.dispatch({
        jsonrpc = "2.0",
    ...
    local out = runSession(
        {
            ...
            },
            {
                ...
                params = {textDocument = {uri = uri}, position = {line = 0, character = #source - 1}}
            },
...
function M.hoverShowsDocsDeclaredInAnotherFile()
    ...
    local out = runSession(
        {
            {jsonrpc = "2.0", id = 1, method = "initialize", params = {}},
            {
                ...
                method = "textDocument/didOpen",
                params = {
                    textDocument = {uri = uri, languageId = "nupp", version = 1, text = "local value: ProjectRecord\n"}
                }
            },
            {
                jsonrpc = "2.0",
    ...
    assertContains(hover.contents.value, "A record shared across files.", "cross-file docs")
end
...
function M.comptimeHoverAndCompletionExposeOnlyEvaluatorState()
    ...
    local out = runSession(
        {
            ...
            },
            {
                ...
                id = 10,
                method = "textDocument/hover",
                params = {textDocument = {uri = uri}, position = {line = 4, character = 18}}
            },
            {
                jsonrpc = "2.0",
                id = 11,
                method = "textDocument/completion",
                params = {textDocument = {uri = uri}, position = {line = 6, character = 3}}
            },
            {
                jsonrpc = "2.0",
                id = 12,
                method = "textDocument/completion",
                params = {textDocument = {uri = uri}, position = {line = 6, character = 26}}
            },
            {jsonrpc = "2.0", id = 2, method = "shutdown"},
            {jsonrpc = "2.0", method = "exit"},
        },
    ...
    assert(labels.helper and labels.inside and labels.table, "comptime helpers, locals, and libraries are completed")
    assert(not labels.runtimeOnly and not labels.print and not labels.require, "runtime-only bindings are omitted")
    local members = {}
    for _, item in ipairs(responseWithId(out, 12).result) do
        members[item.label] = true
    end
...
function M.comptimeTypeIntrinsicsCompleteOnlyOnTheNuppNamespace()
    ...
    assert(
        members.reflect and members.sizeof and members.alignof and members.offsetof,
        "the compiler type intrinsics are members of nupp"
    )

    local globals = {}
    for _, item in ipairs(responseWithId(out, 11).result) do
        globals[item.label] = true
    end
    assert(
        not globals.reflect and not globals.sizeof and not globals.alignof and not globals.offsetof,
        "the compiler type intrinsics are not bare globals"
    )
end
...
function M.queuedRequestsCanBeCancelledWhileComptimeIsInFlight()
    ...
    os.execute("rm -rf '" .. projectDir .. "'")
    assert(responseWithId(out, 10).error.code == -32800, "queued request is answered as cancelled")
end
...
function M.compilerWorkStopsAtACheckpointWhenTheRequestIsCancelled()
    ...
    client.dispatch({
        jsonrpc = "2.0",
        method = "textDocument/didOpen",
        params = {textDocument = {uri = uri, languageId = "nupp", version = 1, text = SHARED}}
    })
    ...
    local function references(id)
        client.dispatch({
            ...
            params = {textDocument = {uri = uri}, position = SHARED_POSITION, context = {includeDeclaration = true}}
        })

        return client.answer(id)
    end

    host.onPump = function()
        host.cancel(10)
    end
    ...
    assert(
        cancelled.error and cancelled.error.code == -32800,
        "work abandoned mid-answer is reported as cancelled: " .. json.encode(cancelled)
    )
    assert(
        completed.result and #completed.result > 1,
        "and the same question answers normally when nothing cancels it: " .. json.encode(completed)
    )
end
...
function M.anAnswerOvertakenByAnEditIsDiscarded()
    ...
    assert(
        overtaken.error and overtaken.error.code == -32801,
        "an answer the client has already typed past is discarded: " .. json.encode(overtaken)
    )
    assert(
        current.result and #current.result > 1,
        "and the next answer, against the version it asked about, is sent: " .. json.encode(current)
    )
end
...
function M.publishedDiagnosticsCarryTheVersionTheyWereFoundIn()
    ...
    for _, message in ipairs(decodeMessages(out)) do
        if message.method == "textDocument/publishDiagnostics" and message.params.uri == uri then
            versions[#versions + 1] = message.params.version
        end
    end
    assert(
        versions[1] == 7 and versions[#versions] == 8,
        "each publication names the version it was found in: " .. json.encode(versions)
    )
end
...
function M.hoverAndInspectExposeAutomaticCleanup()
    ...
    local hover = responseWithId(out, 10).result
    assertContains(hover.contents.value, "handle: Handle", "hover leads with the ordinary representation")
    assertContains(
        hover.contents.value,
        "Capability: cleanup `close_handle`.",
    ...
    assert(inspected.capability == "cleanup `close_handle`", "inspect returns the canonical capability summary")
    assert(
...
function M.republishesDependentDiagnostics()
    ...
    local out = runSession(
        {
            ...
            },
            {
                jsonrpc = "2.0",
                method = "textDocument/didOpen",
                params = {textDocument = {uri = consumerUri, languageId = "nupp", version = 1, text = consumer}}
            },
            {
                jsonrpc = "2.0",
                method = "textDocument/didChange",
                params = {
                    ...
                    contentChanges = {{text = "global record Shared\n   value: string\nend\n",}}
                }
            },
            {
                jsonrpc = "2.0",
    ...
    assert(#diagnostics == 3, "consumer diagnostics published on open and two interface changes")
    assert(#diagnostics[1] == 0, "consumer starts clean")
    assert(diagnostics[2][1] and diagnostics[2][1].code == "NUPP2001", "dependent receives changed export diagnostic")
    assert(#diagnostics[3] == 0, "dependent diagnostic clears when export is restored")
end
...
function M.syntaxErrorsPublished()
    ...
    local out = runSession({
        {jsonrpc = "2.0", id = 1, method = "initialize", params = {}},
        {
            jsonrpc = "2.0",
            method = "textDocument/didOpen",
            params = {textDocument = {uri = uri, languageId = "nupp", version = 1, text = "if broken(\n",}}
        },
        {jsonrpc = "2.0", id = 2, method = "shutdown"},
        {jsonrpc = "2.0", method = "exit"},
    })
...
function M.crossFileDiagnosticsPublishRelatedInformation()
    ...
    local published = diagnosticsFor(out, uri)
    local diagnostic = published[1] and published[1][1]
    assert(diagnostic and diagnostic.code == "NUPP2102", "ambiguous type is published")
    assert(#(diagnostic.relatedInformation or {}) == 2, "both declarations reach LSP related information")
    assert(diagnostic.message:find("help:", 1, true), "LSP message preserves repair help")
    os.execute("rm -rf '" .. projectDir .. "'")
end
...
function M.definitionLocations()
    ...
    for _, request in ipairs(requests) do
        messages[
            ...
            id = request.id,
            method = "textDocument/definition",
            params = {textDocument = {uri = uri}, position = {line = request.line, character = request.character}},
        }
    end
    ...
    for _, request in ipairs(requests) do
        ...
        assert(response.result and response.result.range, "definition response has a location")
        assert(response.result.range.start.line == request.wantLine, "definition line for request " .. request.id)
        assert(
            response.result.range.start.character == request.wantChar,
...
function M.countedByReferencesThePhysicalCountParameter()
    ...
    local out = runSession(
        {
            ...
            },
            {
                ...
                id = 10,
                method = "textDocument/definition",
                params = {textDocument = {uri = uri}, position = {line = 1, character = 46}}
            },
            {
                jsonrpc = "2.0",
                id = 11,
                method = "textDocument/rename",
                params = {textDocument = {uri = uri}, position = {line = 1, character = 46}, newName = "length"}
            },
            {jsonrpc = "2.0", id = 2, method = "shutdown"},
            {jsonrpc = "2.0", method = "exit"},
        },
    ...
    assert(
        definition.range.start.line == 2 and definition.range.start.character == 4,
        "countedBy resolves to the physical count parameter"
    )
    local edits = responseWithId(out, 11).result.changes[uri]
    assert(
        #edits == 2 and edits[1].newText == "length" and edits[2].newText == "length",
...
function M.renamingAnUnaliasedBindingPatternPreservesItsField()
    ...
    local out = runSession(
        {
            ...
            },
            {
                ...
                id = 10,
                method = "textDocument/rename",
                params = {textDocument = {uri = uri}, position = {line = 5, character = 7}, newName = "horizontal"}
            },
            {jsonrpc = "2.0", id = 2, method = "shutdown"},
            {jsonrpc = "2.0", method = "exit"},
        },
    ...
    assert(replacements["x as horizontal"], "the declaration keeps selecting x and introduces an alias")
...
function M.languageFeaturesAndCdefTooling()
    ...
    local messages = {
        ...
        },
        {
            jsonrpc = "2.0",
            method = "textDocument/didOpen",
            params = {textDocument = {uri = formatUri, languageId = "nupp", version = 1, text = "local x=1\n"}}
        },
        {
            jsonrpc = "2.0",
            id = 10,
        ...
        },
        {
            ...
            id = 11,
            method = "textDocument/completion",
            params = {textDocument = {uri = uri}, position = {line = 9, character = 26}}
        },
        {
            jsonrpc = "2.0",
            id = 12,
            method = "textDocument/signatureHelp",
            params = {textDocument = {uri = uri}, position = {line = 6, character = 28}}
        },
        {
            jsonrpc = "2.0",
            id = 13,
        ...
        },
        {
            ...
            params = {textDocument = {uri = uri}, position = {line = 6, character = 16}}
        },
        {
            jsonrpc = "2.0",
            id = 15,
            method = "textDocument/rename",
            params = {textDocument = {uri = uri}, position = {line = 6, character = 16}, newName = "item"}
        },
        {jsonrpc = "2.0", id = 16, method = "textDocument/semanticTokens/full", params = {textDocument = {uri = uri}}},
        {
            jsonrpc = "2.0",
            id = 17,
            method = "textDocument/definition",
            params = {textDocument = {uri = uri}, position = {line = 6, character = 22}}
        },
        {
            jsonrpc = "2.0",
            id = 18,
            method = "textDocument/formatting",
            params = {textDocument = {uri = formatUri}, options = {tabSize = 3, insertSpaces = true}}
        },
        {
            jsonrpc = "2.0",
            id = 19,
            method = "textDocument/definition",
            params = {textDocument = {uri = uri}, position = {line = 11, character = 13}}
        },
        {jsonrpc = "2.0", id = 2, method = "shutdown"},
        {jsonrpc = "2.0", method = "exit"},
    }
    ...
    local hover = responseWithId(out, 10).result
    assertContains(hover.contents.value, "cdef cAdd: function(left: int32, right: int32): int32", "cdef hover type")
    ...
    assert(labels.pinned and labels.retains and labels.releases, "managed-pointer contract completion")
    assert(labels.metamethod and labels.where, "contract syntax completion")

    local signature = responseWithId(out, 12).result
    assert(signature.activeParameter == 1, "second signature parameter active")
    assertContains(signature.signatures[1].label, "cAdd: function(left: int32, right: int32): int32", "C signature")
    ...
    for index = 1, #semantic, 5 do
        ...
        semanticCharacter = semantic[index] == 0 and semanticCharacter + semantic[index + 1] or semantic[index + 1]
        semanticAt[semanticLine .. ":" .. semanticCharacter] = tokenTypes[semantic[index + 3] + 1]
    end
    assert(semanticAt["1:0"] == "nuppKeyword", "semantic cdef keyword")
    assert(semanticAt["1:14"] == "function", "semantic C function")
    assert(semanticAt["2:12"] == "struct", "semantic C struct: " .. tostring(semanticAt["2:12"]))
    assert(semanticAt["3:3"] == "property", "semantic C field")
    assert(semanticAt["3:10"] == "type", "semantic C field type")
    assert(semanticAt["10:6"] == "variable", "contextual cdef identifier remains a variable")
    assert(semanticAt["11:6"] == "variable", "contextual own identifier remains a variable")
    assert(semanticAt["12:6"] == "nuppKeyword", "semantic type keyword")
    local definition = responseWithId(out, 17).result
    assert(definition.range.start.line == 3 and definition.range.start.character == 3, "C struct field definition")
    ...
    assert(
        contextualDefinition.range.start.line == 10 and contextualDefinition.range.start.character == 6,
...
function M.contractSyntaxSemanticTokens()
    ...
    local out = runSession({
        ...
        {jsonrpc = "2.0", id = 10, method = "textDocument/semanticTokens/full", params = {textDocument = {uri = uri}}},
        {jsonrpc = "2.0", id = 2, method = "shutdown"},
        {jsonrpc = "2.0", method = "exit"},
    })
    local initialize = responseWithId(out, 1).result.capabilities
...
function M.unsafeAndNosuspendAreSemanticKeywords()
    ...
    local types = responseWithId(out, 1).result.capabilities.semanticTokensProvider.legend.tokenTypes
    local data = responseWithId(out, 10).result.data
    local line, character = 0, 0
    local at = {}
    for index = 1, #data, 5 do
...
function M.embeddedStringSyntaxLeavesTheLiteralToTheTextMateGrammar()
    ...
    local tokenTypes = responseWithId(out, 1).result.capabilities.semanticTokensProvider.legend.tokenTypes
    local data = responseWithId(out, 10).result.data
    local line, character = 0, 0
    local at = {}
    for index = 1, #data, 5 do
    ...
    assert(at["1:22"] == nil, "the embedded long string does not receive an overriding string token")
end
...
function M.packBindersHaveTypeParameterEditorSemantics()
    ...
    local out = runSession(
        {
            ...
            },
            {
                ...
                id = 12,
                method = "textDocument/rename",
                params = {textDocument = {uri = uri}, position = {line = 0, character = 23}, newName = "Values"}
            },
            {
                jsonrpc = "2.0",
                id = 13,
    ...
    local definition = responseWithId(out, 10).result
    assert(
        definition.range.start.line == 0 and definition.range.start.character == 23,
        "pack splice resolves to its binder"
    )
    assert(#responseWithId(out, 11).result == 3, "binder and both pack references are found")
    local edits = responseWithId(out, 12).result.changes[uri]
    assert(#edits == 3 and edits[1].newText == "Values", "pack binder rename covers every splice")
    ...
    assert(
        at["0:23"] == "typeParameter" and at["0:34"] == "typeParameter" and at["0:41"] == "typeParameter",
...
function M.utf16Positions()
    ...
    local out = runSession({
        ...
        },
        {
            ...
            id = 10,
            method = "textDocument/definition",
            params = {textDocument = {uri = uri}, position = {line = 0, character = 37}}
        },
        {jsonrpc = "2.0", id = 2, method = "shutdown"},
        {jsonrpc = "2.0", method = "exit"},
    })
...
function M.constEditorSemantics()
    ...
    local out = runSession({
        ...
        },
        {
            ...
            id = 10,
            method = "textDocument/hover",
            params = {textDocument = {uri = uri}, position = {line = 1, character = 14},}
        },
        {jsonrpc = "2.0", id = 11, method = "textDocument/semanticTokens/full", params = {textDocument = {uri = uri}}},
        {jsonrpc = "2.0", id = 2, method = "shutdown"},
        {jsonrpc = "2.0", method = "exit"},
    })

    local hover = responseWithId(out, 10).result
    assertContains(hover.contents.value, "const answer: integer", "const hover prefix")

    local initialize = responseWithId(out, 1).result.capabilities
    local tokenTypes = initialize.semanticTokensProvider.legend.tokenTypes
    local modifiers = initialize.semanticTokensProvider.legend.tokenModifiers
    assert(
        modifiers[1] == "declaration" and modifiers[2] == "readonly" and modifiers[3] == "deprecated",
    ...
    for index = 1, #data, 5 do
        line = line + data[index]
        character = data[index] == 0 and character + data[index + 1] or data[index + 1]
        semanticAt[line .. ":" .. character] = {kind = tokenTypes[data[index + 3] + 1], modifiers = data[index + 4],}
    end
    assert(semanticAt["0:0"].kind == "nuppKeyword", "const keyword semantic token")
    assert(semanticAt["0:6"].modifiers == 3, "const declaration is declaration + readonly")
...
function M.deprecatedApisReachHoverCompletionAndSemanticTokens()
    ...
    local hover = responseWithId(out, 10).result
    assertContains(hover.contents.value, "**Deprecated.** kept for compatibility", "deprecated hover reason")
    assertContains(hover.contents.value, "Use `current` instead.", "deprecated hover replacement")

    local legacy
    for _, item in ipairs(responseWithId(out, 11).result) do
        if item.label == "legacy" then
            legacy = item
        end
    end
    assert(legacy and legacy.tags and legacy.tags[1] == 1, "completion carries CompletionItemTag.Deprecated")
    ...
    assert(semanticAt["1:15"] == 5, "deprecated declaration is declaration + deprecated")
...
function M.builtinAnnotationMemberHoverLinksToDocsWithNoFabricatedDefinition()
    ...
    assert(
        definition == nil or definition == json.NULL,
        "builtin annotation member reports no fabricated definition location: " .. json.encode(definition)
    )
end
...
function M.borrowReturnIsAKeyword()
    ...
    for index = 1, #data, 5 do
        line = line + data[index]
        character = data[index] == 0 and character + data[index + 1] or data[index + 1]
        semanticAt[line .. ":" .. character] = tokenTypes[data[index + 3] + 1]
    end
    assert(semanticAt["3:43"] == "nuppKeyword", "borrows in a return annotation is a keyword")
end
...
function M.predicateReturnIsAKeyword()
    ...
    assert(semanticAt["1:45"] == "nuppKeyword", "predicate-return is is a keyword")
    assert(semanticAt["2:16"] == "operator", "expression is remains an operator")
end
...
function M.docCommentsDeferToTextMateScopes()
    ...
    assert(semanticAt["0:0"] == "comment", "ordinary comments retain semantic highlighting")
    for docLine = 1, 4 do
        assert(not semanticLines[docLine], "doc line " .. docLine .. " is left to TextMate highlighting")
    end
end
...
function M.annotationTypeReferencesHaveDefinitions()
    ...
    local out = runSession({
        ...
        },
        {
            ...
            id = 10,
            method = "textDocument/definition",
            params = {textDocument = {uri = uri}, position = {line = 7, character = 11}}
        },
        {jsonrpc = "2.0", id = 2, method = "shutdown"},
        {jsonrpc = "2.0", method = "exit"},
    })
    ...
    assert(
        definition.range.start.line == 6 and definition.range.start.character == 7,
...
function M.namedTerminalsHaveDefinitions()
    ...
    local out = runSession({
        ...
        },
        {
            ...
            id = 10,
            method = "textDocument/definition",
            params = {textDocument = {uri = uri}, position = {line = 2, character = 58}}
        },
        {jsonrpc = "2.0", id = 2, method = "shutdown"},
        {jsonrpc = "2.0", method = "exit"},
    })
    ...
    assert(
        definition.range.start.line == 0 and definition.range.start.character == 14,
...
function M.qualifiedMembersNavigateAcrossFiles()
    ...
    assert(
        ...
        "definition points at the member, not the table it sits on: " .. tostring(location.range.start.character)
    )

    local hover = responseWithId(out, 11).result
    assert(hover and hover.contents, "qualified member hover missing")
    assertContains(hover.contents.value, "A point in the plane.", "qualified member docs")
end
...
function M.reportsAMissingRequireGentlyWhileEditing()
    ...
    assert(last[1].severity == 2, "an editor does not shout: severity " .. tostring(last[1].severity))
end
...
function M.renamesTheMemberAndNotItsTable()
    ...
    local out = runSession(
        {
            ...
            -- on `Point` of the declaration
            {
                ...
                id = 10,
                method = "textDocument/rename",
                params = {textDocument = {uri = uri}, position = {line = 2, character = 15}, newName = "Spot"}
            },
            {jsonrpc = "2.0", id = 2, method = "shutdown"},
            {jsonrpc = "2.0", method = "exit"},
        },
    ...
    for _, edit in ipairs(edits) do
        ...
        assert(edit.range.start.line == edit.range["end"].line, "an edit stays on one line")
        local width = edit.range["end"].character - edit.range.start.character
        assert(width == 5, "an edit covers `Point`, not `shapes.Point`: width " .. width)
    end
end
...
function M.renamesAcrossFilesTheEditorHasNotOpened()
    ...
    local out = runSession(
        {
            {jsonrpc = "2.0", id = 1, method = "initialize", params = {}},
            {
                jsonrpc = "2.0",
                method = "textDocument/didOpen",
                params = {textDocument = {uri = modelUri, languageId = "nupp", version = 1, text = declaration}}
            },
            {
                jsonrpc = "2.0",
                method = "textDocument/didOpen",
            ...
            -- on `Point` of the declaration
            {
                ...
                id = 10,
                method = "textDocument/rename",
                params = {textDocument = {uri = modelUri}, position = {line = 0, character = 14}, newName = "Spot"}
            },
            {
                jsonrpc = "2.0",
                id = 11,
    ...
    assert(edited["closed.nupp"] == 1, "the file nobody opened is renamed too")
    assert(edited["opened.nupp"] == 1 and edited["model.nupp"] == 1, "and so are the declaration and the open use")
    assert(edited["unrelated.nupp"] == nil, "a file that never names the symbol is not touched")
    ...
    table.sort(named)
    assert(
        table.concat(named, " ") == "closed.nupp opened.nupp",
...
function M.refusesToRenameASymbolTheProjectDoesNotDeclare()
    ...
    local out = runSession(
        {
            ...
            },
            {
                ...
                id = 10,
                method = "textDocument/rename",
                params = {textDocument = {uri = uri}, position = {line = 0, character = 22}, newName = "toText"}
            },
            {
                jsonrpc = "2.0",
                id = 11,
    ...
    assert(
        responseWithId(out, 11).result == nil or responseWithId(out, 11).result == json.NULL,
        "and the editor is told before it asks for a new name: " .. json.encode(responseWithId(out, 11))
    )
end
...
function M.completionOffersOnlyNamesThatResolve()
    ...
    assert(not labels.Point, "a member's bare name is not offered: it would not resolve here")
end
...
function M.completionOffersAMemberAsItsPathInItsOwnFile()
    ...
    assert(
        labels["shapes.Point"],
        "the member is offered as the path it was declared under; got: " .. table.concat(seen, " ")
    )
...
function M.completionAfterAModuleDotOffersOnlyItsMembers()
    ...
    local out = runSession(
        {
            {jsonrpc = "2.0", id = 1, method = "initialize", params = {}},
            {
                jsonrpc = "2.0",
                method = "textDocument/didOpen",
                params = {textDocument = {uri = uri, languageId = "nupp", version = 1, text = opened}}
            },
            {
                jsonrpc = "2.0",
                id = 11,
            ...
            -- the dot is typed and the name is not, which is when a client asks
            {
                jsonrpc = "2.0",
                method = "textDocument/didChange",
                params = {textDocument = {uri = uri, version = 2}, contentChanges = {{text = typing}}}
            },
            {
                jsonrpc = "2.0",
                id = 10,
    ...
    assert(byName.Handle.tags and byName.Handle.tags[1] == 1, "a deprecated exported type carries the completion tag")
    assert(byName.double, "and a plain member it carries")
    assert(byName.double.tags and byName.double.tags[1] == 1, "with its deprecated completion tag")
    assert(
        byName.double.documentation and byName.double.documentation:find("Doubles a number.", 1, true),
        "with the docblock it was written under"
    )
    assert(not byName.hidden, "a file-private function is not a member")
    assert(
        not byName["local"] and not byName.util and not byName.string,
        "and nothing ambient is offered: none of it resolves after a dot"
    )
    local hover = responseWithId(out, 11).result
    assertContains(hover.contents.value, "**Deprecated.** use the precise operation", "cross-module deprecated hover")
...
function M.completionAfterAValueDotOffersItsFields()
    ...
    local out = runSession(
        {
            ...
            },
            {
                ...
                id = 10,
                method = "textDocument/completion",
                params = {textDocument = {uri = uri}, position = {line = 12, character = 12}}
            },
            {
                jsonrpc = "2.0",
                method = "textDocument/didChange",
                params = {textDocument = {uri = uri, version = 3}, contentChanges = {{text = method}}}
            },
            {
                jsonrpc = "2.0",
                id = 11,
    ...
    assert(not sendable.x and not sendable.y, "and not the fields, which cannot be")
end
    ...
    local out = runSession(
        {
            ...
            },
            {
                ...
                id = 10,
                method = "textDocument/completion",
                params = {textDocument = {uri = uri}, position = {line = 4, character = 19}}
            },
            {jsonrpc = "2.0", id = 2, method = "shutdown"},
            {jsonrpc = "2.0", method = "exit"},
        },
    ...
    local out = runSession(
        {
            ...
            },
            {
                jsonrpc = "2.0",
                method = "textDocument/didChange",
                params = {textDocument = {uri = uri, version = 2}, contentChanges = {{text = cTyping}}}
            },
            {
                jsonrpc = "2.0",
                id = 10,
            ...
            },
            {
                jsonrpc = "2.0",
                method = "textDocument/didChange",
                params = {textDocument = {uri = uri, version = 3}, contentChanges = {{text = requireTyping}}}
            },
            {
                jsonrpc = "2.0",
                id = 11,
                method = "textDocument/completion",
                params = {textDocument = {uri = uri}, position = {line = 2, character = 26}}
            },
            {jsonrpc = "2.0", id = 2, method = "shutdown"},
            {jsonrpc = "2.0", method = "exit"},
        },
...
function M.completionInATypePositionOffersOnlyTypes()
    ...
    local out = runSession(
        {
            ...
            },
            {
                ...
                id = 10,
                method = "textDocument/completion",
                params = {textDocument = {uri = uri}, position = {line = 11, character = 9}}
            },
            {jsonrpc = "2.0", id = 2, method = "shutdown"},
            {jsonrpc = "2.0", method = "exit"},
        },
    ...
    table.sort(seen)
    assert(labels.Marker, "a declared type is offered; got: " .. table.concat(seen, " "))
    assert(labels.number and labels.string, "and so are the built-in ones")
    assert(labels["function"], "with the keyword that opens a function type, which is written here too")
...
function M.plainModuleMembersNavigateAcrossFiles()
            ...
            out = runSession(
                {
                    {jsonrpc = "2.0", id = 1, method = "initialize", params = {}},
                    {
                        jsonrpc = "2.0",
                        method = "textDocument/didOpen",
                        params = {textDocument = {uri = uri, languageId = "nupp", version = 1, text = use}}
                    },
    ...
    assert(location.uri:match("util%.nupp$"), "the definition is in the declaring module: " .. tostring(location.uri))
    assert(
        location.range.start.line == 3,
        "on the `function util.double` line: " .. tostring(location.range.start.line)
    )
    assert(
        location.range.start.character == 14 and location.range["end"].character == 20,
...
function M.plainModuleMembersAreRenamedAcrossTheProject()
            ...
            return runSession(
                {
                    ...
                    },
                    {
                        ...
                        id = 12,
                        method = "textDocument/rename",
                        params = {textDocument = {uri = uri}, position = {line = 2, character = 17}, newName = "twice"}
                    },
                    {
                        jsonrpc = "2.0",
                        id = 13,
    ...
    assert(
        counted["util.nupp"] == 3,
        "so are the declaration and the two calls inside the module itself: " .. tostring(counted["util.nupp"])
    )
    assert(counted["other.nupp"] == nil, "another module's member of the same name is a different member")

    local uses = 0
    for _, location in ipairs(responseWithId(out, 11).result) do
        assert(
            not (location.uri:match("util%.nupp$") and location.range.start.line == 3),
    ...
    for uri, edits in pairs(changes) do
        ...
        for _, edit in ipairs(edits) do
            ...
            local width = edit.range["end"].character - edit.range.start.character
            assert(width == 6, "and covers `double`, not `util.double`: width " .. width)
        end
    end
    assert(
        edited["use.nupp"] == 1 and edited["util.nupp"] == 3,
    ...
    assert(
        responseWithId(out, 13).result.placeholder == "double",
...
function M.initializeAdoptsTheClientsWorkspaceFolders()
    ...
    local out = runSession(
        {
            ...
            },
            {
                ...
                id = 10,
                method = "textDocument/definition",
                params = {textDocument = {uri = uri}, position = {line = 2, character = 18}}
            },
            {jsonrpc = "2.0", id = 2, method = "shutdown"},
            {jsonrpc = "2.0", method = "exit"},
        },
    ...
    local published = diagnosticsFor(out, uri)
    local last = published[#published]
    assert(last and #last == 0, "a module in the second folder resolves: " .. json.encode(last or json.NULL))
    local location = responseWithId(out, 10).result
    assert(
        location and location.uri:match("shared%.g%.nupp$"),
...
function M.workspaceFolderChangesAreAppliedToTheOpenSession()
    ...
    local published = diagnosticsFor(out, uri)
    assert(#published >= 3, "every folder change republishes: " .. tostring(#published))
    ...
    assert(
        responseWithId(out, 10).result ~= json.NULL and responseWithId(out, 10).result,
        "definition reaches the added folder"
    )
    assert(
        responseWithId(out, 11).result == json.NULL or responseWithId(out, 11).result == nil,
        "and stops reaching it once the folder is gone: " .. json.encode(responseWithId(out, 11))
    )
end
...
function M.eachFolderIsReadUnderItsOwnConfiguration()
    local rootDir = makeDir()
    local otherDir = makeDir()
    writeInto(rootDir, "nupp.lua", 'return { lints = { exhaustiveness = "off" } }\n')
    local source = table.concat(
        {
    ...
    local out = runSession(
        {
            ...
            },
            {
                jsonrpc = "2.0",
                method = "textDocument/didOpen",
                params = {textDocument = {uri = hereUri, languageId = "nupp", version = 1, text = source}}
            },
            {
                jsonrpc = "2.0",
                method = "textDocument/didOpen",
                params = {textDocument = {uri = thereUri, languageId = "nupp", version = 1, text = source}}
            },
            {jsonrpc = "2.0", id = 2, method = "shutdown"},
            {jsonrpc = "2.0", method = "exit"},
        },
    ...
    assert(not codes(hereUri).NUPP2107, "the folder that turned the lint off does not report it")
    assert(codes(thereUri).NUPP2107, "the folder that did not is unaffected by its neighbour's manifest")
end
...
function M.answersSayWhichFolderTheyCameFrom()
    ...
    assert(
        inspected.root == otherDir,
        "a symbol is described by the folder that owns its file: " .. json.encode(inspected)
    )
    local symbols = responseWithId(out, 11).result
    assert(
        symbols[1] and symbols[1].data.root == otherDir,
        "and a workspace search says which folder each declaration is in: " .. json.encode(symbols)
    )
end
...
function M.workspaceFolderCapabilityIsAdvertised()
    ...
    local folders = responseWithId(out, 1).result.capabilities.workspace.workspaceFolders
    assert(folders.supported == true, "workspace folders are supported")
    assert(folders.changeNotifications == true, "and the server wants to hear about changes to them")
end
...
function M.watchedFileCreationJoinsTheProject()
    ...
    local out = runLiveSession(
        {
            ...
            {jsonrpc = "2.0", id = 10, method = "textDocument/references", params = references},
            -- Written while the server is already running, so nothing but the
            -- notification can put it in the project.
            function()
                writeInto(rootDir, "late.nupp", "local b: Point\n")
            end,
            {
                jsonrpc = "2.0",
    ...
    assert(#responseWithId(out, 10).result == 0, "a file that does not exist yet is nobody's reference")
    assert(#responseWithId(out, 11).result == 1, "and once created it is a file the rename would have to edit")
end
...
function M.watchedFileChangeRechecksOpenDependents()
    ...
    assert(#published[#published] > 0, "and reporting the type the module no longer exports")
end
...
function M.watchedFileEventsOutsideTheProjectAreIgnored()
    ...
    assert(not responseWithId(out, 10).error, "the session survives events it has nothing to do with")
    local published = diagnosticsFor(out, uri)
...
local function decodeSemantic(data)
    ...
    for index = 1, #data, 5 do
        line = line + data[index]
        character = data[index] == 0 and character + data[index + 1] or data[index + 1]
        tokens[#tokens + 1] = {line = line, character = character, length = data[index + 2], kind = data[index + 3]}
    end
...
function M.semanticTokenDeltasDescribeOnlyWhatChanged()
    ...
    local out = runSession(
        {
            {jsonrpc = "2.0", id = 1, method = "initialize", params = {}},
            {
                jsonrpc = "2.0",
                method = "textDocument/didOpen",
                params = {textDocument = {uri = uri, languageId = "nupp", version = 1, text = before}}
            },
            {
                jsonrpc = "2.0",
                id = 10,
            ...
            },
            {
                jsonrpc = "2.0",
                method = "textDocument/didChange",
                params = {textDocument = {uri = uri, version = 2}, contentChanges = {{text = after}}}
            },
            {
                jsonrpc = "2.0",
                id = 11,
            ...
            -- An id the server no longer holds cannot be an edit against anything.
            {
                ...
                id = 12,
                method = "textDocument/semanticTokens/full/delta",
                params = {textDocument = {uri = uri}, previousResultId = "not-a-result"}
            },
            {
                jsonrpc = "2.0",
                id = 13,
    ...
    local applied = {}
    for index, value in ipairs(first.data) do
        applied[index] = value
    end
    local edit = delta.edits[1]
    assert(edit.start % 5 == 0 and edit.deleteCount % 5 == 0, "an edit lands on token boundaries, five integers apiece")
    local tail = {}
    for index = edit.start + edit.deleteCount + 1, #applied do
        tail[#tail + 1] = applied[index]
    end
    for index = #applied, edit.start + 1, -1 do
        applied[index] = nil
    end
    for _, value in ipairs(edit.data) do
    ...
    for index = 1, #whole do
        assert(applied[index] == whole[index], "and exactly the tokens a full request answers with, at " .. index)
    end

    assert(responseWithId(out, 12).result.data, "an unknown revision gets the whole file rather than a broken edit")
...
function M.semanticTokensForARangeAreEncodedFromTheRange()
    ...
    for _, token in ipairs(tokens) do
        assert(token.line == 2, "and only tokens from it: line " .. tostring(token.line))
    end
    assert(
        tokens[1].character == 0,
        "the first is counted from the start of its line, not from a token " .. "outside the range"
    )
end
...
function M.formattingAnswersWithTheLinesThatChanged()
    ...
    for _ = 1, 12 do
        lines[#lines + 1] = "local untouched = 2"
    end
    ...
    local out = runSession(
        {
            ...
            -- Only the last line is selected, so only its edit may come back.
            {
                ...
                params = {
                    textDocument = {uri = uri},
                    options = {},
                    range = {start = {line = 13, character = 0}, ["end"] = {line = 13, character = 13}}
                }
            },
            {jsonrpc = "2.0", id = 2, method = "shutdown"},
            {jsonrpc = "2.0", method = "exit"},
    ...
    assert(#edits == 2, "two lines changed far apart, so two edits: " .. tostring(#edits))
    for _, edit in ipairs(edits) do
        assert(edit.range["end"].line - edit.range.start.line == 1, "each covers the one line it rewrites")
        assert(not edit.newText:find("untouched", 1, true), "and no edit carries a line that did not change")
    end

    local ranged = responseWithId(out, 11).result
    assert(#ranged == 1, "the selection has one edit in it")
    assert(
        ranged[1].range.start.line == 13,
        "and it is the one inside the selection: line " .. tostring(ranged[1].range.start.line)
    )
...
function M.formattingHonorsAManifestThatTurnsMethodParensOff()
    ...
    local edits = responseWithId(out, 10).result
    assert(#edits == 0, "the manifest's default leaves the sugar formatted, so nothing to edit")
end
...
function M.rangeFormattingRefusesEditsThatReachPastTheSelection()
    ...
    assert(#responseWithId(out, 10).result == 0, "a run reaching past the selection is not offered")
    assert(#responseWithId(out, 11).result == 1, "and is offered once the selection covers it")
end
...
local function outlineSession(requests)
    ...
    local messages = {
        {jsonrpc = "2.0", id = 1, method = "initialize", params = {}},
        {
            jsonrpc = "2.0",
            method = "textDocument/didOpen",
            params = {textDocument = {uri = uri, languageId = "nupp", version = 1, text = OUTLINE}}
        },
    }
...
function M.documentSymbolsOutlineWhatTheFileDeclares()
    local out = outlineSession(function(uri)
        return {
            {jsonrpc = "2.0", id = 10, method = "textDocument/documentSymbol", params = {textDocument = {uri = uri}}}
        }
    end)
    ...
    assert(fields.x.selectionRange.start.line == 3, "each child selects its own name")

    assert(byName["shapes.Point"].range["end"].line == 9, "a declaration's range covers its whole body")
end
...
function M.workspaceSymbolsFindDeclarationsAcrossTheProject()
    ...
    local out = runSession(
        {
            {jsonrpc = "2.0", id = 1, method = "initialize", params = {}},
            {jsonrpc = "2.0", id = 10, method = "workspace/symbol", params = {query = "point"}},
            {jsonrpc = "2.0", id = 2, method = "shutdown"},
            {jsonrpc = "2.0", method = "exit"},
        },
        projectDir
    ...
    assert(found.Point.containerName == "shapes", "named with the module it belongs to")
    assert(found.Point.location.uri:match("shapes%.nupp$"), "and pointing at the file that declares it")
...
function M.documentHighlightsMarkOneBinding()
    ...
    assert(#highlights >= 2, "the field and its use are both marked: " .. tostring(#highlights))
    for _, highlight in ipairs(highlights) do
        local width = highlight.range["end"].character - highlight.range.start.character
...
function M.foldingRangesCoverBlocks()
    local out = outlineSession(function(uri)
        return {{jsonrpc = "2.0", id = 10, method = "textDocument/foldingRange", params = {textDocument = {uri = uri}}}}
    end)
    ...
    assert(spans["13:14"], "the module member's body folds to the line before its `end`")
end
        ...
        return {
            {
                ...
                params = {textDocument = {uri = uri}, positions = {{line = 14, character = 22}}}
            }
        }
    end)
...
function M.cancellationBeforeAQueuedRequestPreventsItsWork()
    ...
    for _, message in ipairs(decodeMessages(out)) do
        assert(
            not (message.error and message.error.code == -32601),
            "a notification the protocol defines is not answered with " .. "MethodNotFound"
        )
    end
    assert(responseWithId(out, 10).error.code == -32800, "the queued request is answered as cancelled")
end
...
local function positionOf(source, needle)
    ...
    while true do
        ...
        if not newline or newline >= at then
            break
        end
...
return restored, why, codec, shown
    ...
    assert(
        inspected.generatedNamespace == "instance" and inspected.generatedRecipeFingerprint,
    ...
    assert(
        responseWithId(out, 20).result == nil or responseWithId(out, 20).result == json.NULL,
        "prepareRename refuses a generated member: " .. json.encode(responseWithId(out, 20))
    )
end
...
local function applied(source, action, uri)
    local edits = {}
    for _, edit in ipairs(action.edit.changes[uri]) do
        edits[#edits + 1] = edit
    end
    ...
    for _, edit in ipairs(edits) do
        text = text:sub(
            1,
            offsetAt(source, edit.range.start) - 1
        ) .. edit.newText .. text:sub(offsetAt(source, edit.range["end"]))
    end
...
local function actionNamed(actions, title)
    ...
    error(("no action %q; offered: %s"):format(title, #titles > 0 and table.concat(titles, ", ") or "(none)"), 2)
end
...
local function codeActionSession(projectDir, name, source, position, applyTitle)
    ...
    if not applyTitle then
        return actions, uri, nil
    end
    ...
    table.insert(messages, 3, {
        jsonrpc = "2.0",
        method = "textDocument/didOpen",
        params = {textDocument = {uri = appliedUri, languageId = "nupp", version = 1, text = rewritten}}
    })
...
function M.codeActionCorrectsAMetamethodTypo()
    ...
    local actions, _, rewritten, diagnostics = codeActionSession(
        projectDir,
        "metamethod.nupp",
        source,
        positionOf(source, "__cal"),
        "change to `__call`"
    )
    os.execute("rm -rf '" .. projectDir .. "'")
    ...
    assert(actions[1].diagnostics[1].code == "NUPP2118", "the action carries the metamethod diagnostic")
    assertContains(rewritten, "{__call = function() end}", "the key is replaced in place")
...
function M.codeActionRewritesElseIf()
    ...
    assert(actions[1].diagnostics[1].code == "NUPP2510", "the action carries the else-if diagnostic")
    assert(
        rewritten:find("elseif %-%- try the secondary choice\n%s+second then"),
        "the condition follows elseif and the comment is kept"
    )
    assertContains(rewritten, "        -- nested branch\n    end", "the removed nested end leaves its comment behind")
    assert(diagnostics and #diagnostics == 0, "the rewritten file checks clean")
end
...
function M.codeActionOffersEveryVisibilityADeclarationCouldHave()
    ...
    for _, action in ipairs(actions) do
        ...
        assert(
            action.diagnostics and action.diagnostics[1].code == "NUPP2119",
    ...
    assertContains(rewritten, "--- A module of shapes.", "the header comment is untouched")
    assert(diagnostics and #diagnostics == 0, "the rewritten file checks clean")
end
...
function M.codeActionDropsTheModifierThatSaysItTwice()
    ...
    local actions, _, rewritten, diagnostics = codeActionSession(
        ...
        "shapes.nupp",
        source,
        positionOf(source, "Point"),
        "drop `local`"
    )
    os.execute("rm -rf '" .. projectDir .. "'")

    assert(#actions == 1, "one fix, and it is not a choice between two programs")
    assert(actions[1].diagnostics[1].code == "NUPP2119", "the fix carries the diagnostic it discharges")
    assertContains(rewritten, "\nrecord shapes.Point", "the modifier and the space after it are gone")
    assert(diagnostics and #diagnostics == 0, "the rewritten file checks clean")
end
...
function M.codeActionAddsTheMissingRequire()
    ...
    assert(actions[1].diagnostics[1].code == "NUPP2120", "the fix carries the diagnostic it discharges")
    assertContains(
        rewritten,
        "local helper = require(\"mathutil\")\nlocal mathutil = require(\"mathutil\")",
        "the require joins the ones already there"
    )
    assert(rewritten:match("^%-%-%- Uses the math helpers%."), "the header comment still opens the file")
    assert(diagnostics and #diagnostics == 0, "the rewritten file checks clean")
end
...
function M.codeActionQualifiesATypeThroughItsModule()
    ...
    assert(actions[1].diagnostics[1].code == "NUPP2101", "the fix carries the diagnostic it discharges")
    assertContains(rewritten, "local p: geometry.Point", "the type is reached through the local that holds the module")
    assert(diagnostics and #diagnostics == 0, "the rewritten file checks clean")
end
...
function M.codeActionRequiresAndQualifiesTogether()
    ...
    local actions, uri, rewritten, diagnostics = codeActionSession(
        projectDir,
        "use.nupp",
        source,
        {line = 0, character = 10},
    ...
    assert(
        #actions == 1 and #actions[1].edit.changes[uri] == 2,
        "one fix, two edits: either alone leaves the file broken"
    )
    assertContains(rewritten, "local shapes = require(\"shapes\")", "the require is added")
...
local function playRecording(recording, mode)
    ...
    if mode ~= "settled" then
        for index = 2, #recording.states do
            messages[
                ...
                method = "textDocument/didChange",
                params = {
                    textDocument = {uri = uriFor(recording.document), version = index},
    ...
    for _, ask in ipairs(asked) do
        ...
        assert(
            not response.error,
            (
                "%s at %s:%d:%d answered with an error: %s"
            ):format(ask.method, ask.probe.file, ask.probe.line, ask.probe.character, canonical(response.error))
        )
        ...
        assert(
            mode ~= "settled" or written ~= "null",
            (
                "%s at %s:%d:%d resolves to nothing even at rest, so the replay would " .. "be comparing two blanks"
            ):format(ask.method, ask.probe.file, ask.probe.line, ask.probe.character)
        )
    ...
    assert(
        mode ~= "burst" or broken,
        "nothing was ever reported during the burst, so no half-written state " .. "was actually replayed"
    )
    for _, message in ipairs(decodeMessages(out)) do
        assert(
            not (message.error and message.error.code == -32603),
...
local function replayRecording(name)
    ...
    if burstDiagnostics ~= settledDiags then
        error(
            (
                "%s: diagnostics did not converge.\nafter the burst:\n%s\n" .. "after one edit:\n%s"
            ):format(name, burstDiagnostics, settledDiags),
            2
        )
    end
    if burstAnswers ~= settledAnswers then
        error(
            (
                "%s: answers were not stable.\nafter the burst:\n%s\n" .. "after one edit:\n%s"
            ):format(name, burstAnswers, settledAnswers),
            2
        )
    end
end
...
do
    local listing = assert(io.popen("ls '" .. HERE .. "/lspsessions'"), "cannot list the recorded sessions")
    local names = {}
...
function M.blockCommentDocsHoverWithoutTheirDelimiters()
    ...
    assert(
        not hover.contents.value:find("[[", 1, true),
...
function M.definitionResolvesAtTheNameRightEdge()
    ...
    assert(
        location.range.start.line == 0 and location.range.start.character == 6,
...
function M.signatureHelpHighlightsTheVarargTail()
    ...
    local out = runSession(
        {
            ...
            },
            {
                ...
                id = 10,
                method = "textDocument/signatureHelp",
                params = {textDocument = {uri = uri}, position = {line = 3, character = 30}}
            },
            {jsonrpc = "2.0", id = 2, method = "shutdown"},
            {jsonrpc = "2.0", method = "exit"},
        },
    ...
    assert(#signature.signatures[1].parameters == 2, "fixed parameter and vararg tail listed")
    assert(signature.activeParameter == 1, "vararg tail active, got " .. tostring(signature.activeParameter))
end
...
function M.aDidChangeBurstAppliesTheLastFullText()
    ...
    local out = runSession({
        {jsonrpc = "2.0", id = 1, method = "initialize", params = {}},
        {
            jsonrpc = "2.0",
            method = "textDocument/didOpen",
            params = {textDocument = {uri = uri, languageId = "nupp", version = 1, text = "local a = 1\nreturn a\n"}}
        },
        {
            jsonrpc = "2.0",
            method = "textDocument/didChange",
