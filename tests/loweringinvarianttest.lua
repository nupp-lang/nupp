...
-- Every reason `gen` accepts today, and what each is. Adding a case here is the
-- decision this test exists to make deliberate: a new reason is a new loop that will
-- never compile, so it should not be possible to add one quietly.
...
function M.theInventoryOfReasonsIsExactlyWhatIsExpected()
    ...
    for name, text in pairs(found) do
        ...
        assert(
            EXPECTED_REASONS[name] == text,
            ("the reason for `%s` changed:\n  was: %s\n  now: %s"):format(name, EXPECTED_REASONS[name], text)
        )
    end
        ...
        rawRoundTripInALoop = OWNED .. "\n" .. table.concat(
            {
                ...
                "      nupp.drop(@unsafe nupp.adopt<affine(voidptr, free)>(raw))",
                "   end",
                "   n = n + 1",
                "end",
                "return n",
...
function M.aPlainLoopBuildsNoFunctionAtAll()
    local code = generate(table.concat({"local n = 0", "for i = 1, 4 do", "   n = n + 1", "end", "return n",}, "\n"))
