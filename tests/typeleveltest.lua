...
function M.comptimeTypeAliasesNameReusableDescriptorShapes()
   ...
   oneDiagnostic(table.concat({
      ...
      "return value",
   }, "\n"), "NUPP2421", 'comptime type alias "Field" is available only inside comptime')
end
...
function M.aComputedTailTypesAnUnannotatedCallbackParameter()
   clean(TUPLE_QUERY .. table.concat({
      "",
      "each(function(entity, position, velocity): nil",
...
function M.aComputedTailCarriesFieldsIntoTheCallbackBody()
   assertEq(codes(TUPLE_QUERY .. table.concat({
      "",
      "each(function(entity, position, velocity): nil",
      "   position.missing = 1",
      "end)",
...
function M.aComputedTailCountsTheCallbackParameters()
   assertEq(codes(TUPLE_QUERY .. table.concat({
      "",
      "each(function(entity, a, b, c, d): nil",
      "end)",
   }, "\n")), "NUPP2006")
end
...
function M.aComputedTailComparesAWrittenCallbackParameter()
   assertEq(codes(TUPLE_QUERY .. table.concat({
      "",
      "each(function(entity: integer, position: Position, velocity: Position): nil",
      "end)",
   }, "\n")), "NUPP2006")
end
...
function M.aTypeWithNoArgumentsHasNone()
   ...
   }, "\n"), "NUPP2420", "a cell holds one type")
end
...
function M.aPackDerivedFromTheArgumentsReachesTheCallbackBody()
   assertEq(codes(PACK_QUERY .. table.concat({
      "",
      "world:each(function(entity, position, velocity): nil",
      "   position.missing = 1",
      "end, position, velocity)",
