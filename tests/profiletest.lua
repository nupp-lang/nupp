...
local function assertMatch(text, pattern, label)
    if not tostring(text):find(pattern) then
        error(("%s: %q does not match %s"):format(label or "no match", tostring(text), pattern), 2)
    end
end
...
local function describeSampling()
    ...
    return (
        "%s (jit.profile %s, os.clock advanced %.3fs over a probe burn)"
    ):format(jit and jit.os or "?", ok and jitProfile and "present" or "absent", elapsed)
end
...
local function fnewAbortPayload()
    ...
    for code, format in pairs(vmdef.traceerr) do
        if format:find("NYI: bytecode", 1, true) then
            errorCode = code;
            break
        end
    end
    ...
    for code = 0, #vmdef.bcnames / 6 - 1 do
        if registry.opcodeName(code) == "FNEW" then
            opcode = code;
            break
        end
    end
...
local function emitFnewAbort(session)
    session.callback("abort", 1, emitFnewAbort, 0, FNEW_ERROR_CODE, FNEW_OPCODE)
end
...
function M.sampleAttributesToTheOpenZone()
    ...
    for _, line in ipairs(lines(report.text)) do
        if line:find("^frame;physics;") then
            attributed = true
        end
    end
...
function M.sampleZoneFilterKeepsThatSubtreeAlone()
    ...
    assert(
        report.samples > 0,
        ("the kept subtree was sampled, got %d in %s"):format(report.samples, describeSampling())
    )
    for _, line in ipairs(lines(report.text)) do
...
function M.sampleResumeStartsRecordingAgain()
    ...
    assert(
        report.samples > 0,
        ("what ran after the resume was recorded, got %d in %s"):format(report.samples, describeSampling())
    )
end
...
function M.traceProfileDigestIsEightHexCharacters()
    ...
    assertMatch(described.bytecodeSchema, "^%x%x%x%x%x%x%x%x$", "the bytecode schema digest")
    assertMatch(described.id, described.bytecodeSchema, "and the profile id carries it")
end
...
local function run(dir, argv)
    ...
    local status = os.execute(("cd '%s' && '%s' %s > '%s' 2>&1"):format(dir, NUPP, argv, outfile))
...
function M.cliProfileWritesCollapsedStacks()
    ...
    assert(#text > 0, ("samples were written, got %d bytes in %s"):format(#text, describeSampling()))
    for _, line in ipairs(lines(text)) do
        ...
        assert(not line:find("main%.lua"), "the compiler's frames are not the program's: " .. line)
    end
...
function M.cliProfileOutChoosesThePath()
    local dir = tempProject()
    local out, ok = run(dir, "run --profile --profile-out samples.txt work.nupp " .. sampleRepeats(4))
    assert(ok, "the program ran: " .. out)
...
function M.cliJitAbortsWritesNormalizedJson()
    local dir = tempProject()
    local out, ok = run(dir, "run --jit-aborts=aborts.json --json work.nupp 2")
    assert(ok, "the program ran: " .. out)
...
function M.jitAbortWriteFailureStillReportsTheProgramError()
    ...
    local out, ok = run(dir, "run --jit-aborts=locked/aborts.csv fail.nupp")
    assert(not ok, "both the run and its report write fail")
    assertMatch(out, "program sentinel", "the program's own failure survives the report failure: " .. out)
    assertMatch(out, "locked/aborts%.csv", "the report write failure is also retained: " .. out)
...
function M.cliRefusesADestinationForAReportNobodyAskedFor()
    ...
    out, ok = run(dir, "run --profile --profile-out a.txt --profile-out b.txt work.nupp 1")
...
function M.samplePreservesEveryVmStateInCollapsedStacks()
    ...
    session.aggregate = {
        [
            ...
            one = {
                zonePath = "",
                stack = "work:entry;library:callback",
                count = 10,
                compiled = 2,
                interpreted = 1,
                cCode = 3,
