...
-- Annotation constants retain the declared union's domain while their value keeps
-- its own primitive kind. A diagnostic must render that mismatch rather than crash
-- while trying to count a boolean as string bytes.
function M.stringDomainConstantsRenderNonStringValues()
    local term = types.constLiteral("string", false)
    assert(types.tostringConst(term) == "false")
end
...
function M.renamingABinderIsNotAChange()
    local named = types.func({types.typevar("T", "before:10:generic")}, {types.typevar("T", "before:10:generic")})
    ...
    assert(
        modules.typeFingerprint(named) == modules.typeFingerprint(renamed),
        "renaming a binder read as a changed interface: " .. modules.typeFingerprint(
            named
        ) .. " vs " .. modules.typeFingerprint(renamed)
    )
end
...
function M.projectionsOffDistinctBindersDoNotCollide()
    ...
    assert(
        modules.typeFingerprint(a) ~= modules.typeFingerprint(b),
...
function M.constBindersRenameCanonicallyAndValuesStayDistinct()
    ...
    assert(
        modules.typeFingerprint(first) == modules.typeFingerprint(second),
    ...
    assert(
        modules.typeFingerprint(sixteen) ~= modules.typeFingerprint(thirtyTwo),
