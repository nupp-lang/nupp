...
local function firstDifference(got, want)
    ...
    for line in (got .. "\n"):gmatch("(.-)\n") do
        gotLines[#gotLines + 1] = line
    end
    for line in (want .. "\n"):gmatch("(.-)\n") do
        wantLines[#wantLines + 1] = line
    end
    for index = 1, math.max(#gotLines, #wantLines) do
        if gotLines[index] ~= wantLines[index] then
            return (
                "line %d:\n  want: %q\n  got:  %q"
            ):format(index, tostring(wantLines[index]), tostring(gotLines[index]))
        end
    end
...
for _, entry in ipairs(cases()) do
    ...
    M[entry.category .. "/" .. entry.case] = function()
        ...
        local formatted, problems = formatter:format(source, base .. ".nupp")
        assert(#problems == 0, "the formatter refused the input: " .. tostring(problems[1] and problems[1].msg))
        ...
        local again = formatter:format(golden, goldenPath)
        assert(again == golden, "formatting the golden output changed it\n  " .. firstDifference(again, golden))

        assert(tokens(formatted) == tokens(source), "formatting changed the token sequence, so it was not whitespace")
