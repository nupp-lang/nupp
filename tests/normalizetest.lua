...
function M.explicitOpaqueOwnershipSurvivesGenericTypeTransforms()
    ...
    assertEq(T.tostring(rebound), "affine(string?)")
    ...
    assertEq(T.tostring(normalized), "affine(string?)")
end
...
function M.rebindingTheHeadThenNormalizingReduces()
    ...
    assertEq(T.tostring(generics.normalize(open).type), "C.Item", "an opaque projection is already a normal form")
    local bound = generics.rebind(open, {[binder] = lines})
    assertEq(T.tostring(bound), "Lines.Item", "rebinding substitutes the head and does not reduce")
...
function M.aGradualHeadReducesToAnyAndSaysSo()
    ...
    assertEq(T.tostring(materialized), "any.Item", "materializing the head does not reduce the projection")
...
function M.aDirectCycleIsReportedAndNotFollowed()
    ...
    assertEq(T.tostring(result.type), "Loop.Item", "a cycle must not collapse to any")
end
