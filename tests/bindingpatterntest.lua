...
function M.formattingUsesReadableBraces()
    assertEq(fmt.format("const{x,y as vertical}=point"), "const {x, y as vertical} = point\n")
    assertEq(fmt.format("draw({x,y}=point,color='blue')"), "draw({x, y} = point, color = 'blue')\n")
end
...
function M.oldAndAliasedPlucksCarryWholeFixes()
    ...
    assertEq(aliased.errors[1].fixes[1].edits[1].newText, "color = point.y")
end
