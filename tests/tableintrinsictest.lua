...
function M.injectsEachUsedTableBuiltinOnce()
    ...
    assertEq(occurrences(code, 'require("table.new")'), 1, "one table.new binding")
    assertEq(occurrences(code, 'require("table.clear")'), 1, "one table.clear binding")
...
function M.constructorPresizingLeavesTheUserTableNewBindingAlone()
    ...
    assertEq(occurrences(code, 'require("table.new")'), 1, "the explicit call has one table.new binding")
    assertEq(occurrences(code, "__nuppNew"), 2, "one declaration and one explicit call use the binding")
...
function M.tableIntrinsicsRemainIntrinsicAtOptimizationLevelOne()
    ...
    assertEq(occurrences(code, 'require("table.new")'), 1, "optimized source keeps the table.new binding")
    assertEq(occurrences(code, 'require("table.clear")'), 1, "OPT-4 does not capture table.clear before it is loaded")
    assertEq(code:find("__nupp_call_", 1, true), nil, "table intrinsics bypass static-callable binding")
...
function M.leavesAShadowedTableAlone()
    ...
    local code = compile(src)
    assertEq(code:find('require("table.new")', 1, true), nil, "shadowed table.new is ordinary code")
    assertEq(code:find('require("table.clear")', 1, true), nil, "shadowed table.clear is ordinary code")
    assertEq(code:find("__nuppClone", 1, true), nil, "shadowed table.clone is ordinary code")
...
function M.clonesOneLevelDeep()
    ...
    local code = compile(src)
    assertEq(occurrences(code, "local function __nuppClone"), 1, "one table.clone definition")
...
function M.clonesEachUsedIntrinsicOnce()
    ...
    local code = compile(src)
    assertEq(occurrences(code, "local function __nuppClone"), 1, "one definition serves every call")
    assertEq(occurrences(code, "__nuppClone"), 3, "one definition and two calls use the binding")
    local a, b = run(src)
...
function M.cloneBindingAvoidsSourceNames()
    ...
    local code = compile(src)
    assertEq(code:find("local function __nuppClone(", 1, true), nil, "table.clone binding avoids the source name")
