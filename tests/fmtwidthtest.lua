...
function M.softBreaksJoinWhenTheyFit()
    check(
        lines("return {diags = result.errors, moduleType = nil, syntax = true,", "    result = result}"),
...
function M.callArgumentsBreakOnePerLine()
    check(
        "local v = compute(alphaArgument, betaArgument, gammaArgument, "
...
function M.parameterListsBreak()
    check(
        "local function f(firstParameter: number, secondParameter: string, "
...
function M.customWidthBreaksALineThatFitsTheDefault()
    ...
    assertEq(
        ...
        "width 40 breaks what width 120 would not"
    )
end
...
function M.reusableInstanceAppliesTheSameWidthToEveryCall()
    ...
    assert(b:find("\n    firstParameter", 1, true), "second call broke at width 40 too, from the same instance: " .. b)
end

function M.tableConstructorsBreak()
    check(
        "local t = { alphaValue = 1, betaValue = 2, gammaValue = 3, "
...
function M.shapeFieldsAlwaysBreak()
    check(
        ...
        lines("type Pair = {", "    first: string,", "    second: number", "}")
    )
end
...
function M.operatorFallbackWhenNoGroup()
    check(
        "local s = someVeryLongVariableName .. anotherQuiteLongVariableName "
        .. ".. aThirdRatherLongVariableName .. aFourthLongVariableName .. tail\n",
        lines(
            "local s = someVeryLongVariableName",
...
function M.methodChainsHeadedByACallBreakTheSameWay()
    check(
        "local names = registry.lookup(\"identifier\"):resolve(context).entries"
        .. ":filter(predicate):map(transform):collect(sink, extraArgument)\n",
        lines(
            "local names = registry.lookup(\"identifier\")",
...
function M.methodChainStepsInsideAGroupTakeAContinuationIndent()
    check(
        "local wrapped = wrap(endpoint:withUserInfo(nil):withHost(\"api.example.com\")"
        .. ":withPort(nil):withQuery(nil):withFragment(nil):withScheme(\"https\")"
...
function M.operatorsOutrankTheChainTheyJoin()
    check(
        "local resolved = endpoint:withUserInfo(nil):withHost(\"api.example.com\")"
...
function M.aLoneMethodCallStillBreaksItsArguments()
    check(
        "local computed = self:computeSomething(firstArgumentValue, secondArgumentValue,"
        .. " thirdArgumentValue, fourthArgumentValue, fifthArgumentValue)\n",
        lines(
            "local computed = self:computeSomething(",
            "    firstArgumentValue,",
            "    secondArgumentValue,",
            "    thirdArgumentValue,",
            "    fourthArgumentValue,",
...
function M.functionDeclarationsHaveBlankAfterTheirEnd()
    check(
        lines("local function first()", "end", "local function second()", "end"),
        lines("local function first()", "end", "", "local function second()", "end")
    )

    check(
        lines("local function outer()", "local function inner()", "end", "end"),
        lines("local function outer()", "    local function inner()", "    end", "end")
    )
end
...
function M.docblockRewrapsAtEightyEight()
    local got = fmt1(
        "--- This description is deliberately long so that the "
...
function M.commentsBreakOnlyWhenSafeAndNeeded()
    local prose = "-- " .. ("ordinary prose words "):rep(5) .. "ordinary prose words\nlocal x = 1\n"
...
function M.longTernariesBreakBeforeQuestionAndColon()
    check(
        "local result = conditionWithAnExceptionallyLongAndDescriptiveName "
