...
local DEMO = spec.command{
    ...
    options = {
        ...
        {name = "--profile", value = "MS", form = "optional", help = "Sample every MS milliseconds"},
        {
            name = "-Zno-opt",
            value = "CODE",
            form = "attached",
...
function M.optionsReachTheirKeyInEverySpelling()
    ...
    assert(parse({"--out-dir", "d"}).outDir == "d", "and the key comes from the long spelling")
    assert(parse({"--strict"}).strict == true, "a flag is true when given")
    assert(parse({}).strict == nil, "and absent rather than false when it is not")
end

function M.malformedOptionsAreRefusedWithTheirOwnName()
    assert(parse({"--target"}) == "ERR: option --target requires a value", "a missing value names the option")
    assert(
        parse({
            "--target",
            "--strict"
    ...
    assert(parse({"--target="}) == "ERR: option --target requires a value", "an empty attached value is still missing")
    assert(parse({"--wat"}) == "ERR: unknown option --wat", "an unknown option is refused")
    assert(parse({"--strict=1"}) == "ERR: option --strict does not take a value", "a flag given a value is refused")
end
...
function M.choicesAreCheckedAndCanPhraseTheirOwnRefusal()
    ...
    assert(
        parse({
            "-O5"
        }) == "ERR: the optimization level is -O0, -O1 or -O2",
        "a pattern option phrases its own refusal, since -O is not a spelling"
    )
    assert(parse({"-Oz"}) == "ERR: unknown option -Oz", "and what the pattern does not match is simply unknown")
    assert(
        parse({
            "-O",
            "2"
    ...
    assert(parse({"-O=2"}) == "ERR: unknown option -O=2", "nor one that takes an attached value")
end
...
function M.anOptionalValueNeverSwallowsWhatFollowsIt()
    ...
    assert(positional({"--profile", "prog.nupp"}) == "prog.nupp", "the bare form leaves the next argument alone")
    -- The whole reason the form exists: `--profile prog.nupp` must not read the
    -- program name as an interval.
    assert(parse({"--profile", "prog.nupp"}).profile == true, "and does not take it as its value")
end
...
function M.theLiteralTerminatorEndsOptionParsing()
    assert(positional({"--", "--strict", "a"}) == "--strict,a", "everything after -- is positional")
    assert(parse({"--", "--strict"}).strict == nil, "including what would otherwise have been a flag")
    assert(positional({"a", "--strict", "b"}) == "a,b", "and positionals otherwise interleave with options")
end
...
function M.stopAtPositionalHandsTheRestToTheProgram()
    ...
    assert(
        table.concat(parsed.positional, ",") == "p.nupp,--strict,x",
...
function M.helpIsRenderedFromTheGrammarThatParses()
    ansi.withMode("never", function()
        ...
        for _, wanted in ipairs({
            ...
            assert(text:find(wanted, 1, true), "help documents " .. wanted .. ": " .. text)
        end
...
function M.completionsAreRenderedFromTheRegisteredCommandGrammar()
    ...
    assert(bash:find("text json", 1, true), "completes closed option values")
    assert(not bash:find("--color)", 1, true), "optional choices are not offered as a following word")
    ...
    assert(zsh:find("case $words[1] in", 1, true), "dispatches on the command after _arguments narrows the words")
    assert(zsh:find("if (( CURRENT == 2 )); then", 1, true), "offers positional choices at the narrowed first argument")
    assert(zsh:find("check:Type-check source", 1, true), "uses the command schema's summary")

    local fish = rendered.render("fish", commands)
    assert(fish:find("complete -c nupp", 1, true), "installs Fish completion")
    assert(
        fish:find("function __fish_nupp_needs_command", 1, true),
        "has a guard dedicated to top-level command completion"
    )
    assert(
        fish:find("-n '__fish_nupp_needs_command' -a check", 1, true),
        "offers command names before a command is present"
    )
    assert(
        fish:find("-l color -a 'always never auto'", 1, true) and not fish:find("-l color -r", 1, true),
...
function M.colourIsDecidedOncePerStreamAndOverriddenByMode()
    ...
    assert(ansi.forSeverity(painted, "error")("e") == "\27[1;31me\27[0m", "an error is red")
    assert(ansi.forSeverity(painted, "warning")("w") == "\27[1;33mw\27[0m", "a warning is yellow")
    -- A severity that is not one of the known names is treated the way the
    -- renderer already treats it: as an error.
    assert(
        ansi.forSeverity(painted, "wat")("x") == ansi.forSeverity(painted, "error")("x"),
