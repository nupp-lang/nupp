...
-- One environment for the whole suite, not one per assertion.
--
-- Every case here checks a fragment against the same environment built the same
-- way, and building one means checking the prelude: three declaration files, a
-- hundred and thirty milliseconds, thirty-eight times over. That was half of what
...
local function traceLint(src, opts)
   local result = parser.parse(src, "test.g.nupp")
   assertEq(#result.errors, 0, "syntax errors in test source")
   local diags = check.check(result, "test.g.nupp", env, opts or
