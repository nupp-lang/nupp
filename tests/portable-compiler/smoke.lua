...
local function noErrors(response, label)
    for _, diagnostic in ipairs(response.diagnostics) do
        assert(diagnostic.severity ~= "error", label .. ": " .. diagnostic.code .. ": " .. diagnostic.msg)
    end
end
...
return function(Browser)
    ...
    local compiled = session:compile(source, "playground.nupp", {strict = true, optimize = true, dialect = "lua51",})
    noErrors(compiled, "Lua 5.1 compile")
    assert(type(compiled.code) == "string" and #compiled.code > 0, "Lua 5.1 compile returned no source")
    assert(loadstring(compiled.code, "@generated-lua51"), "stock Lua 5.1 cannot parse generated Lua 5.1")

    local hover = session:hover(7)
    assert(hover.found and hover.name == "answer", "hover did not reuse the last check")

    local native = session:compile("const value = 2ULL\nreturn value", "playground.nupp", {dialect = "luajit"})
    noErrors(native, "LuaJIT compile")
    assert(native.code and native.code:find("2ULL", 1, true), "LuaJIT-only literal did not survive generation")
    for _, diagnostic in ipairs(native.diagnostics) do
        assert(diagnostic.code ~= "NUPP3005", "the Lua 5.1 host falsely parsed LuaJIT output")
    end
...
time.sleep(1)
storage.set("key", digest.hexDigest("sha256", crypto.randomBytes(16)))
print(storage.get("key"), mac.hexDigest("hmac-sha256", "key", "message"), uuid.v4(), system.availableParallelism())
]]
    ...
    assert(type(browser.code) == "string" and #browser.code > 0, "browser platform compile returned no source")
...
print(result[1], result[2])
    ...
    assert(type(browserGpu.code) == "string" and #browserGpu.code > 0, "browser WebGPU compile returned no source")

    local wire = session:request([[{"kind":"check","source":"return 1","options":{"dialect":"lua51"}}]])
    assert(type(wire) == "string" and wire:find("diagnostics", 1, true), "JSON request adapter returned no response")
    ...
    for _, name in ipairs(NUPP_REQUIRED) do
        ...
        assert(
            package.loaded[name] ~= nil or package.preload[name] ~= nil,
