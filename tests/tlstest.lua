...
function M.anOmittedAuthorityUsesThePlatformTrustStore()
    ...
    assertTrue(output:find("1 tests, 1 passed", 1, true) ~= nil, "the isolated system-root case ran:\n" .. output)
end
...
function M.resumptionCacheSeparatesProtocolsAndTrustMaterial()
    ...
    assertEq(changedClient:isResumed(), false, "a ticket cached for another ALPN offer is not resumed")
    ...
    assertEq(trustClient:isResumed(), false, "a ticket cached under another certificate set is not resumed")
...
function M.aRejectedTicketFallsBackToAFullHandshake()
    ...
    assertEq(fallbackClient:isResumed(), false, "offering a rejected ticket is not reported as resumption")
    assertEq(fallbackClient:protocol(), "http/1.1", "the replacement handshake negotiates the new server preference")
...
function M.alpnSharingNothingRefusesTheHandshake()
    ...
    local clientDone, clientWhy, serverDone, serverWhy = shake(client, server)
    assertTrue(not clientDone or not serverDone, "a handshake with no protocol in common does not complete")
