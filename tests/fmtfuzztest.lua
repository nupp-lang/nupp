...
local NAMES = {
    ...
    "rows",
    "handler",
    "aRatherLongerNameToPushLinesOverTheWidth",
    "n",
    "x"
}
local ATOMS = {
    ...
    "0.5",
    '"text"',
    '"a rather longer string literal to push a line over the width"',
    "true",
    "false",
    "nil",
    "#rows",
    "-count",
    "`a template ${count} of them`"
}
...
local function build(next_)
    local pick = function(list)
        return list[next_(#list)]
    end
    local expression
    ...
    expression = function(depth)
        ...
        elseif choice == 6 then
            return (
                "%s:%s(%s):%s(%s)"
            ):format(pick(NAMES), pick(NAMES), arguments(depth), pick(NAMES), arguments(depth))
        elseif choice == 7 then
            return ("{%s}"):format(arguments(depth))
        elseif choice == 8 then
            return ("{%s = %s, %s = %s}"):format(pick(NAMES), expression(depth + 1), pick(NAMES), expression(depth + 1))
        ...
        elseif choice == 10 then
            return ("%s ? %s : %s"):format(expression(depth + 1), pick(LITERALS), expression(depth + 1))
        end
    ...
    statement = function(depth)
        ...
        elseif choice == 7 then
            return (
                "if %s then\n%s\nelseif %s then\n%s\nelse\n%s\nend"
            ):format(expression(1), block(depth), expression(1), block(depth), block(depth))
        elseif choice == 8 then
        ...
        elseif choice == 11 then
            ...
            ):format(pick(NAMES), pick(NAMES), pick(TYPES), pick(TYPES), block(depth), expression(0))
...
local function failure(source)
    ...
    local once, problems = formatter:format(source, "fuzz.nupp")
    if #problems > 0 then
    ...
    local twice = formatter:format(once, "fuzz.nupp")
    if twice ~= once then
    ...
    for program = 1, PROGRAMS do
        ...
        if why then
            ...
            error(
                ...
                ):format(why, SEED, program, smallest),
                0
            )
        end
    end
