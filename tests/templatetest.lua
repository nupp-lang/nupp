...
local function withDevelopmentRocks(into)
   ...
   file:close()
   local changed, count = text:gsub('kind = "luarocks",',
      'kind = "luarocks",\n         tree = ' .. string.format("%q", ROOT .. "/.rocks") .. ",", 1)
...
function M.theBrowserSimdTemplateChecksBuildsAndTests()
   ...
   scaffoldAndVerify("browser-simd", "sample-browser-simd", nil, function(into)
      ...
      assert(ok and out:find("lanes", 1, true),
         "the SIMD browser kernel did not lower to lanes:\n" .. out)
   end)
end
