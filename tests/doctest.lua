...
function M.standardJsonApiHasCompleteDocumentation()
   ...
   local expected = {
      ...
      arrayOf = true, asArray = true, asObject = true, isArray = true, encode = true,
...
function M.standardManagedGroupApiHasCompleteDocumentation()
   local source = readFile(HERE .. "/../src/nupp/compiler/decls/prelude.d.nupp")
   assert(source:find("interface nupp.ManagedGroup", 1, true),
      "the root ownership surface lost ManagedGroup")
   assert(source:find("managedGroup: function", 1, true),
      "the root ownership surface lost its ManagedGroup factory")
end
...
function M.aPageTreeSectionsDirectoriesRatherThanPages()
   ...
   local sidebar = assert(page:match('<aside class="nuppdoc%-sidebar".-</aside>'),
      "the page has a sidebar")
   assert(sidebar:find("<summary>Getting started</summary>", 1, true),
      "a directory did not name a section: " .. sidebar)
   assert(not sidebar:find("<summary>Tour</summary>", 1, true),
      "a page opened a section of its own: " .. sidebar)
   assert(not sidebar:find("<summary>Installation</summary>", 1, true),
      "a page opened a section of its own: " .. sidebar)
   assert(sidebar:find(">Installation</a>", 1, true) and sidebar:find(">Tour</a>", 1, true),
...
function M.siteMatchesTheNuppdocPageModel()
   ...
   assert(home:find('data-nuppdoc-search-index="assets/search-index.js"',
      1, true), home)
   assert(home:find('<a href="https://nupp.org/modules/nupp/index.html">API</a>',
      1, true), home)
