...
function M.mathMapCorpusCoversEveryAdmittedIdentityAndVariadicForms()
    for _, target in ipairs({"native", "wasm"}) do
        local refused = target == "wasm" and {["math.sinh"]=true,["math.cosh"]=true,["math.tanh"]=true,["math.atan2"]=true} or {}
        ...
        for _, coverage in ipairs(generated.coverage) do
            ...
            for _, path in ipairs(coverage.targetRefusals) do assert(refused[path]); actualRefusals[path]=true end
            for path in pairs(refused) do assert(actualRefusals[path], "missing explicit portable refusal") end
            ...
            if coverage.element == "float" then
                for path, arity in pairs({["nupp.math.f32.min"]=2,["nupp.math.f32.max"]=2,["nupp.math.f32.fma"]=3}) do
                    local arities = assert(actual[path], "missing corrected map " .. path)
                    assert(arities[arity]); arities[arity]=nil
    ...
    local integers=maps.generate({types={"int8","uint8","int16","uint16","int32","uint32","int64","uint64"},lanes={2,"preferred"}})
    assert(next(integers.probes)==nil and #integers.coverage==0,
...
function M.portableMathTargetRefusalsRemainPositionedAndNativeAccepted()
    ...
    for _, dialect in ipairs({"luajit","lua51"}) do
        ...
        if dialect=="luajit" then assert(#diagnostics==0)
        else
            ...
            for i,diagnostic in ipairs(diagnostics) do
                ...
                assert(diagnostic.line==i+1 and diagnostic.col==10, "refusal must point at the authored math member")
            end
        end
    end
end
