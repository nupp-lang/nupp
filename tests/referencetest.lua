...
local function runCommand(directory, arguments)
    ...
    local status = os.execute(("cd %q && %q reference %s > %q 2>&1"):format(directory, NUPP, arguments, output))
...
local function diagnosticsFor(section)
    ...
    local diagnostics = {}
    project.check(dir, {strict = true, paths = {dir .. "/" .. relative}, diagnostics = diagnostics,})
    ...
    for _, section in ipairs(reference.sections) do
        if section.example then
            ...
            if #diagnostics > 0 then
                ...
                error(
                    (
                        "the %q example reports %s: %s\n%s"
                    ):format(section.title, first.code or "?", first.message or "", section.example),
                    2
                )
            end
        end
...
function M.everyCitedCodeResolves()
    for _, section in ipairs(reference.sections) do
        for _, code in ipairs(section.codes) do
            ...
            assert(entry, ("the %q section cites %s, which nothing explains"):format(section.title, code))
        end
    end
end
...
function M.everyLintAppears()
    ...
    for _, lint in ipairs(lints.all) do
        assert(markdown:find(lint.name, 1, true), ("lint %s is missing from the reference"):format(lint.name))
        assert(markdown:find(lint.code, 1, true), ("code %s is missing from the reference"):format(lint.code))
    end
end
...
function M.theSkillCarriesLoadableFrontmatter()
    ...
    assert(skill:find("# Nupp reference", closing, true), "the document follows the frontmatter")
    assert(skill:find("## Language", closing, true), "groups language sections")
    assert(skill:find("## CLI", closing, true), "groups CLI workflows")
    assert(skill:find("### Improving test coverage", closing, true), "teaches the coverage workflow")
    assert(skill:find("nupp test --coverage --report-json", closing, true), "makes coverage data queryable by agents")
end
...
function M.chaptersAreDiscoverableAndFocused()
    ...
    assert(catalogue:find("performance", 1, true), "lists the performance chapter")
    assert(catalogue:find("performance ", 1, true), "aligns the longest chapter name")
    local cli = assert(reference.chapter("cli"), "finds the CLI chapter")
    local performance = assert(reference.chapter("performance"), "finds the performance chapter")
    ...
    assert(markdown:find("Improving test coverage", 1, true), "keeps coverage guidance in the CLI chapter")
    ...
    assert(performanceMarkdown:find("# Nupp Performance reference", 1, true), "titles performance output")
    assert(
        performanceMarkdown:find("nupp lsp trace-check --json", 1, true),
        "puts deterministic inspection before profiling"
    )
    assert(performanceMarkdown:find("nupp run --profile", 1, true), "teaches sampling")
    assert(performanceMarkdown:find("nupp run --jit-aborts", 1, true), "teaches abort collection")
    assert(
        performanceMarkdown:find("jit/loop-function-construction", 1, true),
        "takes static reasons from the shared catalog"
    )
    for _, reason in ipairs(trace.records()) do
        if reason.explanation then
            assert(performanceMarkdown:find(reason.id, 1, true), "performance skill names " .. reason.id)
            assert(
                performanceMarkdown:find(reason.explanation, 1, true),
                "performance skill shares the explanation for " .. reason.id
            )
        end
    end
    local performanceSkill = reference.skill(performance)
    assert(performanceSkill:find("name: nupp-performance", 1, true), "names focused performance skill")
    assert(performanceSkill:find("performance regressions", 1, true), "describes the requests that load it")
end
...
function M.referenceCommandListsAndSelectsChapters()
    ...
    pipe:close()
    assert(catalogue:find("Nupp reference chapters", 1, true), "bare command lists chapters")
    local cliPipe = assert(io.popen(('%q reference cli'):format(NUPP)))
    local cli = cliPipe:read("*a")
    cliPipe:close()
    assert(cli:find("# Nupp CLI reference", 1, true), "CLI chapter is selectable")
    ...
    assert(performance:find("# Nupp Performance reference", 1, true), "performance chapter is selectable")
end
...
function M.allJsonListsEverySectionItsMarkdownContains()
    ...
    for _, chapter in ipairs(reference.chapters) do
        for _, section in ipairs(chapter.sections) do
            assert(byTitle[section.title], "JSON omitted the " .. section.title .. " section carried by its markdown")
        end
    end
end
...
function M.aDocsPointerNamesItsSection()
    local pipe = assert(io.popen(('%q reference --section "docs/learn/language/modules.md#modules"'):format(NUPP)))
...
function M.theCatalogueNamesEverySection()
    ...
    for _, chapter in ipairs(reference.chapters) do
        for _, section in ipairs(chapter.sections) do
            ...
            assert(catalogue:find(slug, 1, true), slug .. " is a slice the catalogue does not mention")
        end
    end
end
...
function M.everySectionExampleInTheHelpResolves()
    ...
    for name in help:gmatch("nupp reference %-%-section ([^%s]+)") do
        ...
        if not name:match("^%u+$") then
            checked = checked + 1
            local run = assert(io.popen(('%q reference --section %q >/dev/null 2>&1; echo $?'):format(NUPP, name)))
