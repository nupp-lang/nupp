...
local function temporaryRoot()
    ...
    return (
        base:gsub("/$", "")
    ) .. "/nupp-http-test-" .. tostring(os.time()) .. "-" .. unique .. "-" .. tostring(math.random(1, 1e9))
end
...
local function preloadProvider(module, variable, libraryPath)
    local found = assert(package.searchpath(module, package.path), module .. " is not on the path")
    local text = assert(io.open(found, "rb")):read("*a"):gsub('os%.getenv%("' .. variable .. '"%)', function()
...
local function ready()
    ...
    return newHttpClient({timeoutMs = 10000, maxConnections = 8, maxConnectionsPerHost = 8,})
end
...
function M.smallInlineRequestsUseThePooledPublicPath()
    ...
    local response, reason = client:send({url = endpoint("/echo"), method = "POST", body = payload,})
...
function M.buffersViewsAndFilesKeepTheirConcreteUploadPaths()
    ...
    for _, body in ipairs({buffer, buffer:view(0, buffer:length())}) do
        local response, reason = client:send({url = endpoint("/echo"), method = "POST", body = body,})
        assert(response, reason)
...
function M.selectivelyInsecureClientsRerouteEveryRedirect()
    ...
    local client = newHttpClient({insecureHosts = {"127.0.0.1"}, headers = {Authorization = "secret"},})
    ...
    local second = newHttpClient({insecureHosts = {"127.0.0.1"}, headers = {Authorization = "secret"},})
...
function M.selectivelyInsecureClientsStripCookiesAcrossOrigins()
    ...
    local client = newHttpClient({insecureHosts = {"127.0.0.1"}, headers = {Cookie = "session=secret"},})
...
function M.aCancelledPlainRequestReleasesItsTransfer()
    ...
    local value = tasks.race({
        function()
            return client:send({url = endpoint("/slow")})
        end,
        function()
    ...
    test.equal(next(client._native._byHandle), nil, "the abandoned transfer left the client's live table")
    client:close()
end
...
function M.aCancelledStreamingUploadReleasesItsReaderAndTransfer()
    ...
    local reader = {
        ...
        read = function()
            error("readInto is the upload path")
        end,
        close = function()
            return true
        end,
    ...
    local value = tasks.race({
        function()
            return client:send({url = endpoint("/slow-upload"), method = "POST", body = http.reader(reader, total),})
        end,
        function()
            return "settled first"
        end,
    ...
    test.equal(value, "settled first")
    test.equal(next(client._native._byHandle), nil, "the cancelled upload left no live native transfer")
    assert(calls * 64 * 1024 < total, "the cancelled upload stopped before consuming its reader")
    client:close()
end
...
function M.closingAResponseCancelsItsPendingBody()
    ...
    test.equal(value, "settled first")
    response:close()
    test.equal(next(client._native._byHandle), nil, "closing the response retired its pending body transfer")
    client:close()
end
...
function M.genericReadersAndLargeDownloadsStayProgressive()
    ...
    test.equal(downloaded:getString(downloaded:length() - 16, 16), "0123456789abcdef")
    response:close()
...
function M.thePublicModuleAndNativeProviderHaveSeparateFeatureDependencies()
    ready():close()
    test.equal(native.forModule("nupp.io.http"), "runtime.http")
    local feature = assert(native.feature("native.http"))
    test.equal(feature.providerFeature, "http")
    test.equal(feature.runtimeModule, "nupp.runtime.provider.nativehttp")
