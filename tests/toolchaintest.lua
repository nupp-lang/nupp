...
function M.everyPinHasAVersionAndADigest()
    local recorded = pins()
    for _, component in ipairs({"LUAJIT", "LUAROCKS", "LPEG", "LUAUTF8", "CURL", "MBEDTLS", "ADA", "LIBUV",}) do
        ...
        local digest = recorded[component .. "_SHA256"] or recorded[component .. "_CPP_SHA256"]
...
function M.anUnknownNativeFeatureStopsTheDriver()
    ...
    assert(
        output:find("unknown native feature bogus", 1, true),
...
function M.legacyCGpuFeatureIsNotBuildable()
    ...
    assert(
        output:find("unknown native feature gpu", 1, true),
...
function M.windowsHostLinkersCarryPthread()
    ...
    assert(driver:match('windows%)%s+platform_libraries="%-lpthread '), "the Windows host build does not link pthread")
    assert(
...
function M.windowsHostLinkersCarryCryptoApi()
    ...
    assert(ordinary >= 4, "not every Windows toolchain linker carries crypt32")
    assert(
...
function M.macOSHostLinkersCarryTheSecurityFramework()
    ...
    assert(security >= 4 and foundation >= 4, "not every macOS toolchain linker carries the trust-store frameworks")
    assert(
