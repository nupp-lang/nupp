...
function M.returnsTheBufferWithoutAnAccumulatorLocal()
    local source = [[
local function join(items: {string}): string
    local out = ""
    for _, item in ipairs(items) do out = out .. item .. "," end
    return out
end
...
function M.retainsTheAccumulatorWhenACaptureStillReadsIt()
    local source = [[
local out = ""
local function read(): string return out end
for i = 1, 3 do out = out .. i end
return out .. read()
]]
