...
function test.anOwnedPointerFieldIsSilent()
    -- The pointer under it reifies, but the obligation wrapped around it does not: a
    -- struct field written `affine(widget*, widget_free)` is NUPP2201. The two halves
    -- agree, which is the property worth holding -- what a struct refuses is exactly
...
local record Holder
    handle: affine(widget*, widget_free)
    n: int32
end
]]
    )
...
local record Vec2
    ...
    local on = check.check(parser.parse(src, "test.g.nupp"), "test", envMod.new("."), {lints = {performance = "note"}})
    local found = nil
...
local record Holder
    ...
    local applied = src:sub(1, edit.offset - 1) .. edit.newText .. src:sub(edit.offset + edit.length)
    assert(applied:find("^local struct Holder"), "the keyword was replaced, not the field:\n" .. applied)
    assert(applied:find("record: int32", 1, true), "the field called record survives:\n" .. applied)
end
