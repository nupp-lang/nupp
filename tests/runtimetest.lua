...
function M.preservesPreloadAndPlainLuaLoaders()
    withProject(
        ...
        function(dir)
            ...
            package.preload.runtimepreferred = function()
                return "preload"
            end
            local removeLoader = runtime.install(projectEnv(dir), compile)
...
return { a = a }
        ...
        function(dir)
            ...
            assert(
                tostring(err):find("loop or previous error", 1, true),
...
function M.cliWatchCommitsAndKeepsTheLastGoodGeneration()
    local function watched(body)
        return table.concat(
            {"local M = {}", "function M.value(): integer", "   " .. body, "end", "return M", "",},
            "\n"
        )
    end
...
return function(path, text)
   ...
   file:close()
        ...
        function(dir)
            ...
            local errors = dir .. "/errors.txt"
            local command = (
                "cd '%s' && '%s/bin/nupp' run --watch main.g.nupp " .. "> '%s' 2> '%s'"
            ):format(dir, ROOT, output, errors)
            local status = os.execute(command)
...
function M.cliWatchRejectsOptimizedGeneration()
    withProject({["main.nupp"] = "print(1)\n"}, function(dir)
        local output = dir .. "/output.txt"
        local command = (
            "cd '%s' && '%s/bin/nupp' run --watch -O1 main.nupp " .. "> '%s' 2>&1"
        ):format(dir, ROOT, output)
        local status = os.execute(command)
