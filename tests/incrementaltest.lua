...
function M.recursiveDerivedGraphRechecksAcrossThreeModules()
   ...
   write(bridgePath, table.concat({
      "module bridge",
      "local model = require('model')",
      "export function binding(): nupp.data.serde.Binding<model.Node>",
      "   return nupp.data.serde.of(model.Node)",
      "end",
   }, "\n"))
   write(mainPath, table.concat({
      "local model = require('model')",
      "local bridge = require('bridge')",
      "local binding: nupp.data.serde.Binding<model.Node> = bridge.binding()",
   ...
   assertEq(#cold.diags, 0, "three-module recursive derive checks cold: "
   ...
   assertEq(#inc.checkFile(mainPath).diags, 0,
      "recursive derive survives a dependency body edit")
   assertEq(inc.q.stats.checkModule, coldChecks + 1,
      "unchanged derived interface cuts off the two consumers")

   inc.changeDocument(modelPath, model:gsub("   children: {Node}",
      "   children: {Node}\n   tag: string"))
   local changed = inc.checkFile(mainPath)
   assertEq(inc.q.stats.checkModule, coldChecks + 4,
      "a derived record interface change rechecks all three modules")
   assertEq(#changed.diags, 0,
      "the recursive derive remains coherent after all three modules recheck")
...
function M.changingADeriveProviderReplansItsClaim()
   ...
   local changed = inc.checkFile(mainPath)
   assertEq(changed.diags[1] and changed.diags[1].code, "NUPP2004",
...
function M.deprecationMetadataInvalidatesModuleDependents()
   ...
   inc.changeDocument(depPath, dep:gsub("function M.answer",
      '@deprecated(replacement = "dep.currentAnswer")\nfunction M.answer'))
   local changed = inc.checkFile(mainPath)
   assertEq(inc.q.stats.checkModule, coldChecks + 2,
      "deprecation metadata rechecks the dependency and dependent")
   assertEq(changed.diags[1] and changed.diags[1].code, "NUPP2513",
