...
local function kindsOf(listing, mnemonic)
   for _, one in ipairs(listing.instructions) do
      if one.mnemonic == mnemonic then
         local named = {}
         for _, kind in ipairs(one.kinds) do named[kind] = true end
         return named
      end
   end
...
function M.x86ScalarFloatingPointIsNotVector()
   ...
   assert(kindsOf(listing, "vbroadcastss").vector,
...
function M.aCommentIsNotAnOperand()
   ...
   test.equal(only(ARM, "aarch64", "darwin").instructions[9].text, "subs x2, x2, #1",
...
function M.aLayoutReporterIsClaimedByTheBodyItReportsFor()
   local assembly = table.concat({
      "_ks_scale_layout_Point_offset_x:",
      "\tmov\tw0, #4",
      "\tret",
   }, "\n")
...
function M.listingsAreOrderedByRole()
   ...
   local shown = {}
   for index, listing in ipairs(ordered) do shown[index] = listing.symbol end
