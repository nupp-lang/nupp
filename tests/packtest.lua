...
function M.tableLiteralsFitHeterogeneousTupleTypes()
    clean(table.concat({"local pair: {number, string} = {1, 'one'}", "local n: number = pair[1]", "return n",}, "\n"))
end
...
function M.protectedCallOwnersAutoDestroyOnlyInTheSuccessArm()
    ...
    clean(
        declaration .. "\n" .. table.concat(
            {"local ok, resource = pcall(acquire)", "if ok then", "   print(resource)", "end",},
            "\n"
        )
    )
end
...
function M.typedVarargModesParticipateInFunctionCompatibility()
    ...
    assertEq(
        ...
        "NUPP2001",
        "pack-tail ownership is part of the function contract"
    )
end
