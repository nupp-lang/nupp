...
`run-matrix.sh` runs the ordinary `nupp test` harness. Every AOT unit is
compiled by nupp's own LLVM, in process, so the only matrix dimension besides
the host is the tier. Each tier builds two immutable fixtures: a cheap species
inventory for every public type and lane count, and a representative semantic
pack for each operation/type/representation class. Each build requests exactly
one tier (`minimum == maximum`). On x64 it executes baseline, AVX2 and AVX512F
when the CPU and OS advertise support; arm64 executes NEON. Which tiers an x64
host has is answered by the feature detector every AOT library carries, built
into a probe spanning every tier and called before any tier code runs.
...
`tests/fleet/plan.json` keeps host and instruction-tier obligations orthogonal.
It runs the minimum cells needed for Linux x64/arm64, macOS x64/arm64, Windows
x64, baseline, AVX2, AVX512F and NEON.
...
Reports retain the code generator's version, target triples, CPU capabilities,
source identity, harness facts, artifact SHA-256s, and deterministic work
counts.

```sh
tests/simd/run-matrix.sh
NUPP_SIMD_TIERS=baseline NUPP_SIMD_ALGORITHMS=none \
...
timing, with completed native-entry proof and emitted-unit tier checks.
...
also accepts `tier`, `directory`, `report`, `lua` and `nupp` options.
`NUPP_SIMD_TIER` and `NUPP_SIMD_REPORT` provide the first and third options
through the environment. A directly requested unsupported tier fails before execution.
...
call only after the original native entry returns. Every named probe must be called.
...
source, unit, call and case work counts. Missing Rust, Node or child LuaJIT
tooling is recorded as `not-executed` in an ordinary broad test run.

`run-wasm-wasmtime.sh` invokes that ordinary harness and rejects a report unless
both packs actually passed. `NUPP_WASMTIME_HOST_LIBRARY` and
`NUPP_SIMD_WASM_OUTPUT` select an optional prebuilt host and the retained
harness report. The generated browser bundle still owns the independently
...
which a native dual-number LuaJIT must not impersonate. Both SIMD and scalar-twin
...
The Wasm reference route rebuilds the project with `NUPP_AOT_WASM_ORACLE=1`
(`prepare-wasm-reference.lua`), which changes only each independent entry's
call target, to its kernel's unoptimized scalar twin. The launcher checks the
Wasm hashes and executes the same entry inventory again. Reports retain the
original and scalar-twin artifact hashes. This proves the chosen call route; it does not claim that
